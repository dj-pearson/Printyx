...
Client-metrics agent ingest (EDGE-002j, DONE): device-monitoring is split across TWO surfaces. (1) ADMIN-UI path `/api/monitoring-clients/*` — already a canonical edge fn (`supabase/functions/monitoring-clients/`, JWT-gated, in `crmProxies`); the page `MonitoringClients.tsx` calls THIS (the story's "/clients,/clients/:id,…" wording is shorthand). (2) AGENT-INGEST path `/api/client-metrics/*` — called ONLY by the printyx-client agent on customer print servers (never the browser), so it is NOT a getApiUrl prod blocker. EDGE-002j rewrote the DEPRECATED `client-metrics` edge fn (was x-api-key + phantom schema: device_metrics.client_id/metric_type/value/unit, discovered_devices, device_alerts that don't exist) into a FAITHFUL port of the LIVE Express `server/routes-client-monitoring.ts` ingest: POST /submit (find/create `device_registrations` keyed on serial_number + ensure a `manufacturer_integrations` row manufacturer='printanista' name=`Client: <name>` + insert `device_metrics` REAL cols total/bw/color/large_impressions+device_status+toner_levels/paper_levels(jsonb)+error_codes(text[])+raw_data + `client_discovered_devices` + client stats + activity log), POST /heartbeat (expire stale + dispatch queued `client_commands` via `.update().eq('status','queued').select()` + activity + auto-update signal), POST /commands/:id/ack, GET /config, POST /install-report. AUTH is Bearer-as-API-KEY (SHA-256 → `monitoring_clients.api_key`) + `x-tenant-id`, NOT a Supabase JWT — so `[functions.client-metrics] verify_jwt=false` was added to `supabase/config.toml` (signup/calculator pattern) so the native runtime doesn't reject the API-key bearer. Ported `alert-materializer.ts` core (computeDesired + insert/update/resolve `device_alerts`) and `offline-detector.ts` `resolveOfflineFor()` INLINE (plain table ops, portable); DEGRADED `maybeAutoOrder` (Node-only pipeline over product/inventory/supply-order tables) — alerts still materialise so the dashboard is right, auto-order stays Express until EDGE-011. The agent auto-update version comes from `PRINTYX_CLIENT_AGENT_VERSION` env or the baked `bundle-manifest.json` (`Deno.readTextFile`), not the Express fs read. **NOT proxied on purpose**: agents POST to their configured `endpoint` = `PUBLIC_API_BASE_URL` = the Express/app host (NOT functions.printyx.net), so the live agent path still hits Express; the edge fn is the prod-ready replacement for the EDGE-011 sunset — adding `/api/client-metrics` to crmProxies would break local agent ingest. The admin CRUD/enrollment/installer.zip Express surface is `server/routes-client-metrics.ts` (mounted at `/api/client-metrics` with `/clients/*` subpaths) — separate from the agent ingest in `routes-client-monitoring.ts`; both are EDGE-011 sunset candidates. Gates: deno check + 33/33 proxy tests + build + audit:routes (no new prod blockers).

Audit-logs proxy + edge platform-admin gate (EDGE-002i, DONE): `/api/audit-logs` is now in `crmProxies` → `supabase/functions/audit-logs/index.ts`. The page `/admin/audit-logs` (AuditLogViewer.tsx) is PLATFORM-ADMIN ONLY (`ProtectedRoute platformOnly` in App.tsx + `{platformOnly:true}` in navigation-permissions.ts), so the edge gate stays platform-admin (NOT relaxed to company admin — the route gate already blocks non-platform users from loading the page). THE LOAD-BEARING GOTCHA — **the canonical JWT `app_metadata` platform flag is `isPlatformUser` (+ a numeric `roleLevel`), NOT `isPlatformAdmin` and NOT a `role` string.** Authoritative minter: `server/middleware/supabase-auth.ts:213-224` (app_metadata = `{tenantId, roleId, roleLevel, teamId, accessScope, isPlatformUser}`); frontend `useSupabaseAuth.ts` transformUser + `usePermissions().isPlatformUser` read the same; the correct edge reference is `supabase/functions/me/index.ts` (`app_metadata.isPlatformUser`). Several edge fns (audit-logs's original check, `feature-flags`, `reports/handlers/executive.ts`, the `blog-*` fns) gate on the NON-canonical `app_metadata.isPlatformAdmin === true || role === 'platform_admin'` — those keys are never minted, so the gate is **effectively always-false** and 403s every legitimate platform admin once proxied (a latent bug class worth a future sweep). Fixed audit-logs to `isPlatformUser === true || roleLevel >= 8` (matches the frontend `platformOnly` definition), legacy keys kept as harmless fallbacks. The edge fn reads JWT app_metadata only (no DB role lookup) — contrast the EDGE-004 platform fns (root-admin/platform-*) which query `users→roles(level, can_access_all_tenants)` and gate `roleLevel >= 7 || can_access_all_tenants`; the server baseline `utils/auth-helpers.ts isPlatformAdmin` is far more permissive (DB-augmented req.user: isPlatformUser flag, roleLevel >= 8, hasAllPermissions, exact role codes incl company_admin) but isn't reproducible from the JWT alone. SECOND FIX: the frontend `AuditLogResponse` reads FLAT `total`/`page`/`limit`/`totalPages`, but both the edge fn and the Express baseline nested them under `pagination` (edge even used `pages` not `totalPages`) — so the page's pagination was permanently stuck at "Page X of 1 (0 total)". The fn now returns the flat shape (a `pagination` block kept for back-compat). Proxy is SAFE per the EDGE-002h rule: the single caller (AuditLogViewer.tsx:118) uses `apiRequest`/getApiUrl, not raw fetch, so dev now matches prod (prod hits functions.printyx.net/audit-logs directly; 404'd before). Gates: deno check + 33/33 proxy tests + build + `npm run check` (no new errors; edge fn not in tsc include) + audit:routes→`proxied`.
...
Automation dashboards edge fns (EDGE-002g, DONE): `/api/auto-lead-routing`, `/api/auto-supply-replenishment`, `/api/contract-renewal`, `/api/predictive-dispatch` are now in `crmProxies`, each with its dashboard-page read endpoints ported on top of the pre-existing rules/CRUD. TWO load-bearing drift lessons. (1) INTEGER-vs-UUID `tenant_id`: `supply_monitoring`/`auto_supply_orders`/`supply_replenishment_analytics` (`shared/auto-supply-replenishment-schema.ts`) and `contract_renewal_tracking`/`renewal_proposals` (`shared/contract-renewal-schema.ts`) ALL declare `tenant_id` as `integer('tenant_id')`, but real tenant IDs are UUID varchars — so `.eq('tenant_id', <uuid>)` errors at the DB layer (22P02). EVERY read against those tables MUST be degrade-tolerant (check supabase-js `{error}` → return shape-compatible empty/zeros + `degraded:true`), so the dashboard renders empty states instead of 500-ing. By contrast `auto-lead-routing`'s dashboard uses the CORRECT per-tenant tables (`lead_assignment_history`/`lead_score_calculations`/`rep_capacity` in `shared/lead-assignment-schema.ts`+`shared/lead-scoring-schema.ts` — all varchar tenant_id) and returns REAL data. (2) The corresponding Express routes were already broken-baseline: `routes-auto-lead-routing.ts` imported the `platform_*` CRM tables but referenced lead-assignment column names (phantom on the platform tables); `routes-predictive-service-dispatch.ts` references `serviceCallsEnhanced`/`technicianResourcesEnhanced`/`equipmentMetrics` which are UNDEFINED identifiers (tables don't exist in any `shared/*` schema) — so predictive-dispatch dashboard endpoints are degraded (only `devicesMonitored`, from the real `equipment` table, is live). All four migrated off `pathParts[1]` onto `normalizePath` (else the new sub-routes 404 under Coolify's prefix-strip). `contract-renewal` already used normalizePath + had `upcoming/stats/renew/mark-churned` over the `contracts` table with ZERO frontend callers (kept); ALL contract-renewal callers are `ContractRenewalDashboard`, so `/expiring` was repointed from the `{contracts,count,urgency}` shape to the dashboard's bare-array over `contract_renewal_tracking`. Heavy AI paths (`/analyze`, `/analyze-all`, `/route/:leadId`) are shape-compatible degraded stubs (Claude calls are Node-only). Gates: `deno check` (all 4) + build + 33/33 proxy tests + `npm run check` (edge fns not in tsc include) + `audit:routes`→`proxied`.

16-domain surface-gap port (EDGE-002h, DONE): ported the missing sub-routes into 16 edge fns and migrated the Coolify-broken `pathParts[1]` dispatchers onto `normalizePath` (commission, gdpr, manufacturer-integrations, platform-deals, rbac, remote-monitoring, social-media, warehouse-operations were ALL still pathParts[1] = every pre-existing endpoint 404'd in prod). THE LOAD-BEARING LESSON — **the crmProxies gate is HOW THE FRONTEND CALLS the domain, not just edge coverage.** Six domains (equipment, manufacturer-integrations, oid-mappings, platform-deals, rbac, social-media) call their edge routes via RAW relative `fetch('/api/<d>/…')`, which in prod hits the SPA origin = **Express, not the edge fn** (same class as the calculator/oid-mappings note below). Proxying those would make dev (edge) DIVERGE from prod (Express) — a regression — so they were ported but NOT proxied (a frontend rewrap onto `getApiUrl`/`apiRequest` is the real follow-up; the port still helps prod for any non-raw callers + readies them). Detection: `grep -rnoE "fetch\(\s*[\`'\"]/api/<d>" client/src` — **zero raw-fetch hits ⇒ calls go through getApiUrl ⇒ edge in prod ⇒ safe to proxy.** The 10 safe ones added to crmProxies: commission, gdpr, performance, parts-orders, products, purchase-orders, quickbooks, remote-monitoring, supplies, warehouse-operations. SECOND LESSON: a bare `'/api/<d>'` in `queryClient.invalidateQueries({queryKey:[...]})` or the `queryOptimizations.ts` invalidation GROUPS is a CACHE KEY, not a fetch — it does NOT imply the frontend GETs the root, so the edge fn needs no root handler for it (products root stays 405; nothing fetches it). THIRD: the audit's Express detection only matches `app.METHOD('/api/x')` literals, so router-relative mounts (oid-mappings, social-media) show as `edge-only` not `both-divergent` — heuristic blind spot, not a blocker. Pre-existing prod bugs left OUT OF SCOPE (not "missing endpoints"): supplies list/POST reference phantom cols (sku/note/quantity) on the real product_code/product_name table (base GET renders; search/writes fail in prod already; the new `/import` uses correct cols); rbac bare `GET /roles` returns the legacy `roles` table as a bare array vs RoleManagement's expected `{roles,pagination}` from `enhanced_roles`. Gates: `deno check` (16/16) + 33/33 proxy tests + build + `audit:routes` (both-divergent 51→42, proxied 55→65).

Leads edge fn (EDGE-002f, DONE): `/api/leads` is now in `crmProxies` → `supabase/functions/leads/index.ts`, which gained full frontend parity beyond the original list/`:id`/convert/qualify: GET+POST `/:id/contacts` (the `lead_contacts` table — LeadDetail.tsx attach-contact flow; POST requires firstName+lastName, maps `isPrimary`→`is_primary`), GET `/map-data` (ports `server/routes-lead-map.ts` — camelCase `{leads,stats}`, parses brand/equipment/UCC meta out of the `notes` blob via regex, post-query filters source/city/brand/status/hasCoords), POST `/geocode` (delegates to `../geocode-leads/index.ts` handler — cross-fn import is the accepted idiom, avoids functions-URL plumbing), and POST `/import-eda` (multipart `req.formData()` + a hand-rolled RFC-4180 CSV parser since Deno has no `csv-parser`; groups by `BUYCOMP1`, dedups vs existing `source='EDA Import'`, inserts `business_records`). A `RESERVED` set (`map-data`/`geocode`/`import-eda`) + `isReserved` guards stop those first segments being treated as a lead `:id`. THREE load-bearing lessons: (1) SCHEMA DRIFT — `businessRecords.leadSource` maps to the DB column literally named **`source`** (`shared/schema.ts:1328` `leadSource: varchar('source')`), NOT `lead_source`; also `billingPostalCode`→`billing_zip_code`, `assignedSalesRep`→`assigned_sales_rep`. Raw supabase-js queries MUST use the `varchar('...')` string, not the Drizzle property name. (2) supabase-js typed `.select()` with a runtime-CONCATENATED column string infers `GenericStringError` and every `r.<col>` fails `deno check` — cast the rows to `any[]`. (3) the EDA multipart upload had to move off raw cookie `fetch` onto a Bearer-authenticated `getApiUrl()` upload — `apiRequest` forces `application/json` so it can't send `FormData`, so the upload is hand-wrapped (`getAccessToken()` + `x-tenant-id` from localStorage); a cookie `fetch` 401s through the JWT-gated edge fn (same lesson as subscriptions/billing). EDGE-002l (multipart proxy fix) already landed, so the dev proxy forwards multipart correctly. Two lead identifiers coexist: the existing list filters `status='lead'` but map-data/import use `record_type='lead'` (EDA leads are `status='new'` + `record_type='lead'`, so they appear in /map-data but not the list — pre-existing, left unchanged). The cross-fn import also surfaced + fixed geocode-leads's pre-existing `getUser(jwt)` overload (null→undefined). The old Express `routes-lead-map.ts` + `routes-crm-core.ts` `/api/leads/:id/contacts` are left for the EDGE-008/011 sunset (zero edge-routed callers now). Gates: `deno check` (both fns) + build + 33/33 proxy tests + `npm run check` (no new errors) + `audit:routes`→`proxied`.
