      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 gap-3">
          {PAIN_POINTS.map((point) => (
            <div key={point.value} className="flex items-center space-x-2 border rounded-lg p-3 hover:bg-gray-50">
              <Checkbox
                id={point.value}
                checked={(data.painPoints || []).includes(point.value)}
                onCheckedChange={(checked) => handlePainPointToggle(point.value, checked as boolean)}
              />
              <label
                htmlFor={point.value}
