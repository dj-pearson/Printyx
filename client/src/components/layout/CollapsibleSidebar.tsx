          ...
          <CollapsibleTrigger asChild>
            ...
            >
              <div className="flex items-center gap-3">
                <section.icon
                  className={cn('h-5 w-5', shouldShowAsActive ? 'text-white' : 'text-slate-600')}
                />
                <span className="font-semibold">{section.title}</span>
              </div>
              {isExpanded(section.id) ? (
                <ChevronDown
                  className={cn('h-4 w-4', shouldShowAsActive ? 'text-white' : 'text-slate-500')}
                />
              ) : (
                <ChevronRight
                  className={cn('h-4 w-4', shouldShowAsActive ? 'text-white' : 'text-slate-500')}
      ...
      return (
        <Link key={section.id} href={section.path}>
          ...
          >
            <div className="flex items-center gap-3">
              <section.icon
                className={cn('h-5 w-5', isCurrentlyActive ? 'text-white' : 'text-slate-600')}
              />
              <span className="font-semibold">{section.title}</span>
            </div>
            {isCurrentlyActive && (
