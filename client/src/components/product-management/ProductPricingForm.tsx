...
export function ProductPricingForm({
  ...
  const handleDealerCostChange = (value: string) => {
    ...
    if (numValue > 0) {
      const markup = values.repMarkupPercentage
        ? parseFloat(values.repMarkupPercentage.toString())
        : undefined;
    ...
    <div className="space-y-4 p-4 border rounded-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {visibility?.canEditDealerCost && (
          <div className="space-y-2">
            <Label htmlFor={`${tier}-markup`}>Rep Markup (%)</Label>
            <div className="flex items-center gap-2">
              <Input
                ...
                disabled={disabled}
              />
              {calculating && <Calculator className="h-4 w-4 animate-spin text-muted-foreground" />}
            </div>
            <p className="text-xs text-muted-foreground">Leave blank to use company default</p>
          </div>
        )}
        ...
        {visibility?.showRepCost && (
          <div className="space-y-2">
            <Label htmlFor={`${tier}-rep-cost`}>Rep Cost ($)</Label>
            <Input
              id={`${tier}-rep-cost`}
              type="text"
              value={
                values.repCost ? `$${parseFloat(values.repCost.toString()).toFixed(2)}` : '$0.00'
              }
              disabled
              className="bg-muted"
            />
        ...
        <div className="space-y-2">
          <Label htmlFor={`${tier}-retail`}>Suggested Retail ($)</Label>
          <Input
            id={`${tier}-retail`}
            type="number"
            step="0.01"
          ...
          />
          <p className="text-xs text-muted-foreground">MSRP / Suggested customer price</p>
        </div>
      </div>
        ...
        <>
          ...
          <div className="flex items-center justify-between p-3 bg-muted/50 rounded-md">
            ...
            <div className="flex items-center gap-4">
              <div className="text-right">
                ...
                <p
                  className={`text-sm font-bold ${isMarginLow() ? 'text-orange-600' : 'text-green-600'}`}
                >
                  {calculateMargin()}%
                </p>
              </div>
          ...
          {isMarginLow() && (
            <Alert variant="destructive">
              ...
              <AlertDescription>
                Warning: Margin is below minimum ({visibility?.minMarginPercentage || 5}%). This
                price may require manager approval.
              </AlertDescription>
            </Alert>
          )}
        </>
