...
export default function LocationPipelineGrid({ pipelines, aggregated, summary }: any) {
  const formatCurrency = (value: number) => value >= 1000 ? `$${(value / 1000).toFixed(0)}K` : `$${value.toFixed(0)}`;
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {Array.from(locationMap.entries()).map(([locationId, stageMap]) => {
          ...
          const totalValue = Array.from(stageMap.values()).reduce((sum, p) => sum + p.totalValue, 0);
          return (
          ...
          <div className="grid grid-cols-5 gap-4">
            {aggregated.map((agg: any) => (
              <div key={agg.stage} className="text-center p-3 bg-muted rounded-lg">
                ...
                <div className="text-xs text-muted-foreground">{formatCurrency(agg.totalValue)}</div>
              </div>
            ))}
          </div>
        </div>
