/**
 * WAREHOUSE TEAM STATS WIDGET
 * In-module stat widget that displays warehouse team performance metrics
 * Shows FPY (First Pass Yield), accuracy rate, productivity, and quality scores
...
interface WarehouseTeamStatsWidgetProps {
  variant?: 'compact' | 'full';
  showAutoRefresh?: boolean;
  className?: string;
}

export default function WarehouseTeamStatsWidget({
  variant = 'full',
  ...
  const {
    ...
    error,
    refetch,
  } = useQuery<WarehouseTeamQuickStats>({
    queryKey: ['warehouse-team-quick-stats'],
    queryFn: async () => {
      const res = await fetch('/api/warehouse-reports/team/quick-stats', {
        credentials: 'include',
      });
      if (!res.ok) {
        if (res.status === 403) {
          throw new Error('You do not have permission to view warehouse team stats');
        }
        throw new Error('Failed to fetch warehouse team stats');
      }
      return res.json();
    },
      ...
      <Card className={className}>
        <CardHeader>
          <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
            ...
            Warehouse Team Stats
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="text-sm text-muted-foreground">
            {error instanceof Error ? error.message : 'Unable to load warehouse team stats'}
          </div>
        </CardContent>
      </Card>
    );
    ...
    <div className={cn('space-y-4', className)}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-semibold">Warehouse Team Performance</h3>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              Based on {stats?.activity.completedKits || 0} completed kits
            </p>
          </CardContent>
        </Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <p className="text-2xl font-bold">{stats?.activity.totalKits || 0}</p>
            </div>
            <div>
