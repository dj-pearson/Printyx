...
/*
 * generateOrganizationSchema and generateWebSiteSchema lived here. Both emitted
 * page-independent entities that client/index.html already ships in the static
 * head with stable @ids, so every Organization-schema route carried two
 * Organization nodes and two WebSite nodes. The WebSite one also offered a
 * sitelinks SearchAction at /search, which is not a registered route - the same
 * dead action SEO-002 removed from the static copy.
 */
...
function generateLocalBusinessSchema(config: SEORouteConfig): Record<string, unknown> {
  return {
    ...
    // No telephone (there is no published number) and no priceRange - '$$' was
    // a made-up band for a product whose pricing page states real figures.
...
function generateSchema(config: SEORouteConfig): Record<string, unknown>[] {
  ...
  switch (config.schema) {
    case 'Organization':
      // Nothing to add. Organization and WebSite are page-independent and
      // already ship in the static <head> of client/index.html with stable
      // @ids. Injecting them again gave every Organization-schema route two
      // Organization nodes and two WebSite nodes, which is a consumer's
      // problem to reconcile rather than a signal.
      break;
    ...
    case 'HowTo':
      ...
      break;
    case 'LocalBusiness':
      // A LocalBusiness with no address is not a local business. No route sets
      // `localBusiness` today, so this emits nothing rather than an entity that
      // asserts a physical presence Printyx has not stated.
      if (config.localBusiness?.address) {
...
function applySEO(config: SEORouteConfig, dynamicData?: DynamicSEOData): () => void {
  ...
  // No article:modified_time. It was `new Date()` evaluated on every render,
  // so every page claimed it had been updated on the day it was viewed - a
  // freshness signal generated by the act of looking at it, for content that
  // had not changed. A real one needs a real timestamp on the content
  // (AUDIT-020: a fabricated measurement that moves is harder to spot than one
  // that sits still, because refreshing appears to confirm it).
