...
export default function AccountsPayable() {
  ...
  const createAPMutation = useMutation({
    mutationFn: (data: AccountsPayableFormData) =>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>{editingAP ? 'Edit Bill' : 'Add New Bill'}</DialogTitle>
                <DialogDescription>
                  {editingAP
            ...
            <Card key={ap.id} className="hover:shadow-md transition-shadow">
              <CardHeader>
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center space-x-1">{getStatusIcon(ap.status)}</div>
                </div>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium">
                        ${parseFloat(ap.totalAmount.toString()).toFixed(2)}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-600">Balance:</span>
                      <span className="font-medium">
                        ${parseFloat(ap.balanceAmount.toString()).toFixed(2)}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                ...
                <div className="flex justify-end space-x-2 mt-4">
                  <Button size="sm" variant="outline" onClick={() => setViewingAP(ap)}>
                    <Eye className="h-3 w-3 mr-1" />
                    View
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => handleOpenDialog(ap)}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="mt-2 space-y-1 text-sm">
                      <div>
                        <span className="text-gray-600">Bill Number:</span> {viewingAP.billNumber}
                      </div>
                      <div>
                        <span className="text-gray-600">Vendor:</span>{' '}
                        {getVendorName(viewingAP.vendorId)}
                      </div>
                      <div>
                        <span className="text-gray-600">Bill Date:</span>{' '}
                        {format(new Date(viewingAP.billDate), 'MMM d, yyyy')}
                      </div>
                      <div>
                        <span className="text-gray-600">Due Date:</span>{' '}
                        {format(new Date(viewingAP.dueDate), 'MMM d, yyyy')}
                      </div>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="mt-2 space-y-1 text-sm">
                      <div>
                        ...
                        {parseFloat(viewingAP.subtotal.toString()).toFixed(2)}
                      </div>
                      <div>
                        <span className="text-gray-600">Tax:</span> $
                        {parseFloat(viewingAP.taxAmount?.toString() || '0').toFixed(2)}
                      </div>
                      <div>
                        <span className="text-gray-600">Total:</span> $
                        {parseFloat(viewingAP.totalAmount.toString()).toFixed(2)}
                      </div>
                      <div>
                        <span className="text-gray-600">Paid:</span> $
                        {parseFloat(viewingAP.paidAmount?.toString() || '0').toFixed(2)}
                      </div>
                      <div>
                        <span className="text-gray-600">Balance:</span> $
                        {parseFloat(viewingAP.balanceAmount.toString()).toFixed(2)}
                      </div>
                    </div>
                  </div>
                </div>
