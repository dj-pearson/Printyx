...
export default function AccountsReceivable() {
  ...
  const createARMutation = useMutation({
    mutationFn: (data: AccountsReceivableFormData) =>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle>{editingAR ? 'Edit Invoice' : 'Create New Invoice'}</DialogTitle>
                <DialogDescription>
                  {editingAR
            ...
            <Card key={ar.id} className="hover:shadow-md transition-shadow">
              <CardHeader>
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center space-x-1">{getStatusIcon(ar.status)}</div>
                </div>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium">
                        ${parseFloat(ar.totalAmount.toString()).toFixed(2)}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-600">Balance:</span>
                      <span className="font-medium">
                        ${parseFloat(ar.balanceAmount.toString()).toFixed(2)}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                ...
                <div className="flex justify-end space-x-2 mt-4">
                  <Button size="sm" variant="outline" onClick={() => setViewingAR(ar)}>
                    <Eye className="h-3 w-3 mr-1" />
                    View
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => handleOpenDialog(ar)}>
                  ...
                  <div>
                    ...
                    <div className="mt-2 space-y-1 text-sm">
                      <div>
                        ...
                        {viewingAR.invoiceNumber}
                      </div>
                      <div>
                        <span className="text-gray-600">Customer:</span>{' '}
                        {getCustomerName(viewingAR.customerId)}
                      </div>
                      <div>
                        <span className="text-gray-600">Invoice Date:</span>{' '}
                        {format(new Date(viewingAR.invoiceDate), 'MMM d, yyyy')}
                      </div>
                      <div>
                        <span className="text-gray-600">Due Date:</span>{' '}
                        {format(new Date(viewingAR.dueDate), 'MMM d, yyyy')}
                      </div>
                      <div>
                        <span className="text-gray-600">Payment Terms:</span>{' '}
                        {viewingAR.paymentTerms}
                      </div>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="mt-2 space-y-1 text-sm">
                      <div>
                        ...
                        {parseFloat(viewingAR.subtotal.toString()).toFixed(2)}
                      </div>
                      <div>
                        <span className="text-gray-600">Tax:</span> $
                        {parseFloat(viewingAR.taxAmount?.toString() || '0').toFixed(2)}
                      </div>
                      <div>
                        <span className="text-gray-600">Total:</span> $
                        {parseFloat(viewingAR.totalAmount.toString()).toFixed(2)}
                      </div>
                      <div>
                        <span className="text-gray-600">Paid:</span> $
                        {parseFloat(viewingAR.paidAmount?.toString() || '0').toFixed(2)}
                      </div>
                      <div>
                        <span className="text-gray-600">Balance:</span> $
                        {parseFloat(viewingAR.balanceAmount.toString()).toFixed(2)}
                      </div>
                    </div>
                  </div>
                </div>
