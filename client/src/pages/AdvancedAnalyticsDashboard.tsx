...
import { BarChart3, TrendingUp, TrendingDown, Users, DollarSign, Target, Calendar, Download, Filter, RefreshCw, PieChart, LineChart, Activity, AlertTriangle, CheckCircle, Clock, Zap } from 'lucide-react';
...
interface AnalyticsData {
  executiveSummary: {
    totalRevenue: { current: number; previous: number; growth: number; trend: string };
    activeCustomers: { current: number; previous: number; growth: number; trend: string };
    serviceTickets: { current: number; previous: number; growth: number; trend: string };
    grossMargin: { current: number; previous: number; growth: number; trend: string };
  };
  revenueAnalytics: {
    monthlyRevenue: Array<{ month: string; revenue: number; contracts: number; newCustomers: number }>;
    revenueByCategory: Array<{ category: string; amount: number; percentage: number; growth: number }>;
    topPerformingProducts: Array<{ product: string; revenue: number; units: number; margin: number; trend: string }>;
  };
  customerAnalytics: {
    customerSegmentation: Array<{ segment: string; count: number; revenue: number; percentage: number }>;
    customerLifetimeValue: { average: number; median: number; top10Percent: number; churnRate: number; retentionRate: number };
    topCustomers: Array<{ name: string; revenue: number; contracts: number; satisfaction: number; lastPurchase: Date; nextRenewal: Date }>;
  };
  serviceAnalytics: {
    serviceMetrics: { totalTickets: number; avgResolutionTime: number; firstCallResolution: number; customerSatisfaction: number; technicianUtilization: number };
    ticketTrends: Array<{ month: string; tickets: number; resolved: number; satisfaction: number }>;
    topIssues: Array<{ issue: string; count: number; avgTime: number; resolution: number }>;
    technicianPerformance: Array<{ technician: string; tickets: number; avgTime: number; satisfaction: number; efficiency: number }>;
  };
  equipmentAnalytics: {
    fleetOverview: { totalUnits: number; averageAge: number; utilizationRate: number; maintenanceCompliance: number };
    equipmentByManufacturer: Array<{ manufacturer: string; units: number; percentage: number; avgAge: number }>;
    maintenanceSchedule: { overdue: number; dueSoon: number; upcoming: number; compliant: number };
  };
  financialAnalytics: {
    profitability: { grossProfit: number; grossMargin: number; netProfit: number; netMargin: number; ebitda: number };
  ...
  predictiveAnalytics: {
    ...
    churnPrediction: { highRisk: number; mediumRisk: number; lowRisk: number; actions: Array<{ customer: string; risk: number; action: string }> };
  };
  competitiveAnalysis: {
    marketShare: { company: number; competitor1: number; competitor2: number; competitor3: number; others: number };
    winLossAnalysis: { totalOpportunities: number; won: number; lost: number; pending: number; winRate: number; lossReasons: Array<{ reason: string; count: number; percentage: number }> };
  };
}
  ...
  const { data: analyticsData, isLoading, refetch } = useQuery({
    ...
    select: (data: any) => ({
      ...
      customerAnalytics: {
        ...
        topCustomers: data.customerAnalytics?.topCustomers?.map((customer: any) => ({
          ...customer,
              ...
              <CardContent className="p-6">
                ...
                <div className="flex items-center mt-2">
                  ...
                  <span className={`ml-2 text-sm font-medium ${
                    analyticsData.executiveSummary.totalRevenue.growth >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
            ...
            <Card>
              <CardContent className="p-6">
                ...
                <div className="flex items-center mt-2">
                  ...
                  <span className={`ml-2 text-sm font-medium ${
                    analyticsData.executiveSummary.activeCustomers.growth >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
            ...
            <Card>
              <CardContent className="p-6">
                ...
                <div className="flex items-center mt-2">
                  ...
                  <span className={`ml-2 text-sm font-medium ${
                    analyticsData.executiveSummary.serviceTickets.growth >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
            ...
            <Card>
              <CardContent className="p-6">
                ...
                <div className="flex items-center mt-2">
                  ...
                  <span className={`ml-2 text-sm font-medium ${
                    analyticsData.executiveSummary.grossMargin.growth >= 0 ? 'text-green-600' : 'text-red-600'
                  }`}>
          ...
          <Tabs defaultValue="revenue" className="space-y-6">
            ...
            <TabsContent value="revenue" className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {analyticsData.revenueAnalytics.revenueByCategory.map((category: any, idx: number) => (
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {analyticsData.revenueAnalytics.topPerformingProducts.map((product: any, idx: number) => (
                  ...
                  <div className="h-64 flex items-center justify-center bg-gray-50 rounded">
                    <div className="text-center">
                      ...
                      <p className="text-sm text-gray-500">
                        Showing trend from {analyticsData.revenueAnalytics.monthlyRevenue[0]?.month} to{' '}
                        {analyticsData.revenueAnalytics.monthlyRevenue[analyticsData.revenueAnalytics.monthlyRevenue.length - 1]?.month}
                      </p>
                    </div>
                  </div>
                </CardContent>
            ...
            <TabsContent value="customers" className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {analyticsData.customerAnalytics.customerSegmentation.map((segment: any, idx: number) => (
                        <div key={idx} className="space-y-2">
                          <div className="flex justify-between items-center">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="text-center p-3 bg-gray-50 rounded">
                        <div className="text-lg font-bold text-gray-900">
                          {formatCurrency(analyticsData.customerAnalytics.customerLifetimeValue.average)}
                        </div>
                        <div className="text-sm text-gray-600">Average CLV</div>
                      </div>
                      <div className="text-center p-3 bg-gray-50 rounded">
                        <div className="text-lg font-bold text-gray-900">
                          {formatCurrency(analyticsData.customerAnalytics.customerLifetimeValue.median)}
                        </div>
                        <div className="text-sm text-gray-600">Median CLV</div>
                      </div>
                      <div className="text-center p-3 bg-green-50 rounded">
                        <div className="text-lg font-bold text-green-900">
                          {formatPercentage(analyticsData.customerAnalytics.customerLifetimeValue.retentionRate)}
                        </div>
                        <div className="text-sm text-green-600">Retention Rate</div>
                      </div>
                      <div className="text-center p-3 bg-red-50 rounded">
                        <div className="text-lg font-bold text-red-900">
                          {formatPercentage(analyticsData.customerAnalytics.customerLifetimeValue.churnRate)}
                        </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {analyticsData.customerAnalytics.topCustomers.map((customer: any, idx: number) => (
                      <div key={idx} className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="flex-1">
                          <div className="font-medium">{customer.name}</div>
                          <div className="text-sm text-gray-600">
                            {customer.contracts} contracts • Last purchase: {format(customer.lastPurchase, 'MMM dd, yyyy')}
                          </div>
            ...
            <TabsContent value="service" className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="text-center p-3 bg-purple-50 rounded">
                        <div className="text-lg font-bold text-purple-900">
                          {formatPercentage(analyticsData.serviceAnalytics.serviceMetrics.firstCallResolution)}
                        </div>
                        <div className="text-sm text-purple-600">First Call Resolution</div>
                      </div>
                      <div className="text-center p-3 bg-yellow-50 rounded">
                        <div className="text-lg font-bold text-yellow-900">
                          {analyticsData.serviceAnalytics.serviceMetrics.customerSatisfaction.toFixed(1)}/5.0
                        </div>
                      ...
                      {analyticsData.serviceAnalytics.topIssues.map((issue: any, idx: number) => (
                        <div key={idx} className="flex items-center justify-between p-3 border rounded">
                          <div className="flex-1">
                            ...
                            <div className="text-xs text-gray-600">
                              Avg time: {issue.avgTime}h • Resolution: {formatPercentage(issue.resolution)}
                            </div>
                          </div>
                          <div className="text-right">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {analyticsData.serviceAnalytics.technicianPerformance.map((tech: any, idx: number) => (
                      <div key={idx} className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4">
                          <div className="text-center">
                            <div className="text-sm font-bold">{tech.satisfaction.toFixed(1)}/5.0</div>
                            <div className="text-xs text-gray-500">Rating</div>
                          </div>
                          <div className="text-center">
                            <div className="text-sm font-bold">{formatPercentage(tech.efficiency)}</div>
            ...
            <TabsContent value="equipment" className="space-y-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="text-center p-3 bg-gray-50 rounded">
                        <div className="text-lg font-bold text-gray-900">
                          {analyticsData.equipmentAnalytics.fleetOverview.averageAge.toFixed(1)} years
                        </div>
                        <div className="text-sm text-gray-600">Average Age</div>
                      </div>
                      <div className="text-center p-3 bg-green-50 rounded">
                        <div className="text-lg font-bold text-green-900">
                          {formatPercentage(analyticsData.equipmentAnalytics.fleetOverview.utilizationRate)}
                        </div>
                        <div className="text-sm text-green-600">Utilization</div>
                      </div>
                      <div className="text-center p-3 bg-purple-50 rounded">
                        <div className="text-lg font-bold text-purple-900">
                          {formatPercentage(analyticsData.equipmentAnalytics.fleetOverview.maintenanceCompliance)}
                        </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {analyticsData.equipmentAnalytics.equipmentByManufacturer.map((manufacturer: any, idx: number) => (
                      <div key={idx} className="space-y-2">
                        <div className="flex justify-between items-center">
                          ...
                          <div className="text-right">
                            ...
                            <div className="text-xs text-gray-600">Avg age: {manufacturer.avgAge.toFixed(1)} years</div>
                          </div>
                        </div>
            ...
            <TabsContent value="financial" className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div className="flex justify-between items-center p-3 bg-gray-50 rounded">
                        ...
                        <span className="font-bold">{formatCurrency(analyticsData.financialAnalytics.profitability.grossProfit)}</span>
                      </div>
                      <div className="flex justify-between items-center p-3 bg-gray-50 rounded">
                        <span className="font-medium">Gross Margin</span>
                        <span className="font-bold">{formatPercentage(analyticsData.financialAnalytics.profitability.grossMargin)}</span>
                      </div>
                      <div className="flex justify-between items-center p-3 bg-green-50 rounded">
                        <span className="font-medium">Net Profit</span>
                        <span className="font-bold text-green-900">{formatCurrency(analyticsData.financialAnalytics.profitability.netProfit)}</span>
                      </div>
                      <div className="flex justify-between items-center p-3 bg-green-50 rounded">
                        <span className="font-medium">Net Margin</span>
                        <span className="font-bold text-green-900">{formatPercentage(analyticsData.financialAnalytics.profitability.netMargin)}</span>
                      </div>
                      <div className="flex justify-between items-center p-3 bg-blue-50 rounded">
                        <span className="font-medium">EBITDA</span>
                        <span className="font-bold text-blue-900">{formatCurrency(analyticsData.financialAnalytics.profitability.ebitda)}</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {analyticsData.financialAnalytics.expenseBreakdown.map((expense: any, idx: number) => (
                        <div key={idx} className="flex items-center justify-between p-3 border rounded">
                          <div className="flex-1">
            ...
            <TabsContent value="predictive" className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {analyticsData.predictiveAnalytics.revenueForecast.map((forecast: any, idx: number) => (
                        <div key={idx} className="flex items-center justify-between p-3 border rounded">
                          <div className="flex-1">
                          ...
                          <div className="text-right">
                            <div className="font-bold text-sm">{formatCurrency(forecast.predicted)}</div>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        {analyticsData.predictiveAnalytics.churnPrediction.actions.map((action: any, idx: number) => (
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div>
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-bold">{formatPercentage(analyticsData.competitiveAnalysis.marketShare.company)}</span>
                        </div>
                        <Progress value={analyticsData.competitiveAnalysis.marketShare.company} className="h-2" />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="text-center p-3 bg-blue-50 rounded mb-3">
                        <div className="text-lg font-bold text-blue-900">
                          {formatPercentage(analyticsData.competitiveAnalysis.winLossAnalysis.winRate)}
                        </div>
                        <div className="text-sm text-blue-600">Win Rate</div>
                      </div>
                      <div className="text-sm text-gray-600">
                        {analyticsData.competitiveAnalysis.winLossAnalysis.won} won • {analyticsData.competitiveAnalysis.winLossAnalysis.lost} lost • {analyticsData.competitiveAnalysis.winLossAnalysis.pending} pending
                      </div>
                    </div>
                  </div>
                </CardContent>
