...
export default function AdvancedBillingEngine() {
  ...
  const [swipeAction, setSwipeAction] = useState<{ invoiceId: string; action: string } | null>(
    null,
  );
  ...
  return (
    <MainLayout
      ...
      description="Comprehensive billing management with automated cycles and complex pricing scenarios"
    >
      <div className="container mx-auto p-6 space-y-6">
        ...
        {typeof window !== 'undefined' &&
          (new URLSearchParams(window.location.search).get('ticketId') ||
            ...
            <div className="p-3 bg-blue-50 border border-blue-200 rounded text-sm text-blue-800 flex items-center justify-between">
              {new URLSearchParams(window.location.search).get('ticketId') && (
                <span>
                  Filtering for Service Ticket ID:{' '}
                  {new URLSearchParams(window.location.search).get('ticketId')}
                </span>
              )}
              {new URLSearchParams(window.location.search).get('contractId') && (
                <span className="ml-2">
                  Filtering for Contract ID:{' '}
                  {new URLSearchParams(window.location.search).get('contractId')}
                </span>
              )}
            ...
            <Dialog open={isConfigDialogOpen} onOpenChange={setIsConfigDialogOpen}>
              ...
              <DialogContent className="max-w-3xl">
                ...
                <Form {...configurationForm}>
                  <form
                    onSubmit={configurationForm.handleSubmit(onConfigurationSubmit)}
                    className="space-y-4"
                  >
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid w-full grid-cols-6">
            ...
            <TabsTrigger value="ai-insights" className="hidden sm:inline-flex">
              AI Insights
            </TabsTrigger>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {analytics?.collectionRate
                      ? `${(analytics.collectionRate * 100).toFixed(1)}%`
                      : '0%'}
                  </div>
                  <p className="text-xs text-muted-foreground">
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                      ...
                      <p className="text-sm text-blue-700 mb-3">
                        Based on market analysis and customer behavior, we've identified
                        opportunities to optimize your pricing strategy.
                      </p>
                      <div className="space-y-2">
                    ...
                    <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                      ...
                      <p className="text-sm text-green-700 mb-3">
                        AI analysis suggests contract term adjustments that could increase customer
                        lifetime value.
                      </p>
                      ...
                      <Card key={config.id}>
                        ...
                        <CardContent className="space-y-2">
                          <div className="text-sm">
                            <p>
                              <strong>Type:</strong> {config.billing_type.replace('_', ' ')}
                            </p>
                            <p>
                              <strong>Frequency:</strong> {config.billing_frequency}
                            </p>
                            <p>
                              <strong>Base Rate:</strong> ${(config.base_rate || 0).toFixed(2)}
                            </p>
                            <p>
                              <strong>Min Charge:</strong> $
                              {(config.minimum_charge || 0).toFixed(2)}
                            </p>
                            {config.maximum_charge && (
                              <p>
                                <strong>Max Charge:</strong> ${config.maximum_charge.toFixed(2)}
                              </p>
                            )}
                            <p>
                              <strong>Tax Rate:</strong> {((config.tax_rate || 0) * 100).toFixed(2)}
                              %
                            </p>
                            <p>
                      ...
                      <div key={cycle.id} className="border rounded-lg p-4">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <div className="text-sm text-muted-foreground space-y-1">
                              <p>
                                Cycle Date: {format(new Date(cycle.cycle_date), 'MMM dd, yyyy')}
                              </p>
                              <p>
                                Customers: {cycle.processed_customers}/{cycle.total_customers}
                              </p>
