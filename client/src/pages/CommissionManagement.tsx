...
export default function CommissionManagement() {
  ...
  const { data: plans = [], isLoading: plansLoading } = useQuery({
    ...
    select: (data: any[]) =>
      data.map((plan) => ({
        ...
        // CORRECTED by CR-017: this used to say Express served /plans in dev
        // from routes-commission.ts. That file is deleted - its plans handler
        // returned one hardcoded "Sales Rep Standard" plan - and
        // /api/commission is proxied now, so supabase/functions/commission/
        // serves both environments. Every row there goes through toCamel(), so
        // camelCase is authoritative and there is no snake_case to fall back to.
  ...
  const { data: calculations = [] } = useQuery<CommissionCalculation[]>({
    ...
    // CR-017: one shape now. The two backends used to disagree about the whole
    // response - Express returned an array of calculations while the edge
    // function returned an object of deal-derived rows at an invented 5% rate -
    // so this had to guard against `data` not being an array at all. Both
    // Express routers are deleted, /api/commission is proxied, and the edge
    // function reads commission_calculations, the table POST /calculate says it
    // would write to. The engine that fills it is not built (that endpoint
    // answers 501), so this list is honestly empty rather than plausibly wrong.
    select: (data: CommissionCalculation[]) =>
      data.map((calc) => ({
        ...calc,
        calculationPeriod: {
        ...
        summary: {
          ...calc.summary,
          payoutDate: calc.summary.payoutDate ? new Date(calc.summary.payoutDate) : undefined,
        },
  ...
  const calculateMutation = useMutation({
    mutationFn: (data: any) =>
      apiRequest('/api/commission/calculate', {
        method: 'POST',
  ...
  const totalCommissionThisMonth = calculations.reduce(
    (sum, calc) => sum + calc.summary.netCommission,
    0,
  );
  const averageCommissionPayout =
    calculations.length > 0 ? totalCommissionThisMonth / calculations.length : 0;
            ...
            <Dialog open={isCreatePlanOpen} onOpenChange={setIsCreatePlanOpen}>
              ...
              <DialogContent>
                ...
                <div className="text-sm text-gray-600">
                  Commission plan creation form would be implemented here with tier configuration,
                  rate settings, and rule definitions.
                </div>
              </DialogContent>
            </Dialog>
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  ${analytics.summary.totalCommissionPaid.toLocaleString()}
                </div>
                <p className="text-xs text-muted-foreground">This quarter total payout</p>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">Active commission earners</p>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {analytics.performance_metrics.quotaAchievementRate}%
                </div>
                <p className="text-xs text-muted-foreground">Overall quota achievement</p>
                <Progress
                  value={analytics.performance_metrics.quotaAchievementRate}
                  className="mt-2"
                />
              </CardContent>
            </Card>
              ...
              <Card>
                <CardContent className="text-center py-12">
                  ...
                  <p className="text-gray-600 mb-4">
                    Run your first commission calculation for the current period.
                  </p>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 text-xs sm:text-sm text-gray-600 mb-3">
                            ...
                            <div>
                              ...
                              <br />${calc.salesMetrics.totalSales?.toLocaleString() || 'N/A'}
                            </div>
                            <div>
                        ...
                        <div className="text-right">
                          ...
                          <div className="mt-2 text-xs">
                            ...
                            {calc.summary.totalAdjustments !== 0 && (
                              <div>
                                Adjustments: ${calc.summary.totalAdjustments.toLocaleString()}
                              </div>
                            )}
                          </div>

                          {calc.summary.payoutDate && (
                      ...
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 text-xs sm:text-sm text-gray-600 mb-3">
                            ...
                            <div>
                              ...
                              <br />${dispute.disputeDetails.difference.toLocaleString()}
                            </div>
                          </div>
                          ...
                          {dispute.resolution && (
                            <div className="bg-blue-50 rounded-lg p-3">
                              ...
                              <div className="text-sm">
                                <div>
                                  <strong>Assigned to:</strong> {dispute.resolution.assignedToName}
                                </div>
                                {dispute.resolution.estimatedResolution && (
                                  <div>
                                    <strong>Estimated resolution:</strong>{' '}
                                    {format(
                                      new Date(dispute.resolution.estimatedResolution),
                                      'MMM dd, yyyy',
                                    )}
                                  </div>
                                )}
                        ...
                        <div className="text-right">
                          ...
                          <div className="mt-1 text-xs">
                            <div>
                              Expected: ${dispute.disputeDetails.expectedAmount.toLocaleString()}
                            </div>
                            <div>
                              Difference: ${dispute.disputeDetails.difference.toLocaleString()}
                            </div>
                          </div>
                        </div>
                      </div>
              ...
              <>
                          ...
                          >
                            ...
                            <div className="text-right">
                              <div className="font-medium">
                                ${performer.totalCommission.toLocaleString()}
                              </div>
                              <div className="text-sm text-gray-600">
                                {performer.quotaAchievement}% quota
                              </div>
                            </div>
                          </div>
                        ))}
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Commission plan effectiveness and participation
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <ResponsiveContainer width="100%" height={400}>
                      <BarChart data={analytics.plan_performance}>
                        ...
                        <Bar
                          dataKey="avgQuotaAchievement"
                          fill="#ffc658"
                          name="Avg Quota Achievement (%)"
                        />
                      </BarChart>
                    </ResponsiveContainer>
                  </CardContent>
