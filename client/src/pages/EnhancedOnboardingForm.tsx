...
const enhancedOnboardingSchema = z.object({
  ...
  printManagement: z.object({
    system: z.enum(['papercut', 'equitrac', 'ysoft', 'other', 'none']).default('none'),
...
export default function EnhancedOnboardingForm() {
  ...
  const [selectedBusinessRecord, setSelectedBusinessRecord] = useState<any>(null);
  ...
  const { data: businessRecords = [] } = useQuery({
    ...
    queryFn: () =>
      apiRequest(
        `/api/business-records?search=${encodeURIComponent(businessRecordSearch)}&limit=10`,
      ),
  ...
  const { data: quotes = [] } = useQuery({
    ...
    queryFn: () => {
      ...
      if (selectedBusinessRecord?.id) params.append('businessRecordId', selectedBusinessRecord.id);
  ...
  const { data: companyContacts = [] } = useQuery({
    ...
    queryFn: () => apiRequest(`/api/companies/${selectedBusinessRecord.id}/contacts`),
    enabled: !!selectedBusinessRecord?.id,
  });
  ...
  useEffect(() => {
    if (selectedBusinessRecord) {
      ...
      form.setValue('customerData.address', selectedBusinessRecord.address || '123 Main Street');
      ...
      const fullAddress = [
        ...
        .join(', ');
      form.setValue(
        'siteInformation.installationAddress',
      ...
      form.setValue(
        'siteInformation.phoneNumber',
        selectedBusinessRecord.phone || primaryContact?.phone || '555-0123',
      );
  ...
  const createChecklistMutation = useMutation({
    mutationFn: (data: any) => apiRequest('/api/onboarding/checklists', 'POST', data),
  ...
  const onSubmit = (data: EnhancedOnboardingFormData) => {
    ...
    if (
      !data.customerData.primaryContact ||
      data.customerData.primaryContact.trim() === '' ||
      data.customerData.primaryContact === 'undefined undefined' ||
      data.customerData.primaryContact.includes('undefined')
    ) {
      ...
      if (selectedBusinessRecord?.firstName && selectedBusinessRecord?.lastName) {
        ...
        !selectedBusinessRecord.primaryContactName.includes('undefined')
      ) {
    ...
    if (
      !data.siteInformation.contactPerson ||
      data.siteInformation.contactPerson.trim() === '' ||
      data.siteInformation.contactPerson === 'undefined undefined' ||
      data.siteInformation.contactPerson.includes('undefined')
    ) {
    ...
    const payload = {
      ...
      installationType: installationType as
        | 'new_installation'
        | 'replacement'
        | 'relocation'
        | 'upgrade',
  ...
  const importEquipmentFromQuote = () => {
    ...
    const getEquipmentType = (
      description: string,
    ): 'printer' | 'copier' | 'scanner' | 'fax' | 'mfp' => {
      const desc = description.toLowerCase();
      if (
        desc.includes('mfp') ||
        desc.includes('multifunction') ||
      ...
      .filter((item: any) => {
        ...
        return (
          ...
          (desc.includes('printer') ||
            ...
            desc.includes('advance') ||
            item.description?.match(/^[A-Za-z]+ [A-Za-z0-9-]+ [A-Za-z0-9-]+/))
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-6">
              <h3 className="text-lg font-semibold mb-2">Customer & Quote Selection</h3>
              <p className="text-sm text-gray-600">
                Search and select a customer to auto-populate information, then optionally import
                equipment from a quote.
              </p>
            </div>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Search for an existing customer to auto-populate their information
                </CardDescription>
              </CardHeader>
                            ...
                            >
                              <div className="flex flex-col">
                                ...
                                <span className="text-sm text-gray-500">
                                  ...
                                  {record.phone || record.primaryContactPhone || 'N/A'}
                                </span>
                              </div>
                            </CommandItem>
                          ))}
                ...
                {selectedBusinessRecord && (
                  <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-medium text-green-800">Selected Customer</span>
                    </div>
                    <div className="text-sm">
                      ...
                      <div>
                        ...
                        •{' '}
                        {selectedBusinessRecord.email || selectedBusinessRecord.primaryContactEmail}
                      </div>
                    </div>
                  </div>
                )}
            ...
            {selectedBusinessRecord && (
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg mb-6">
                ...
                <p className="text-sm text-green-700">
                  The following information has been automatically filled from the customer record.
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="outline" onClick={addEquipmentItem}>
                  Add Manual Line
                </Button>
                <Button onClick={importEquipmentFromQuote} variant="default">
                  Import From Quote
                </Button>
              </div>
            </div>
                ...
                <CardContent className="space-y-4">
                  ...
                  {selectedQuote && (
                    <div className="p-3 bg-green-50 border border-green-200 rounded">
                      ...
                      <p className="text-sm text-green-700">
                        {selectedQuote.quoteNumber || `Quote #${selectedQuote.id.slice(-6)}`} -{' '}
                        {selectedQuote.title || 'Untitled'}
                      </p>
                    </div>
                  )}
                  ...
                  {catalogStep === 2 && (
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-3 md:grid-cols-4 gap-3">
                        {selectedProductType === 'Product Models' &&
                          [
                            'Canon',
                            'HP',
                            'Brother',
                        ...
                        {selectedProductType === 'Product Accessories' &&
                          [
                            'Canon',
                            'HP',
                            'Brother',
                        ...
                        {selectedProductType === 'Professional Services' &&
                          [
                            'Installation',
                            'Training',
                            'Consulting',
                        ...
                        {selectedProductType === 'Service Products' &&
                          ['Maintenance', 'Support', 'Repair', 'Warranty', 'Extended Coverage'].map(
                            (brand) => (
                              <Button
                                key={brand}
                                variant="outline"
                        ...
                        {selectedProductType === 'IT & Managed Services' &&
                          [
                            'Microsoft',
                            'Adobe',
                            'Citrix',
                          ...
                          {catalogProducts
                            .filter(
                              (p: any) =>
                                p.category
                                  ?.toLowerCase()
                                  .includes(selectedProductType.toLowerCase().slice(0, -1)) &&
                            ...
                            .map((p: any) => (
                              ...
                              >
                                <div className="flex flex-col">
                                  ...
                                  {p.price && (
                                    <span className="text-sm text-blue-600">${p.price}</span>
                                  )}
                                </div>
                                <Button
                                  size="sm"
                                  onClick={() => {
                                    const newItem = {
                                      ...
                                      networkConfiguration: {
                                        customerNumber:
                                          selectedBusinessRecord?.customer_number || '',
                                      },
                                    };
                                    ...
                                    toast({
                                      title: 'Added',
                                      description: `${p.modelName || p.name} added to equipment`,
                                    });
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="border-t pt-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <FormField
                        control={form.control}
                        name={`equipment.${index}.replacedEquipment.oldIPAddress` as const}
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Old IP</FormLabel>
                            <FormControl>
                              <Input placeholder="e.g., 10.209.20.82" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        control={form.control}
                        name={`equipment.${index}.replacedEquipment.oldHostname` as const}
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Old Hostname</FormLabel>
                            <FormControl>
                              <Input placeholder="e.g., VHS-PEOFFICE" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        control={form.control}
                        name={`equipment.${index}.replacedEquipment.oldSerialNumber` as const}
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Old Serial #</FormLabel>
                            <FormControl>
                              <Input placeholder="Enter old serial" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <FormField
                        control={form.control}
                        name={`equipment.${index}.replacedEquipment.oldMake` as const}
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Old Make</FormLabel>
                            <FormControl>
                              <Input placeholder="HP, Lexmark..." {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        control={form.control}
                        name={`equipment.${index}.replacedEquipment.oldModel` as const}
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Old Model</FormLabel>
                            <FormControl>
                              <Input placeholder="LaserJet P401..." {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        control={form.control}
                        name={`equipment.${index}.replacedEquipment.oldAssetTag` as const}
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Old Asset Tag</FormLabel>
                            <FormControl>
                              <Input placeholder="Old asset tag" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                  ...
                  <div className="border-t pt-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FormField
                        control={form.control}
                        name={`equipment.${index}.networkConfiguration.targetIPAddress` as const}
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Target IP Address</FormLabel>
                            <FormControl>
                              <Input placeholder="e.g., 10.36.20.52" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        control={form.control}
                        name={`equipment.${index}.networkConfiguration.newHostname` as const}
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>New Hostname</FormLabel>
                            <FormControl>
                              <Input placeholder="e.g., CR12POD-LASER" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        control={form.control}
                        name={`equipment.${index}.networkConfiguration.locationAddress` as const}
                        render={({ field }) => (
                          <FormItem>
                      ...
                      <FormField
                        control={form.control}
                        name={`equipment.${index}.networkConfiguration.buildingLocation` as const}
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Building</FormLabel>
                            <FormControl>
                              <Input placeholder="e.g., Valley High School" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        control={form.control}
                        name={`equipment.${index}.networkConfiguration.roomLocation` as const}
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Room</FormLabel>
                            <FormControl>
                              <Input placeholder="e.g., 1st floor TW Science" {...field} />
                            </FormControl>
                            <FormMessage />
                          </FormItem>
                        )}
                      ...
                      <FormField
                        control={form.control}
                        name={`equipment.${index}.networkConfiguration.locationContact` as const}
                        render={({ field }) => (
                          <FormItem>
          ...
          <div className="space-y-6">
            <div className="bg-green-50 border border-green-200 rounded-lg p-6">
              ...
              <p className="text-green-700 mb-4">
                Review all information before creating your checklist. You can export this as PDF or
                Excel after creation.
              </p>
              ...
              <div className="bg-white rounded-lg p-4 mb-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    {form.watch('customerData.companyName') || 'Not specified'}
                  </div>
                  <div>
                    <strong>Contact:</strong>{' '}
                    {form.watch('customerData.primaryContact') || 'Not specified'}
                  </div>
                  <div>
                    <strong>Phone:</strong> {form.watch('customerData.phone') || 'Not specified'}
                  </div>
                  <div>
                    <strong>Email:</strong> {form.watch('customerData.email') || 'Not specified'}
                  </div>
                  <div>
                    <strong>Address:</strong>{' '}
                    {form.watch('customerData.address') || 'Not specified'}
                  </div>
                  <div>
                    <strong>Industry:</strong>{' '}
                    {form.watch('customerData.industry') || 'Not specified'}
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg p-4 mb-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    {form.watch('siteInformation.installationAddress') || 'Not specified'}
                  </div>
                  <div>
                    <strong>Contact Person:</strong>{' '}
                    {form.watch('siteInformation.contactPerson') || 'Not specified'}
                  </div>
                  <div>
                    <strong>Phone:</strong>{' '}
                    {form.watch('siteInformation.phoneNumber') || 'Not specified'}
                  </div>
                  <div>
                    <strong>Building Type:</strong>{' '}
                    {form.watch('siteInformation.buildingType') || 'office'}
                  </div>
                  <div>
                    ...
                    {equipmentItems.map((item, index) => (
                      <div key={index} className="border border-gray-200 rounded-lg p-3">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm">
                          <div>
                            <strong>Type:</strong> {item.equipmentType}
                          </div>
                          <div>
                            <strong>Manufacturer:</strong> {item.manufacturer || 'Not specified'}
                          </div>
                          <div>
                            <strong>Model:</strong> {item.model || 'Not specified'}
                          </div>
                          {item.serialNumber && (
                            <div>
                          ...
                          {item.isReplacement && (
                            <div className="md:col-span-3 text-orange-600">
                              <strong>Replacement for:</strong> {item.replacedEquipment?.oldMake}{' '}
                              {item.replacedEquipment?.oldModel}
                            </div>
                          )}
                        </div>
                      </div>
              ...
              {selectedQuote && (
                <div className="bg-white rounded-lg p-4 mb-4">
                  ...
                  <div className="text-sm">
                    <div>
                      <strong>Quote Number:</strong> {selectedQuote.quoteNumber}
                    </div>
                    <div>
                      <strong>Title:</strong> {selectedQuote.title}
                    </div>
                    <div>
                      <strong>Total Amount:</strong> $
                      {parseFloat(selectedQuote.totalAmount).toLocaleString()}
                    </div>
                    <div>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Checklist Title *</FormLabel>
                  <FormControl>
                    <Input placeholder="Enter a descriptive title for this checklist" {...field} />
                  </FormControl>
                  <FormDescription>
  ...
  return (
    <MainLayout
      title="Enhanced Onboarding Checklist"
      description="Create a comprehensive equipment installation and customer onboarding checklist with auto-population and machine replacement tracking"
    >
          ...
          <Dialog open={showExportModal} onOpenChange={setShowExportModal}>
            <DialogContent className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  Your onboarding checklist has been created. You can now export it in multiple
                  formats or continue managing your checklists.
                </DialogDescription>
              </DialogHeader>
