...
export default function FinancialForecasting() {
  ...
  const { data: cashFlowProjections = [], isLoading: cashFlowLoading } = useQuery<
    CashFlowProjection[]
  >({
  ...
  const { data: profitabilityAnalysis = [], isLoading: profitabilityLoading } = useQuery<
      ...
      <div className="container mx-auto p-6 space-y-6">
          ...
          <div className="flex space-x-2">
            ...
            <Dialog open={isCashFlowDialogOpen} onOpenChange={setIsCashFlowDialogOpen}>
              ...
              <DialogContent className="max-w-2xl">
                ...
                <Form {...cashFlowForm}>
                  <form
                    onSubmit={cashFlowForm.handleSubmit(onCashFlowSubmit)}
                    className="space-y-4"
                  >
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
                      <FormField
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid w-full grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-1 sm:gap-0">
            ...
            <TabsTrigger value="forecasts" className="text-xs sm:text-sm px-2 py-2">
              Forecasts
            </TabsTrigger>
            <TabsTrigger value="cashflow" className="text-xs sm:text-sm px-2 py-2">
              Cash Flow
            </TabsTrigger>
            <TabsTrigger value="profitability" className="text-xs sm:text-sm px-2 py-2">
              Profit
            </TabsTrigger>
            <TabsTrigger value="kpis" className="text-xs sm:text-sm px-2 py-2">
              KPIs
            </TabsTrigger>
          </TabsList>
                      ...
                      {(kpis as FinancialKPI[]).slice(0, 5).map((kpi) => (
                        ...
                        >
                          ...
                          <div className="text-right">
                            <p
                              className={`text-sm font-medium ${getPerformanceColor(kpi.performance_vs_target)}`}
                            >
                              {kpi.current_value.toLocaleString()}
                            </p>
                            <p className="text-xs text-muted-foreground">
                              {kpi.percentage_change > 0 ? '+' : ''}
                              {Number(kpi.percentage_change || 0).toFixed(1)}%
                            </p>
                          </div>
                        </div>
                      ))}
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center space-x-2 mb-2">
                              ...
                              <Badge variant="outline">{forecast.scenario_type}</Badge>
                            </div>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <div className="text-sm text-muted-foreground space-y-1">
                              <p>
                                Period:{' '}
                                {format(new Date(projection.projection_period), 'MMM dd, yyyy')}
                              </p>
                      ...
                      <Card key={kpi.id}>
                        ...
                        <CardContent className="space-y-2">
                          <div className="text-center">
                            <p
                              className={`text-2xl font-bold ${getPerformanceColor(kpi.performance_vs_target)}`}
                            >
                              {kpi.current_value.toLocaleString()}
                            </p>
                          ...
                          <div className="grid grid-cols-2 gap-2 text-xs">
                            ...
                            <div>
                              ...
                              <p
                                className={`font-medium ${kpi.percentage_change >= 0 ? 'text-green-600' : 'text-red-600'}`}
                              >
                                {kpi.percentage_change > 0 ? '+' : ''}
                                {Number(kpi.percentage_change || 0).toFixed(1)}%
                              </p>
                            ...
                            <div>
                              ...
                              <p
                                className={`font-medium ${getPerformanceColor(kpi.performance_vs_target)}`}
                              >
