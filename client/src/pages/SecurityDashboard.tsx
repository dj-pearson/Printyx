...
export default function SecurityDashboard() {
  ...
  const failedLoginsQuery = useQuery<{ data: FailedLogin[] }>({
    queryKey: ['/api/security/failed-logins'],
    queryFn: () => apiRequest('/api/security/failed-logins?limit=20'),
    refetchInterval: 30000,
  });

  const eventsQuery = useQuery<{ data: SecurityEvent[] }>({
    queryKey: ['/api/security/events'],
    queryFn: () => apiRequest('/api/security/events?limit=20'),
    refetchInterval: 30000,
  });

  // CR-033: all three kept only `.data`, and this page polls every 30s. A
  // failed poll rendered zero failed logins, an empty event feed and an empty
  // threat table — on a SECURITY dashboard that is not a blank screen, it is an
  // all-clear. Held whole so a broken feed is visibly broken.
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* CR-033: the title block above is static; everything below is polled. */}
      <QueryStates
        queries={[overviewQuery, failedLoginsQuery, eventsQuery]}
        loading={<DashboardSkeleton />}
        errorTitle="Could not load security monitoring"
        className="py-6"
      >
        {/* KPI Cards */}
