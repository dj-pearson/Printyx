...
export default function ServiceForecastingAnalytics() {
  ...
  const [forecastType, setForecastType] = useState<'all' | 'toner' | 'maintenance' | 'repair'>(
    'all',
  );
  ...
  const { data: forecasts = [], isLoading: forecastsLoading } = useQuery<ServiceForecast[]>({
    ...
    queryFn: () =>
      apiRequest(
        `/api/reports/service-forecasts?horizon=${timeHorizon}&territory=${selectedTerritory}&type=${forecastType}`,
      ),
  });
  ...
  const { data: techCapacity = [] } = useQuery<TechnicianCapacity[]>({
    ...
    queryFn: () =>
      apiRequest(
        `/api/reports/technician-capacity?horizon=${timeHorizon}&territory=${selectedTerritory}`,
      ),
  });
  ...
  const { data: summaryMetrics } = useQuery({
    ...
    queryFn: () =>
      apiRequest(
        `/api/reports/service-summary?horizon=${timeHorizon}&territory=${selectedTerritory}`,
      ),
  });
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <Card className="lg:col-span-2">
                <CardHeader>
                  ...
                  <CardDescription>
                    AI-predicted service requirements with confidence scores
                  </CardDescription>
                </CardHeader>
                <CardContent>
                        ...
                        <div className="flex items-center space-x-3">
                          <div
                            className={`p-2 rounded-lg ${
                              forecast.forecastType === 'toner'
                                ? 'bg-blue-100'
                                : forecast.forecastType === 'maintenance'
                                  ? 'bg-green-100'
                                  : forecast.forecastType === 'repair'
                                    ? 'bg-red-100'
                                    : 'bg-purple-100'
                            }`}
                          >
                    ...
                    <PieChart>
                      <Pie
                        data={[
                          {
                            name: 'Toner',
                            value: forecasts.filter((f) => f.forecastType === 'toner').length,
                          },
                          {
                            name: 'Maintenance',
                            value: forecasts.filter((f) => f.forecastType === 'maintenance').length,
                          },
                          {
                            name: 'Repair',
                            value: forecasts.filter((f) => f.forecastType === 'repair').length,
                          },
                          {
                            name: 'Renewals',
                            value: forecasts.filter((f) => f.forecastType === 'contract_renewal')
                              .length,
                          },
                        ]}
                        cx="50%"
                        ...
                        <div className="flex-1">
                          ...
                          <div className="space-y-2">
                            <div className="flex items-center justify-between text-sm">
                              ...
                              <span
                                className={`font-medium ${getHealthColor(customer.healthScore)}`}
                              >
                            ...
                            <div className="text-xs text-muted-foreground">
                              Last service:{' '}
                              {format(new Date(customer.lastServiceDate), 'MMM dd, yyyy')}
                            </div>
                          </div>
                        </div>
                      </div>
                        ...
                        <div key={index} className="p-4 border rounded-lg">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <Badge variant="outline">
                              {customer.upcomingRenewals.length} contract
                              {customer.upcomingRenewals.length !== 1 ? 's' : ''}
                            </Badge>
                          </div>
                              ...
                              <div className="flex justify-between">
                                ...
                                <span
                                  className={`font-medium ${
                                    renewal.renewalProbability >= 80
                                      ? 'text-green-600'
                                      : renewal.renewalProbability >= 60
                                        ? 'text-yellow-600'
                                        : 'text-red-600'
                                  }`}
                                >
          ...
          <TabsContent value="inventory" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Predicted inventory needs and reorder recommendations
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground space-y-1">
                            <p>
                              Current Stock: {item.currentStock} | Predicted Demand:{' '}
