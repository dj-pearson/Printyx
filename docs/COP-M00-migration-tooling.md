...
**Diagnosed 2026-08-11. Fixed 2026-08-26** — see "Resolved 2026-08-26" at the end for what shipped
and what is still open. Everything above that heading is the original diagnosis, kept because the
reasoning is what makes the fix reviewable; in particular "The trap" is still the wrong answer.
...
## Resolved 2026-08-26: what shipped

`db:generate` emits your delta and nothing else. The two defects are fixed, and the fix was run
against a real Postgres rather than reasoned about.

### The reconciliation migration

`drizzle/migrations/0057_cop_m00_schema_reconciliation.sql`, generated by the sanctioned
`db:generate`, never hand-authored. It carries the whole drift: **118 `CREATE TABLE`, 299
`CREATE INDEX`, 20 `CREATE UNIQUE INDEX`, 15 `ADD COLUMN`, 9 FK constraints, 2 enum types, 1 enum
value, 1 `SET DEFAULT`**: 445 statements. Nothing is dropped or narrowed; there is not a single
`DROP` or `ALTER COLUMN … TYPE` in the file.

Every statement is idempotent, so it is a no-op wherever the object already exists:
`CREATE TABLE IF NOT EXISTS`, `CREATE INDEX IF NOT EXISTS`, `ADD COLUMN IF NOT EXISTS`,
`ADD VALUE IF NOT EXISTS`, and a `DO $$ … EXCEPTION WHEN duplicate_object THEN null; END $$;`
wrapper for the FK constraints and enum types, which have no `IF NOT EXISTS` form.

This is the opposite of "The trap" above: the 107 unmigrated tables now have a migration, so a
fresh environment gets them instead of silently missing them.

It was renumbered from the `0040_*` drizzle-kit picked to `0057_*` (the next free prefix) and its
journal `tag` updated to match. The file numbering stays non-contiguous on purpose (see the
SUPA-002 note above).

### The numbering wrapper

`npm run db:generate` is now `node scripts/db-generate.mjs`, which runs `drizzle-kit generate` and
then renumbers whatever it wrote to `(highest prefix on disk + 1)`, rewriting the journal tag.
drizzle-kit names files from the journal LENGTH, which will collide forever otherwise. Renaming is
safe because the migrator resolves files by `${entry.tag}.sql` and tracks applied state by sha256
of file CONTENT, and the rename happens before the migration has ever been applied anywhere.

### The guard

`scripts/check-migration-journal.mjs` check `(e)` is now split. A **hard gate**: the LAST journal
entry must have its snapshot, because that is the only snapshot `generate` diffs against. A
**ratchet at 25**: the mid-chain gaps (idx 10–13, 19–39) affect nothing drizzle-kit does and cannot
be reconstructed from SQL, so they stay warned-about rather than fatal. Verified by moving
`meta/0040_snapshot.json` aside: the guard fails with the head-snapshot message.

### Verification

Against a scratch PostgreSQL 16 cluster, not a static read:

| Check                                                     | Result                                                                                                                    |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| Reproduce the symptom (before)                            | 2,306 lines / 445 statements, written as `0040_repro_probe.sql`, colliding with `0040_auto_orders.sql`                    |
| No-op `db:generate` (after)                               | `No schema changes, nothing to migrate`                                                                                   |
| One nullable column added                                 | `ALTER TABLE "crm_notes" ADD COLUMN "cop_m00_probe" varchar;` — one line, and the wrapper renumbered it `0041_* → 0058_*` |
| `0057` applied to a DB built from the other 40 migrations | 0 errors                                                                                                                  |
| `0057` applied a second time                              | 0 errors (idempotent)                                                                                                     |
| `npm run check:migrations`                                | passes                                                                                                                    |

The probe column and its migration were reverted; only the reconciliation migration remains.

### Still open: this migration has not been applied anywhere

`0057` is committed but unapplied. Applying it needs the runbook above, which needs database
access this work did not have:

1. `npm run db:backup` first.
2. `npm run db:migrate:status`, expecting `0036` through `0056` plus `0057` pending.
3. `npm run db:migrate`.
4. On an environment that already holds the 107 tables, `0057` is a no-op by construction, so
   either path (apply or `db:migrate:baseline`) is safe. Prefer applying it, so the ledger records
   real execution.

### A separate blocker this uncovered: `db:migrate` cannot build a fresh database

Running the full chain against an empty PostgreSQL 16 database fails in **migration `0000`**, long
before reaching anything from this story:

```
ERROR: foreign key constraint "api_key_rotations_tenant_id_tenants_id_fk" cannot be implemented
DETAIL: Key columns "tenant_id" and "id" are of incompatible types: uuid and character varying.
```

`db:migrate` wraps the run in one transaction, so it rolls back and creates nothing at all.
Applying statement-by-statement (`psql -v ON_ERROR_STOP=0`) gets 667 tables and **30 errors**, all
of them in `0000` (15 FKs across `api_keys`/`sso_*`/`customer_maintenance_appointments` with
uuid-vs-varchar key columns), `0001` (4), `0002` (10), and `0043` (1, `o.template_type` does not
exist). **`0057` contributes zero.**

That is PA-032 ("make a fresh database provisionable from versioned migrations alone"), now
measured rather than inferred, and it is out of scope here.
