...
| Meeting intelligence | `MeetingTranscription.tsx`, `MeetingToProposalDashboard.tsx` | Maps directly onto HubSpot's 2026 "Smart Sales Meetings" — but filed under *Productivity*, not Sales. |
...
that `opportunities` is a *deprecated Salesforce-mirror table*. Reps, admins, and the schema are
...
Transcription — a core selling tool — sits under *Productivity*.
...
**The pattern worth stealing:** HubSpot's 2026 direction is not more features — it is *one workspace
that tells the rep what to do next, and AI that removes CRM data entry*. Printyx should copy the
...
2. **Quote Builder is catalog-only.** `LineItemManager.tsx` maps "equipment" to `product_models` —
   what we *sell*. It has no awareness of `equipment` — what they *have*. A rep quoting a fleet
...
Tagged `COP-###`. Effort is S (≤3d) / M (≤2w) / L (>2w).
...
| COP-B04 | **🔑 Installed-Base Opportunity Radar** — the copier wedge. A scheduled scan over `equipment`, `contracts`, `leases`, `meter_readings`, and service history that produces dated, quantified, ranked plays: *lease expiring in 120 days*, *contract ending*, *volume up 40% vs. contracted tier*, *machine past service-cost threshold*, *color capability underused*, *device off-network*. Each play one-clicks into a deal pre-loaded with the fleet, the current spend, and the recommended replacement. **HubSpot structurally cannot build this.** | L |
...
COP-E02, COP-M03, COP-M07, COP-B02, COP-M04, COP-M05.
