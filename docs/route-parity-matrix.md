...
**Architecture:** prod frontend → `functions.printyx.net/<route>` (edge) for ALL `/api/*` via `getApiUrl`. The Express proxy map (45 prefixes) only affects **dev**. Express stays canonical for `/api/client-metrics/*` (agent) and `/install/*`.

**Totals:** 299 domains · 231 edge fns · 159 Express-served · 183 frontend-called · 45 proxied

## Summary by class

| Class | Count | Meaning |
|---|---|---|
| `missing-edge` | 34 | Frontend calls it, NO edge function → **PROD BLOCKER** |
...
| `proxied` | 45 | In proxy map → dev forwards to edge (aligned) |
...
**missing-edge** (34): `accessories`, `ai`, `ai-analytics`, `approvals`, `assignment-groups`, `business-process`, `content`, `contract-tiered-rates`, `deals-management`, `deployment`, `documents`, `email-parser`, `equipment-disposal`, `erp-integration`, `financial-forecasting`, `incident-response`, `integration-hub`, `lead-intelligence`, `lease-payments`, `pipeline-forecast`, `platform`, `predictive-analytics`, `quote-templates`, `record-layout-config`, `sales-forecasts`, `sales-performance`, `salesforce`, `security-compliance`, `service`, `system-monitoring`, `task-workflows`, `technician-sessions`, `white-label`, `workflow-automation`
