...
**Architecture:** prod frontend → `functions.printyx.net/<route>` (edge) for ALL `/api/*` via `getApiUrl`. The Express proxy map (42 prefixes) only affects **dev**. Express stays canonical for `/api/client-metrics/*` (agent) and `/install/*`.

**Totals:** 338 domains · 248 edge fns · 182 Express-served · 200 frontend-called · 42 proxied

## Summary by class

| Class | Count | Meaning |
|---|---|---|
| `missing-edge` | 51 | Frontend calls it, NO edge function → **PROD BLOCKER** |
| `both-divergent` | 61 | Express + edge both exist, not proxied → dev ≠ prod (maintenance trap) |
...
| `proxied` | 42 | In proxy map → dev forwards to edge (aligned) |
