...
**Architecture:** prod frontend → `functions.printyx.net/<route>` (edge) for ALL `/api/*` via `getApiUrl`. The Express proxy map (37 prefixes) only affects **dev**. Express stays canonical for `/api/client-metrics/*` (agent) and `/install/*`.

**Totals:** 306 domains · 228 edge fns · 168 Express-served · 188 frontend-called · 37 proxied
...
| `proxied` | 37 | In proxy map → dev forwards to edge (aligned) |
| `edge-only` | 110 | Edge function only, no Express handler → fully migrated |
