...
**Architecture:** prod frontend → `functions.printyx.net/<route>` (edge) for ALL `/api/*` via `getApiUrl`. The Express proxy map (81 prefixes) only affects **dev**. Express stays canonical for `/api/client-metrics/*` (agent) and `/install/*`.

**Totals:** 376 domains · 284 edge fns · 240 Express-served · 187 frontend-called · 81 proxied

## Summary by class

| Class               | Count | Meaning                                                                |
| ------------------- | ----- | ---------------------------------------------------------------------- |
| `missing-edge`      | 4     | Frontend calls it, NO edge function → **PROD BLOCKER**                 |
| `both-divergent`    | 68    | Express + edge both exist, not proxied → dev ≠ prod (maintenance trap) |
