...
**Architecture:** prod frontend → `functions.printyx.net/<route>` (edge) for ALL `/api/*` via `getApiUrl`. The Express proxy map (66 prefixes) only affects **dev**. Express stays canonical for `/api/client-metrics/*` (agent) and `/install/*`.

**Totals:** 294 domains · 225 edge fns · 159 Express-served · 182 frontend-called · 66 proxied
...
| `both-divergent` | 41 | Express + edge both exist, not proxied → dev ≠ prod (maintenance trap) |
...
| `proxied` | 66 | In proxy map → dev forwards to edge (aligned) |
