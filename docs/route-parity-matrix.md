...
**Architecture:** prod frontend → `functions.printyx.net/<route>` (edge) for ALL `/api/*` via `getApiUrl`. The Express proxy map (43 prefixes) only affects **dev**. Express stays canonical for `/api/client-metrics/*` (agent) and `/install/*`.

**Totals:** 302 domains · 231 edge fns · 162 Express-served · 186 frontend-called · 43 proxied

## Summary by class

| Class | Count | Meaning |
|---|---|---|
| `missing-edge` | 38 | Frontend calls it, NO edge function → **PROD BLOCKER** |
| `both-divergent` | 60 | Express + edge both exist, not proxied → dev ≠ prod (maintenance trap) |
| `express-only` | 24 | Express handler, no edge function → not yet migrated |
| `dead-express` | 19 | Express + edge exist, no frontend ref → Express likely dead |
| `proxied` | 43 | In proxy map → dev forwards to edge (aligned) |
| `edge-only` | 108 | Edge function only, no Express handler → fully migrated |
| `express-canonical` | 10 | Express is canonical by design (agent ingest) |

## Action-needed first

**missing-edge** (38): `accessories`, `ai`, `ai-analytics`, `approvals`, `assignment-groups`, `business-process`, `content`, `contract-tiered-rates`, `deals-management`, `deployment`, `documents`, `email-parser`, `equipment-disposal`, `erp-integration`, `financial-forecasting`, `incident-response`, `integration-hub`, `lead-intelligence`, `lease-payments`, `phone-tickets`, `pipeline-forecast`, `platform`, `predictive-analytics`, `quote-templates`, `record-layout-config`, `sales-forecasts`, `sales-performance`, `salesforce`, `security-compliance`, `service`, `signature-analytics`, `signature-requests`, `signature-templates`, `system-monitoring`, `task-workflows`, `technician-sessions`, `white-label`, `workflow-automation`

**both-divergent** (60): `admin`, `audit-logs`, `auto-lead-routing`, `auto-supply-replenishment`, `catalog`, `commission`, `content-gap-analysis`, `contract-renewal`, `contracts`, `crm`, `dashboard`, `dashboards`, `database-updater`, `deal-stages`, `demos`, `devices`, `document-management`, `enrichment`, `equipment`, `financial`, `fleet`, `gdpr`, `import`, `integrations`, `inventory`, `invoices`, `leads`, `maintenance`, `managed-services`, `manufacturer-integrations`, `meter-readings`, `mobile`, `mobile-field`, `onboarding`, `parts-orders`, `performance`, `phone-in-tickets`, `platform-deals`, `predictive-dispatch`, `pricing`, `product-accessories`, `product-models`, `professional-services`, `projects`, `purchase-orders`, `quickbooks`, `rbac`, `remote-monitoring`, `root-admin`, `seo`, `service-analysis`, `service-analytics`, `service-tickets`, `software-products`, `subscriptions`, `supplies`, `technician-management`, `templates`, `user`, `warehouse-operations`
