...
**Architecture:** prod frontend → `functions.printyx.net/<route>` (edge) for ALL `/api/*` via `getApiUrl`. The Express proxy map (51 prefixes) only affects **dev**. Express stays canonical for `/api/client-metrics/*` (agent) and `/install/*`.

**Totals:** 294 domains · 225 edge fns · 159 Express-served · 182 frontend-called · 51 proxied
...
| `both-divergent` | 55 | Express + edge both exist, not proxied → dev ≠ prod (maintenance trap) |
...
| `proxied` | 51 | In proxy map → dev forwards to edge (aligned) |
