...
**Architecture:** prod frontend → `functions.printyx.net/<route>` (edge) for ALL `/api/*` via `getApiUrl`. The Express proxy map (54 prefixes) only affects **dev**. Express stays canonical for `/api/client-metrics/*` (agent) and `/install/*`.

**Totals:** 383 domains · 259 edge fns · 244 Express-served · 202 frontend-called · 54 proxied

## Summary by class

| Class | Count | Meaning |
|---|---|---|
| `missing-edge` | 42 | Frontend calls it, NO edge function → **PROD BLOCKER** |
...
| `proxied` | 54 | In proxy map → dev forwards to edge (aligned) |
