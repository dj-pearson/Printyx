...
| technician-sessions   | ~~DEAD~~ **PORTED** (PROD-011, 2026-08-07) | This row was wrong: Express DID serve `/technician-sessions/:sessionId/workflow-steps` (`routes-enhanced-service.ts`) and the tables `technician_ticket_sessions` + `workflow_steps` DO exist. The page was broken because 3 of its 4 URLs existed on neither side. | ported to `supabase/functions/technician-sessions/`                 |
