...
export default function TicketsScreen() {
  ...
  const {
    ...
    queryKey: [
      '/api/service-tickets',
      `?search=${search}&status=${statusFilter === 'All' ? '' : statusFilter}&limit=50`,
    ],
  });
    ...
    <SafeAreaView style={styles.container} edges={[]}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          ...
          >
            <Text
              style={[styles.filterChipText, statusFilter === item && styles.filterChipTextActive]}
            >
              {item}
            </Text>
          </TouchableOpacity>
        ...
        ListEmptyComponent={
          !isLoading ? (
            <EmptyState
              icon="ticket-outline"
              title="No Tickets"
              description="Service tickets will appear here"
            />
          ) : null
        }
      />
