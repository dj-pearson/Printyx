 ...
 *   - GENERATED markdown, for the same reason one step over. A doc carrying a
 *     `GENERATED by <script>` marker is a machine-managed list that happens to
 *     be Markdown, and its unresolved paths are usually its POINT:
 *     edge-rbac-express-predecessors.md cites the deleted Express routers it
 *     recovered from git history, each annotated `(deleted)`. Baselining those
 *     would put three permanently-correct entries in a list whose value is that
 *     every entry is a prompt to re-read something - which is where a real one
 *     hides. Regenerate the doc instead; its own guard checks its content.
 ...
 */
/**
 * A doc that declares itself generated, by NAMING ITS GENERATOR, near the top.
 *
 * Both halves are load-bearing and the first was found by a test rather than by
 * reading: `/GENERATED by [\w./-]+/` matched the sentence "a file is GENERATED
 * by hand", which is the opposite claim. The marker has to name a script, and it
 * has to be the document's own header rather than a phrase buried in its prose -
 * an exclusion that a hand-written doc can fall into by accident silently stops
 * watching whatever that doc points at.
 */
export function isGenerated(source) {
  return /GENERATED by [\w./-]+\.(mjs|cjs|js|ts)\b/.test(source.slice(0, 600));
}
...
function guidanceDocs() {
  ...
  for (const p of pathsIn(readFileSync(claudeMd, 'utf8'))) {
    ...
    if (isGenerated(readFileSync(full, 'utf8'))) continue;
    docs.push(p);
  }
  return [...new Set(docs)];
}
