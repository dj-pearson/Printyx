...
import { PgTable, PgEnumColumn, getTableConfig } from 'drizzle-orm/pg-core';
  ...
  for (const value of Object.values(schema)) {
    ...
    out.push({
      ...
      columns: cfg.columns.map((c) => ({
        column: c.name,
        // `information_schema.columns.data_type` answers 'USER-DEFINED' for
        // every enum and never names the type, so an enum column can only be
        // compared as "is an enum". Collapsing here rather than in
        // normalizeType, because the enum's NAME is all getSQLType() returns
        // and nothing downstream could tell it from a domain or a table type.
        sqlType: is(c, PgEnumColumn) ? 'enum' : c.getSQLType(),
        notNull: c.notNull,
      })),
    });
  }
...
function normalizeType(t: string): string {
  const base = t
    ...
    .trim();
  /**
   * An enum column and an array column cannot be compared by NAME, and the
   * first real run proved it: 331 of 332 TYPE_MISMATCH rows were enum-vs-enum
   * or array-vs-array, burying the one genuine finding.
   *
   * `information_schema.columns.data_type` answers 'USER-DEFINED' for every
   * enum (the live side maps that to 'enum') and 'ARRAY' for every array,
   * never the specific type - so drizzle's side, which names the enum
   * ('review_decision') or the element ('text array'), disagreed with the live
   * side on every single one. That is a comparison the query cannot win, not
   * drift, and a guard whose output is 99.7% false positives is one nobody
   * reads. Both sides collapse to the same word here; a WRONG enum or a wrong
   * element type is out of scope for this check and its header already says it
   * compares loosely on purpose.
   */
  if (base.endsWith(' array')) return 'array';
...
with expected(tbl, col, sqltype, is_required) as (values
${rows.join(',\n')}
),
live as (
  select table_name::text tbl, column_name::text col,
         case data_type
           ...
           -- data_type is literally 'ARRAY' for any array column and never
           -- names the element type, which is the other half of why the first
           -- real run produced 331 unwinnable comparisons.
           when 'ARRAY' then 'array'
           else data_type
         end::text sqltype,
         (is_nullable = 'NO') is_required
...
select 'MISSING_COLUMN', e.tbl, e.col,
       'expected ' || e.sqltype || case when e.is_required then ' NOT NULL' else '' end
  from expected e
...
select 'NULLABILITY', e.tbl, e.col, 'expected NOT NULL, live nullable'
  from expected e
  join live l on l.tbl = e.tbl and l.col = e.col
 where e.is_required and not l.is_required
union all
