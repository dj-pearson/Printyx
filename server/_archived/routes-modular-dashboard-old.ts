    ...
    revenue: async (tenantId: string, userId?: string) => {
      ...
      const [currentRevenue, previousRevenue, deals_data] = await Promise.all([
        db
          ...
          .where(
            and(
              ...
              ...(userId ? [eq(invoices.salesRepId, userId)] : []),
            ),
          ),
        ...
        db
          ...
          .from(deals)
          .where(
            and(eq(deals.tenantId, tenantId), ...(userId ? [eq(deals.assignedTo, userId)] : [])),
          ),
      ]);
        ...
        db
          ...
          .where(
            and(
              ...
              ...(userId ? [eq(deals.assignedTo, userId)] : []),
            ),
          ),
        ...
        db
          ...
          .where(
            and(
              ...
              sql`created_at >= NOW() - INTERVAL '30 days'`,
              ...(userId ? [eq(serviceTickets.assignedTechnician, userId)] : []),
            ),
          ),
...
export function registerModularDashboardRoutes(app: Express) {
  ...
  app.get('/api/dashboard/alerts', async (req: any, res) => {
    try {
      ...
      const alerts = await dashboardModules.management.alerts(tenantId);
      res.json(alerts);
    } catch (error) {
