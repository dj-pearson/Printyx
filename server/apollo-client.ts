...
export class ApolloClient {
  ...
  async searchPeople(filters: ApolloSearchFilters): Promise<ApolloSearchResponse> {
    try {
      const response = await this.client.post<ApolloSearchResponse>('/v1/mixed_people/search', {
        page: filters.page || 1,
...
export const createApolloClientForTenant = async (tenantId: string): Promise<ApolloClient> => {
  ...
  if (!credential || !credential.apiKey) {
    throw new Error(
      'Apollo.io API key not configured for this company. Please add your Apollo.io API key in Integration Hub → Apollo.io',
    );
  }

  if (credential.status !== 'active') {
    throw new Error(
      `Apollo.io integration is ${credential.status}. Please activate it in Integration Hub.`,
    );
  }
