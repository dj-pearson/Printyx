...
const envSchema = z.object({
  ...
  // Session (required in production, must be at least 32 characters)
  SESSION_SECRET: z.string().min(1, 'SESSION_SECRET is required for secure sessions').optional(),
  ...
  if (isProduction) {
    ...
    if (!process.env.STRIPE_SECRET_KEY) {
      ...
      if (process.env.STRIPE_SECRET_KEY.startsWith('sk_test_')) {
        errors.push(
          'STRIPE_SECRET_KEY: Test key (sk_test_*) detected in production. Use a live key (sk_live_*)',
        );
      }
      ...
      if (!process.env.STRIPE_WEBHOOK_SECRET) {
        ...
        warnings.push(
          'STRIPE_WEBHOOK_SECRET: Test webhook secret detected. Verify this is correct for production',
        );
      }
    }
    if (process.env.STRIPE_PUBLISHABLE_KEY?.startsWith('pk_test_')) {
      errors.push(
        'STRIPE_PUBLISHABLE_KEY: Test key (pk_test_*) detected in production. Use a live key (pk_live_*)',
      );
    }
