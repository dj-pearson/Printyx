...
import { db } from '../db';
import { getSecuritySettings, updateSecuritySettings, getTenantAdmins } from '../storage/security-storage';
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const config = tenantId
        ? await getTenantIpConfig(tenantId)
        : { ...DEFAULT_IP_CONFIG };
      ...
      const isAllowed = config.mode === 'allow'
      ...
      if (!isAllowed) {
        // Log denied access
        if (config.logDenied) {
          console.warn(`[IP WHITELIST] Access denied for IP: ${clientIp}, path: ${req.path}, tenant: ${tenantId || 'unknown'}`);
        }
