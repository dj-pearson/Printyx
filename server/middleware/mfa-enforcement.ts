...
export function requireMfaForAdmins(options: MfaEnforcementOptions = {}) {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const isTestMode = skipForTestMode &&
      ...
      if (!mfaStatus?.enabled) {
        return res.status(403).json({
          ...
          code: 'MFA_REQUIRED',
          message: 'Your role requires Multi-Factor Authentication. Please enable MFA in your security settings.',
 ...
 */
export async function getUsersNeedingMfa(tenantId: string): Promise<Array<{
  id: string;
  email: string;
  firstName: string;
  lastName: string;
