...
export async function getSessionConfig(tenantId?: string): Promise<SessionTimeoutConfig> {
  ...
  try {
    ...
    if (settings) {
      return {
        idleTimeoutMinutes: settings.sessionTimeoutMinutes || DEFAULT_SESSION_CONFIG.idleTimeoutMinutes,
        absoluteTimeoutHours: DEFAULT_SESSION_CONFIG.absoluteTimeoutHours,
        warningBeforeTimeoutMinutes: settings.sessionWarningMinutes || DEFAULT_SESSION_CONFIG.warningBeforeTimeoutMinutes,
        maxConcurrentSessions: settings.maxConcurrentSessions || DEFAULT_SESSION_CONFIG.maxConcurrentSessions,
...
export function enforceSessionTimeout(options?: {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (expired) {
        // Log the timeout
        console.log(`[SESSION] Session expired due to ${reason} timeout for user ${session.userId}`);
      ...
      if (timeUntilWarning !== null && timeUntilWarning <= 0 && !session.sessionMetadata.warningShown) {
        // Set header to indicate session is about to expire
        res.setHeader('X-Session-Expiring-Soon', 'true');
        res.setHeader('X-Session-Expires-In', Math.floor((session.sessionMetadata.idleExpiry - Date.now()) / 1000).toString());
...
export async function terminateSession(
  ...
  try {
    await db
      ...
      .where(
        and(
          eq(securitySessions.id, sessionId),
          eq(securitySessions.userId, userId)
        )
      );
