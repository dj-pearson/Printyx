...
import { storeExportFile, loadExportFile } from './services/address-book/export-storage';
...
async function finishExport(
  ...
  // PA-055 AC4: tenant-scoped object storage, the same bucket and key the edge
  // function writes, so an export generated on either host downloads from either
  // host. The in-process Map this replaced could not survive a restart and was
  // already wrong behind more than one Node process.
  const stored = await storeExportFile(tenantId, exportRow.id, file.content);
  if (!stored) {
    return res.status(500).json({
      message: 'The file was generated but could not be stored',
...
router.get(
  ...
  async (req: any, res) => {
    ...
    const [row] = await db
      .select({ id: addressBookExports.id, filename: addressBookExports.targetFilename })
      .from(addressBookExports)
      .where(
        and(
          eq(addressBookExports.id, req.params.exportId),
          eq(addressBookExports.tenantId, tenantId),
        ),
      )
      .limit(1);

    if (!row) {
      return res.status(404).json({ message: 'Export not found', code: 'NOT_FOUND' });
    }

    const content = await loadExportFile(tenantId, req.params.exportId);
    if (!content) {
      // The row outlives the object if the bucket is pruned. That is a 410 about
      // the FILE, which is a different answer from an export that never existed.
      return res.status(410).json({
        message: 'The export file is no longer available; re-run the export',
        code: 'GONE',
      });
    }

    const filename = String(row.filename || 'address-book.csv').replace(/["\r\n]/g, '');
