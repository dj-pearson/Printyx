...
export function registerClientMonitoringRoutes(app: Express) {
  // ============================================================
  // CLIENT MANAGEMENT ENDPOINTS - RETIRED (EDGE-016)
  // ============================================================
  //
  // The admin CRUD for /api/monitoring-clients used to live here: list,
  // create, detail, PUT, PATCH, DELETE, rotate-key, activity and
  // discovered-devices. All nine were unreachable. registerEdgeFunctionProxy
  // runs before this function in routes-registry.ts and forwards the whole
  // /api/monitoring-clients prefix to supabase/functions/monitoring-clients,
  // and production hits that function directly - so nothing ever reached
  // these handlers on either host.
  //
  // They had also drifted from the live copy: the UI calls
  // /:id/regenerate-key while Express implemented /:id/rotate-key, Express
  // had no installer.zip, and its list returned a bare array where the UI
  // reads { clients }. Editing the dead copy looked like shipping a fix and
  // changed nothing.
  //
  // activity and discovered-devices had no edge counterpart, so they were
  // ported rather than dropped. Everything else was already served there.
  // server/tests/unit/monitoring-clients-convergence.test.ts fails if an
  // admin route reappears here.
  //
  // The agent ingest below (/api/client-metrics/*) is NOT proxied and stays
  // on Express.
  ...
  // The three /api/fleet/* handlers that stood here are gone (AUDIT-031).
  // FleetMonitoringDashboard now calls /api/device-monitoring/fleet-dashboard,
  // /fleet-devices and /fleet-devices/:id/{metrics,order-toner}, which is a
  // proxied prefix - so dev and production run the same code. /api/fleet is not
  // proxied and belongs to supabase/functions/fleet/, which is about vehicles:
