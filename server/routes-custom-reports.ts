...
const DATA_SOURCE_TABLES: Record<string, any> = {
  ...
  equipment: equipment,
  contacts: businessRecords, // Contacts are stored in businessRecords with recordType='contact'
  invoices: invoices,
};
...
export function registerCustomReportsRoutes(app: Express) {
      ...
      try {
        ...
        const [newReport] = await db
          ...
          .values({
            ...
            availableFilters: config.filters.reduce(
              (acc, f) => {
            ...
            availableGroupings: config.groupings.reduce(
              (acc, g) => {
   ...
   */
  app.post(
    '/api/reports/custom/:identifier/execute',
    isAuthenticated,
    async (req: CustomReportRequest, res: Response) => {
      try {
        const tenantId = req.tenantId!;
