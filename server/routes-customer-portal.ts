...
const requireCustomerPortalAuth = async (req: any, res: any, next: any) => {
  try {
    // First check standard auth using unified helper
    const userId = getUserId(req);
    ...
    if (req.path.includes('/equipment-') || req.path.includes('/customer-')) {
      if (!req.user.customerId && !req.customerPortalUser) {
        return res.status(403).json({
          message:
            "Customer context required. Please ensure you're accessing through the customer portal.",
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const stats = {
        ...
        pendingRequests: serviceRequestsCount.filter((request) => request.status === 'submitted')
          .length,
        unreadNotifications: notificationsCount.filter((notification) => !notification.isPortalRead)
          .length,
      };
  ...
  try {
    ...
    const validationResult = equipmentHealthRequestSchema.safeParse({
      ...
      equipmentIds: req.query.equipmentIds
        ? JSON.parse(req.query.equipmentIds as string)
        : undefined,
...
router.get('/usage-analytics', requireCustomerPortalAuth, async (req, res) => {
  try {
    ...
    const analytics = await customerPortalService.getUsageAnalytics(tenantId, customerId, options);
...
router.get('/equipment-usage/:equipmentId', requireCustomerPortalAuth, async (req, res) => {
  try {
    ...
    const analytics = await customerPortalService.getUsageAnalytics(tenantId, customerId, {
      timeRange: options.timeRange,
...
router.get(
  '/maintenance-appointments/:appointmentId',
...
router.put(
  '/maintenance-appointments/:appointmentId/reschedule',
  requireCustomerPortalAuth,
  async (req, res) => {
    try {
      const tenantId = req.user?.tenant_id;
...
router.get('/service-requests', requireCustomerPortalAuth, async (req, res) => {
  try {
    ...
    const result = await customerPortalService.getCustomerServiceRequests(tenantId, customerId, {
      status,
      limit,
      offset,
      fields,
  ...
  try {
    ...
    const availableSurveys = await db
      .select({
        ...
        relatedMaintenanceAppointmentId:
          customerSatisfactionSurveys.relatedMaintenanceAppointmentId,
...
router.post(
  '/satisfaction/surveys/:surveyId/start',
  requireCustomerPortalAuth,
  async (req, res) => {
    try {
      const tenantId = req.user?.tenant_id;
...
router.post(
  '/satisfaction/surveys/:surveyId/submit',
  requireCustomerPortalAuth,
  async (req, res) => {
    try {
      const tenantId = req.user?.tenant_id;
      ...
      const updatedSurvey = await db.transaction(async (tx) => {
        ...
        if (validatedResponses.length > 0) {
          await tx.insert(customerSatisfactionSurveyResponses).values(validatedResponses);
        }
