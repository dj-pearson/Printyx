...
export function registerDealDeskRoutes(app: Express) {
  ...
  app.get("/api/deal-desk/dashboard", isAuthenticated, cacheControl(300), etag(), async (req: any, res) => {
    try {
      const tenantId = req.user.tenantId;
      const userId = req.user.claims.sub;
      ...
      const approvalRate = totalDecisions > 0
        ? ((approvedCount.count || 0) / totalDecisions) * 100
        : 0;
      ...
      if (recentCompleted.length > 0) {
        const totalHours = recentCompleted.reduce((sum, req) => {
          ...
          const hours = (req.completedAt.getTime() - req.submittedAt.getTime()) / (1000 * 60 * 60);
          return sum + hours;
        }, 0);
