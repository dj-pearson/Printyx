  ...
  async (req: TenantRequest, res: Response) => {
    try {
      ...
      let query = db
        ...
        .where(eq(equipmentDisposal.tenantId, tenantId));
 ...
 */
router.get(
  '/api/equipment-disposal/:disposalId',
  async (req: TenantRequest, res: Response) => {
    try {
      ...
      const [disposal] = await db
        ...
        .where(
          and(
            eq(equipmentDisposal.id, disposalId),
            eq(equipmentDisposal.tenantId, tenantId)
          )
        );
