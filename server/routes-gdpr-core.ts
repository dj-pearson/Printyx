 ...
 */
router.get('/data-export/categories',
...
router.post('/data-export/requests',
  ...
  async (req: TenantRequest, res: Response) => {
    try {
      const exportRequest = await gdprDataExportService.createExportRequest(
        req.tenantId!,
        {
          ...req.body,
 ...
 */
router.get('/data-export/requests/:id',
  requireRole(['admin', 'compliance_officer', 'legal', 'manager']),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.post('/data-export/requests/:id/process',
 ...
 */
router.get('/data-export/requests/:id/download',
  requireRole(['admin', 'compliance_officer', 'legal', 'manager']),
 ...
 */
router.get('/data-export/templates',
  requireRole(['admin', 'compliance_officer', 'manager']),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.post('/data-export/templates',
  requireRole(['admin', 'compliance_officer']),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.get('/consent/subject/:type/:id',
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.get('/consent/subject/:id/summary',
  async (req: TenantRequest, res: Response) => {
    try {
      const summary = await consentManagementService.getConsentSummary(
        req.tenantId!,
        req.params.id
      );
 ...
 */
router.get('/consent/check/:subjectId/:consentType',
  async (req: TenantRequest, res: Response) => {
    try {
...
router.post('/dpa',
  ...
  async (req: TenantRequest, res: Response) => {
    try {
      const dpa = await dpaManagementService.createDpa(
        req.tenantId!,
        req.body,
        req.user!.id
      );
...
router.put('/dpa/:id',
  ...
  async (req: TenantRequest, res: Response) => {
    try {
      ...
      res.status(500).json({ message: error instanceof Error ? error.message : 'Failed to update DPA' });
    }
  }
);
 ...
 */
router.post('/dpa/:id/submit-review',
  requireRole(['admin', 'compliance_officer', 'legal']),
  auditLogMiddleware('SUBMIT_DPA_REVIEW', 'data_processing_agreements', 'medium', 'data_modification'),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 * Get expiring DPAs
 */
router.get('/dpa/alerts/expiring',
  requireRole(['admin', 'compliance_officer', 'legal', 'manager']),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.get('/dpa/alerts/pending-compliance',
  requireRole(['admin', 'compliance_officer']),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.post('/dpa/:id/subprocessors',
  requireRole(['admin', 'compliance_officer', 'legal']),
  auditLogMiddleware('ADD_SUBPROCESSOR', 'data_processing_agreements', 'medium', 'data_modification'),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.delete('/dpa/:id/subprocessors/:name',
  requireRole(['admin', 'compliance_officer', 'legal']),
  auditLogMiddleware('REMOVE_SUBPROCESSOR', 'data_processing_agreements', 'medium', 'data_modification'),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.post('/dpa/:id/compliance-checks',
  requireRole(['admin', 'compliance_officer']),
  auditLogMiddleware('CREATE_COMPLIANCE_CHECK', 'dpa_compliance_checks', 'high', 'data_modification'),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.get('/dpa/:id/compliance-checks',
  requireRole(['admin', 'compliance_officer', 'legal']),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.post('/deduplication/rules',
  requireRole(['admin', 'manager']),
  auditLogMiddleware('CREATE_DEDUP_RULE', 'duplicate_detection_rules', 'medium', 'data_modification'),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.get('/deduplication/rules',
  requireRole(['admin', 'manager']),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.get('/deduplication/rules/:id',
  requireRole(['admin', 'manager']),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.put('/deduplication/rules/:id',
  requireRole(['admin', 'manager']),
  auditLogMiddleware('UPDATE_DEDUP_RULE', 'duplicate_detection_rules', 'medium', 'data_modification'),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.post('/deduplication/find',
  requireRole(['admin', 'manager']),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.get('/deduplication/matches',
  requireRole(['admin', 'manager']),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.post('/deduplication/matches/:id/resolve',
  requireRole(['admin', 'manager']),
...
router.post('/deduplication/merge',
  ...
  async (req: TenantRequest, res: Response) => {
    try {
      const { entityType, survivingRecordId, mergedRecordId, fieldResolutions, duplicateMatchId, mergeReason } = req.body;
      ...
      res.status(500).json({ message: error instanceof Error ? error.message : 'Failed to merge records' });
    }
  }
);
 ...
 * Rollback merge
 */
router.post('/deduplication/merge/:id/rollback',
  requireRole(['admin']),
  ...
  async (req: TenantRequest, res: Response) => {
    try {
      ...
      res.status(500).json({ message: error instanceof Error ? error.message : 'Failed to rollback merge' });
    }
  }
);
 ...
 */
router.get('/deduplication/merge/history/:recordId',
  requireRole(['admin', 'manager']),
  async (req: TenantRequest, res: Response) => {
    try {
 ...
 */
router.post('/deduplication/scan',
  requireRole(['admin', 'manager']),
 ...
 * Run scan job
 */
router.post('/deduplication/scan/:id/run',
  requireRole(['admin', 'manager']),
  async (req: TenantRequest, res: Response) => {
    try {
      // Run the scan job asynchronously
      contactDeduplicationService.runScanJob(req.tenantId!, req.params.id)
