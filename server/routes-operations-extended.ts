...
export function registerOperationsExtendedRoutes(app: Express) {
  // Remote monitoring handlers REMOVED (AUDIT-030).
  //
  // Both were hardcoded fixtures, and they were the ones Express actually
  // served: routes-sample-data.ts held a third copy and routes-remote-monitoring.ts
  // held the only real one, over device_registrations and device_metrics, and
  // that file was never registered. Three implementations, and the two that ran
  // were invented. /remote-monitoring redirects to /fleet-monitoring now, which
  // reads the same fleet through supabase/functions/device-monitoring/.
