...
export function registerPipelineConfigurationRoutes(app: Express) {
   ...
   */
  app.get("/api/pipeline-config/templates", isAuthenticated, async (req: Request, res: Response) => {
    try {
   ...
   */
  app.get("/api/pipeline-config/templates/:id", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const { user } = req as AuthenticatedRequest;
      const { id } = req.params;
      if (!user) return res.status(401).json({ message: "Unauthorized" });
   ...
   */
  app.post("/api/pipeline-config/templates", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const { user } = req as AuthenticatedRequest;
      if (!user) return res.status(401).json({ message: "Unauthorized" });
   ...
   */
  app.put("/api/pipeline-config/templates/:id", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const { user } = req as AuthenticatedRequest;
      const { id } = req.params;
      if (!user) return res.status(401).json({ message: "Unauthorized" });
   ...
   */
  app.delete("/api/pipeline-config/templates/:id", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const { user } = req as AuthenticatedRequest;
      const { id } = req.params;
      if (!user) return res.status(401).json({ message: "Unauthorized" });
      ...
      const dealsCount = await db
        ...
        .where(
          and(
            ...
            eq(deals.pipelineTemplateId, id)
          )
        );
   ...
   */
  app.post("/api/pipeline-config/templates/:id/clone", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const { user } = req as AuthenticatedRequest;
      const { id } = req.params;
      const { name } = req.body;
      ...
      await db.transaction(async (tx) => {
        ...
        const clonedStages = await tx
          .insert(pipelineStages)
          .values(clonedStageData)
          .returning();
   ...
   */
  app.get("/api/pipeline-config/stages/:templateId", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const { user } = req as AuthenticatedRequest;
   ...
   * Update stage
   */
  app.put("/api/pipeline-config/stages/:id", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const { user } = req as AuthenticatedRequest;
      const { id } = req.params;
      if (!user) return res.status(401).json({ message: "Unauthorized" });
   ...
   * Reorder stages
   */
  app.put("/api/pipeline-config/stages/reorder", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const { user } = req as AuthenticatedRequest;
   ...
   * Delete stage
   */
  app.delete("/api/pipeline-config/stages/:id", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const { user } = req as AuthenticatedRequest;
      const { id } = req.params;
      if (!user) return res.status(401).json({ message: "Unauthorized" });
   ...
   */
  app.post("/api/pipeline-config/deals/:dealId/transition", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const { user } = req as AuthenticatedRequest;
   ...
   */
  app.get("/api/pipeline-config/deals/:dealId/history", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const { user } = req as AuthenticatedRequest;
      const { dealId } = req.params;
      if (!user) return res.status(401).json({ message: "Unauthorized" });
      ...
      const history = await db
        ...
        .where(
          and(
            ...
            eq(dealStageHistory.tenantId, user.tenantId)
          )
        )
   ...
   */
  app.get("/api/pipeline-config/analytics/conversion", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const { user } = req as AuthenticatedRequest;
   ...
   */
  app.get("/api/pipeline-config/analytics/velocity", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const { user } = req as AuthenticatedRequest;
      if (!user) return res.status(401).json({ message: "Unauthorized" });
