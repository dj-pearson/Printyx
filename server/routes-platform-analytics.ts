...
router.get('/cohort-analysis', async (req, res) => {
  try {
    ...
    if (startDate) {
      whereConditions.push(gte(platformCohortAnalysis.cohortStartDate, new Date(startDate as string)));
    }
    if (endDate) {
      whereConditions.push(lte(platformCohortAnalysis.cohortStartDate, new Date(endDate as string)));
    }

    const cohorts = await db.query.platformCohortAnalysis.findMany({
      where: and(
        eq(platformCohortAnalysis.periodType, periodType as string),
        ...whereConditions
      ),
...
router.post('/cohort-analysis/generate', async (req, res) => {
  try {
    ...
    const revenueAtStart = cohortCustomers.reduce((sum, c) =>
      sum + parseFloat(c.currentMRR || '0'), 0
    );

    // Calculate current retention
    const activeCustomers = cohortCustomers.filter(c =>
      c.status === 'active_customer' || c.status === 'trial_active'
    );
    const customersNow = activeCustomers.length;
    const revenueNow = activeCustomers.reduce((sum, c) =>
      sum + parseFloat(c.currentMRR || '0'), 0
    );

    const retentionRate = customersAtStart > 0
      ? ((customersNow / customersAtStart) * 100).toFixed(2)
      : '0';
    const revenueRetentionRate = revenueAtStart > 0
      ? ((revenueNow / revenueAtStart) * 100).toFixed(2)
      : '0';

    // Calculate churn
    const churnedCustomers = cohortCustomers.filter(c =>
      c.status === 'churned' || c.status === 'trial_churned'
    );
    ...
    const [cohort] = await db.insert(platformCohortAnalysis).values({
...
router.get('/revenue-metrics', async (req, res) => {
  try {
    ...
    const totalMRR = activeTenants.reduce((sum, t) =>
      sum + parseFloat(t.currentMRR || '0'), 0
    );
    ...
    const churnRate = (activeTenants.length + churnedTenants.length) > 0
    ...
    const expandedTenants = activeTenants.filter(t =>
      t.lastMRRChange && parseFloat(t.lastMRRChange) > 0
    );
    const expansionMRR = expandedTenants.reduce((sum, t) =>
      sum + parseFloat(t.lastMRRChange || '0'), 0
    );
...
router.get('/conversion-funnel', async (req, res) => {
  try {
    ...
    const contactedRate = totalProspects > 0
      ? (statusCounts.contacted / totalProspects) * 100
      : 0;
    const qualifiedRate = totalProspects > 0
      ? (statusCounts.qualified / totalProspects) * 100
      : 0;
    const trialRate = totalProspects > 0
      ? (statusCounts.trial_active / totalProspects) * 100
      : 0;
    const conversionRate = totalProspects > 0
      ? (totalTenants / totalProspects) * 100
      : 0;

    // Get deal conversion rates
    const deals = await db.query.platformDeals.findMany({
      where: whereConditions.length > 0
        ? and(...whereConditions.map(c => {
...
router.get('/sales-performance', async (req, res) => {
  try {
    ...
    const repMetrics = reports.reduce((acc, report) => {
    ...
    Object.values(repMetrics).forEach((metrics: any) => {
      metrics.avgDealSize = metrics.dealsWon > 0
        ? metrics.revenueGenerated / metrics.dealsWon
        : 0;
      metrics.conversionRate = metrics.leadsGenerated > 0
        ? (metrics.dealsWon / metrics.leadsGenerated) * 100
        : 0;
    });

    // Get top performers
    const sortedReps = Object.values(repMetrics).sort((a: any, b: any) =>
    ...
    const teamTotals = {
      ...
      opportunitiesCreated: sortedReps.reduce((sum: number, r: any) => sum + r.opportunitiesCreated, 0),
...
router.get('/churn-analysis', async (req, res) => {
  try {
    ...
    const whereConditions = [];
    if (startDate) {
      whereConditions.push(gte(platformChurnPredictions.predictedAt, new Date(startDate as string)));
    }
    if (endDate) {
    ...
    // Group by risk level
    const byRiskLevel = predictions.reduce((acc, pred) => {
    ...
    const churnedRevenue = churnedTenants.reduce((sum, t) =>
      sum + parseFloat(t.currentMRR || '0'), 0
    ) * 12; // Annualized
...
router.get('/goals-tracking', async (req, res) => {
  try {
    ...
    const goalsWithAttainment = goals.map(goal => {
      const revenueAttainment = goal.revenueTarget && parseFloat(goal.revenueTarget) > 0
        ? (parseFloat(goal.actualRevenue || '0') / parseFloat(goal.revenueTarget)) * 100
        : 0;

      const dealsAttainment = goal.dealsTarget && goal.dealsTarget > 0
        ? ((goal.actualDeals || 0) / goal.dealsTarget) * 100
        : 0;

      const activitiesAttainment = goal.activitiesTarget && goal.activitiesTarget > 0
      ...
      return {
        ...
        gaps: {
          revenue: (parseFloat(goal.revenueTarget || '0') - parseFloat(goal.actualRevenue || '0')).toFixed(2),
    ...
    const teamSummary = {
      ...
      goalsOnTrack: goalsWithAttainment.filter((g: any) => parseFloat(g.attainment.overall) >= 90).length,
      goalsAtRisk: goalsWithAttainment.filter((g: any) => parseFloat(g.attainment.overall) < 70).length,
    };
...
router.get('/health-trends', async (req, res) => {
  try {
    ...
    const byStatus = healthScores.reduce((acc, score) => {
