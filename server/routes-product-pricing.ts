...
export function registerProductPricingRoutes(app: Express) {
   ...
   */
  app.post("/api/pricing/calculate-rep-cost", isAuthenticated, async (req: AuthRequest, res: Response) => {
    try {
      const tenantId = req.user?.tenantId;
      if (!tenantId) {
        return res.status(400).json({ error: "Tenant ID required" });
   ...
   */
  app.patch("/api/product-models/:id/pricing", isAuthenticated, async (req: AuthRequest, res: Response) => {
    try {
      const tenantId = req.user?.tenantId;
      const userRole = req.user?.role || 'standard';
      ...
      if (repMarkupPercentage !== undefined) {
        ...
        // Recalculate rep cost
        const currentDealerCost = updateData[`${tier}DealerCost`] ||
          product[`${tier}DealerCost` as keyof typeof product];
   ...
   */
  app.post("/api/pricing/bulk-update-dealer-cost", isAuthenticated, async (req: AuthRequest, res: Response) => {
    try {
      const tenantId = req.user?.tenantId;
      const userRole = req.user?.role || 'standard';
   ...
   * Accessible to: Managers and above
   */
  app.get("/api/pricing/margin-report", isAuthenticated, async (req: AuthRequest, res: Response) => {
    try {
      const tenantId = req.user?.tenantId;
      const userRole = req.user?.role || 'standard';
      ...
      const lineItems = quoteIds.length > 0
        ? await db
            .select()
   ...
   * Accessible to: Managers and above
   */
  app.get("/api/pricing/margin-report/export", isAuthenticated, async (req: AuthRequest, res: Response) => {
    try {
      const tenantId = req.user?.tenantId;
      const userRole = req.user?.role || 'standard';
   ...
   */
  app.post("/api/pricing/request-approval", isAuthenticated, async (req: AuthRequest, res: Response) => {
    try {
      const tenantId = req.user?.tenantId;
   ...
   * Accessible to: Managers and above
   */
  app.patch("/api/pricing/approval/:id", isAuthenticated, async (req: AuthRequest, res: Response) => {
    try {
      const tenantId = req.user?.tenantId;
      const userId = req.user?.id;
      const userRole = req.user?.role || 'standard';
   ...
   * Accessible to: Managers and above
   */
  app.get("/api/pricing/approvals/pending", isAuthenticated, async (req: AuthRequest, res: Response) => {
    try {
      const tenantId = req.user?.tenantId;
      const userRole = req.user?.role || 'standard';
