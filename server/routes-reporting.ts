...
router.get(
  '/kpis',
  requireReportPermission('canViewKPIs'),
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const user = req.user!;
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { id } = req.params;
      const user = req.user!;
      const { from_date, to_date, granularity = 'monthly', dimension } = req.query;
      ...
      res.json({
        ...
        summary: {
          ...
          average_value:
            historicalData.reduce((sum, val) => sum + (Number(val.actualValue) || 0), 0) /
