...
router.get('/api/reports/customer-health', async (req: any, res) => {
  try {
      ...
      if (custContracts.length > 0) {
        ...
        const upcomingRenewals = custContracts.filter((con) => {
          ...
          const daysUntilEnd = Math.floor(
            (new Date(con.endDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24),
          );
    ...
    const avgHealthScore =
      customerHealthScores.length > 0
        ? Math.round(
            customerHealthScores.reduce((sum, c) => sum + c.healthScore, 0) /
              customerHealthScores.length,
          )
        : 0;
    ...
    res.json({
      ...
      scoreDistribution: {
        ...
        medium: customerHealthScores.filter((c) => c.healthScore >= 50 && c.healthScore < 75)
          .length,
...
router.get('/api/reports/technician-utilization', async (req: any, res) => {
  try {
    ...
    const byTechnician = techList.map((t) => {
      ...
      const completedTickets = techTickets.filter(
        (tk) => tk.status === 'completed' || tk.status === 'closed',
      );
      const openTickets = techTickets.filter(
        (tk) => tk.status !== 'completed' && tk.status !== 'closed',
      );
      ...
      const firstTimeFix = completedTickets.filter(
        (tk) =>
          !techTickets.some(
            (other) =>
              other.id !== tk.id &&
    ...
    const avgUtilization =
      byTechnician.length > 0
        ? Math.round(
            byTechnician.reduce((sum, t) => sum + t.utilizationPercent, 0) / byTechnician.length,
          )
        : 0;
    ...
    const avgSLA =
      byTechnician.length > 0
        ? Math.round(
            byTechnician.reduce((sum, t) => sum + t.slaCompliance, 0) / byTechnician.length,
          )
        : 0;
    ...
    const avgFirstTimeFix =
      byTechnician.length > 0
        ? Math.round(
            byTechnician.reduce((sum, t) => sum + t.firstTimeFixRate, 0) / byTechnician.length,
          )
        : 0;
    ...
    res.json({
      ...
      goodCount: byTechnician.filter((t) => t.utilizationPercent >= 75 && t.utilizationPercent < 90)
        .length,
