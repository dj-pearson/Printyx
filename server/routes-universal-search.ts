...
// CR-029/CR-031: short-lived, tenant-scoped cache for universal-search results.
// Searches are read-only and typing-driven (many near-identical requests), so a
// small TTL absorbs bursts without risking meaningfully stale results.
...
router.get('/api/universal-search', async (req: TenantRequest, res: Response) => {
  try {
    ...
    // CR-031: serve identical recent searches from the tenant-scoped cache.
    const cacheKey = buildSearchCacheKey(tenantId, query, limit);
    const cached = await searchCache.get(cacheKey);
    if (cached) {
      log.debug('universal-search cache hit', { tenantId, query });
      res.setHeader('X-Cache', 'HIT');
      return res.json(cached);
    }
    log.debug('universal-search cache miss', { tenantId, query });

    // CR-029: run all six entity searches in parallel instead of sequentially.
    // business-records and deals surface errors (outer 500); the four optional
    // entities degrade to [] via safeSearch so a missing table can't fail search.
    const [
      businessRecordsResults,
      dealsResults,
      activitiesResults,
      quotesResults,
      invoiceResults,
      ...
      safeSearch(
        () =>
          db
            ...
            .limit(limit),
        'activities',
        (label, error) => log.warn(`Could not search ${label}:`, error),
      ),
      safeSearch(
        () =>
          db
    ...
    // Map deals results (fetched in parallel above)
    ...
    // Map activities results (fetched in parallel above)
    ...
    // Map quotes results (fetched in parallel above)
    ...
    // Map invoices results (fetched in parallel above)
    for (const invoice of invoiceResults) {
      const title = `Invoice ${invoice.invoiceNumber || invoice.id}`;
      const subtitle = [
        invoice.companyName,
        invoice.invoiceDate ? `Date: ${new Date(invoice.invoiceDate).toLocaleDateString()}` : null,
      ]
        .filter(Boolean)
        .join(' • ');
    ...
    // Map equipment results (fetched in parallel above)
    for (const equip of equipmentResults) {
      const title =
        [equip.manufacturer, equip.modelNumber].filter(Boolean).join(' ') ||
        equip.serialNumber ||
        'Unknown Equipment';
      const subtitle = [equip.companyName, equip.serialNumber ? `S/N: ${equip.serialNumber}` : null]
        .filter(Boolean)
        .join(' • ');
    ...
    // CR-029: sort by relevance and limit (pure helper).
    const sortedResults = sortAndLimitResults(results, limit);

    // CR-031: cache the assembled results (tenant-scoped key, short TTL).
