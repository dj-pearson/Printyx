...
// Phase 3 Routes Refactor - Modular route files
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Determine cookie domain for cross-subdomain sharing in production
  // This allows cookies to work across printyx.net and api.printyx.net
  const cookieDomain =
  ...
  app.use(
    session({
      ...
      cookie: {
        ...
        sameSite: app.get('env') === 'production' ? 'none' : 'lax', // 'none' for cross-origin with credentials
        maxAge: 24 * 60 * 60 * 1000,
        domain: cookieDomain, // Allow cross-subdomain cookies in production
      },
  ...
  // Current user (Supabase JWT / session-aware)
  // This endpoint is intended for the frontend to hydrate profile/role/team from the server DB
  ...
  // NOTE: Contacts routes migrated to routes-contacts.ts (Phase 3 Refactor)
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        console.log(
          `[IMPORT DEBUG] Import completed: ${imported} imported, ${skipped} skipped, ${duplicates} duplicates`,
        );
...
`;
  ...
  const dashboardCustomizationRoutes = await import('./routes-dashboard-customization');
  app.use('/api/dashboard', dashboardCustomizationRoutes.default);
  ...
  // Email Marketing Service Integration Routes
  ...
  app.get(
    ...
    async (req: any, res) => {
      ...
      `;
        ...
        const result = await db.$client.query(query, [String(tenantId), futureDate.toISOString()]);
        res.json(result.rows);
      } catch (error) {
      ...
      try {
        ...
        if (search) {
          whereConditions.push(
            `(title ILIKE $${queryParams.length + 1} OR content ILIKE $${queryParams.length + 1})`,
          );
  ...
  // NOTE: Automation routes migrated to routes-automation.ts (Phase 3 Refactor)
  ...
  // ============================================
  // Phase 3 Routes Refactor - Register modular routes
  // ============================================
