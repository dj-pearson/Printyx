...
function isAdmin(req: Request): boolean {
  const user = (req as any).user || (req as any).supabaseUser;
  if (!user) return false;

  // Check for admin role or platform admin
  const roleLevel = user.roleLevel || user.role?.level || 0;
  const isPlatformAdmin = user.isPlatformUser || user.canAccessAllTenants;

  return roleLevel >= 6 || isPlatformAdmin;
}
...
router.get('/status', async (req: Request, res: Response) => {
  try {
    ...
    const [existingCompany] = await db
      .select()
      .from(companies)
      .where(and(eq(companies.tenantId, user.tenantId), eq(companies.name, 'Acme Corporation')));
...
router.post('/demo', async (req: Request, res: Response) => {
  try {
    ...
    if (!user?.id || !user?.tenantId) {
      return res.status(401).json({ error: 'Unauthorized - no user/tenant context' });
    }

    const DEMO_TENANT_ID = user.tenantId;
    const DEMO_USER_ID = user.id;
    const force = req.body.force === true;

    console.log(`[Seed] Starting demo data seeding for tenant: ${DEMO_TENANT_ID}`);
    console.log(`[Seed] User: ${user.email} (${DEMO_USER_ID})`);
