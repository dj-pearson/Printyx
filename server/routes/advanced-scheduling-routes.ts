...
router.post('/constraints/validate', requireAuth, async (req, res) => {
  try {
    ...
    if (hardConstraints.length > (tasks?.length || 0) * 3) {
      ...
      validationResult.suggestions.push('Consider relaxing some hard constraints to soft constraints');
    }
  ...
  try {
    ...
    const userPatterns = {
      ...
      productivityPeaks: [
        ...
        { hour: 10, score: 95, confidence: 0.90 },
        { hour: 14, score: 78, confidence: 0.75 },
        { hour: 15, score: 82, confidence: 0.80 }
      ],
      taskCompletionRates: {
        'lead_follow_up': 0.87,
