...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const data = await DirectorReportingService.getCompanySalesPerformance(
        req.user!,
        dateRange
      );
      ...
      res.json({
        ...data,
        insights: {
          performanceStatus: data.quotaAttainment >= 100 ? 'exceeding' :
                            data.quotaAttainment >= 90 ? 'on_track' :
                            data.quotaAttainment >= 75 ? 'near_target' : 'at_risk',
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const data = await DirectorReportingService.getCompanyServicePerformance(
        req.user!,
        dateRange
      );
      ...
      res.json({
        ...data,
        insights: {
          serviceHealth: data.avgFirstTimeFixRate >= 85 && data.avgSlaCompliance >= 90 ? 'excellent' :
                        data.avgFirstTimeFixRate >= 75 && data.avgSlaCompliance >= 85 ? 'good' :
                        data.avgFirstTimeFixRate >= 65 && data.avgSlaCompliance >= 75 ? 'fair' : 'needs_improvement',
          topRegions: data.regions
            .sort((a, b) => b.ftfRate - a.ftfRate)
            .slice(0, 3),
          utilizationStatus: data.avgUtilization >= 80 ? 'optimal' :
                            data.avgUtilization >= 60 ? 'adequate' : 'low',
        },
      });
    } catch (error) {
