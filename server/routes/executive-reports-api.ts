...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const data = await ExecutiveReportingService.getExecutiveDashboard(
        req.user!,
        dateRange
      );
      ...
      res.json({
        ...data,
        insights: {
          overallHealth: data.kpis.filter(k => k.status === 'green').length >= 3 ? 'strong' :
                        data.kpis.filter(k => k.status === 'red').length >= 2 ? 'concerning' : 'moderate',
          profitability: data.financial.netProfit / data.financial.totalRevenue * 100,
          growthTrajectory: data.financial.revenueGrowth >= 15 ? 'accelerating' :
                           data.financial.revenueGrowth >= 5 ? 'steady' : 'slowing',
          customerHealth: data.operational.customerChurn < 3 && data.operational.nps > 40 ? 'excellent' :
                         data.operational.customerChurn < 5 && data.operational.nps > 30 ? 'good' : 'needs_attention',
        },
      });
    } catch (error) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        ...data,
        insights: {
          systemHealth: data.system.uptime >= 99.9 && data.system.errorRate < 0.1 ? 'excellent' :
                       data.system.uptime >= 99.5 && data.system.errorRate < 0.5 ? 'good' : 'needs_attention',
          tenantGrowth: ((data.tenants.active - data.tenants.churned) / data.tenants.total * 100).toFixed(1) + '%',
          revenueHealth: data.billing.expansionRevenue > data.billing.contractionRevenue ? 'positive' : 'negative',
