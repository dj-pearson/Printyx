  ...
  try {
    ...
    let highlights = [
      ...
      },
      {
        ...
        durationSeconds: 13,
        transcriptExcerpt:
          "We'll need to hire 3 additional sales representatives to handle the increased demand.",
      ...
      },
      {
        ...
        durationSeconds: 13,
        transcriptExcerpt:
          'We should also invest in our customer support infrastructure to maintain service quality as we scale.',
...
router.post('/recordings/:recordingId/process', async (req, res) => {
  try {
    const { recordingId } = req.params;
    const { forceReprocess = false, generateNotes = true, extractHighlights = true } = req.body;
