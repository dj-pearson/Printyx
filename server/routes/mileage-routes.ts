/**
 * Mileage: records, vehicles, IRS rates, auto-generation from GPS, and
 * expense reports. 17 handlers over /api/mileage.
 *
 ...
 *
 * No client tree calls /api/mileage and there is no edge function. Its
 * auto-generate path reads the GPS location history that gps-tracking-routes.ts
 * would have written, so the two are unreachable together.
 *
...
router.post('/records', async (req: Request, res: Response) => {
  ...
  try {
    const recordSchema = z.object({
      ...
      date: z.coerce.date(),
      totalMiles: z.string().or(z.number()).transform(String).optional(),
      businessMiles: z.string().or(z.number()).transform(String).optional(),
      personalMiles: z.string().or(z.number()).transform(String).optional(),
      commuteMiles: z.string().or(z.number()).transform(String).optional(),
      startOdometer: z.string().or(z.number()).transform(String).optional(),
      endOdometer: z.string().or(z.number()).transform(String).optional(),
      numberOfTrips: z.number().optional(),
      numberOfStops: z.number().optional(),
      fuelPurchased: z.string().or(z.number()).transform(String).optional(),
      fuelCost: z.string().or(z.number()).transform(String).optional(),
...
router.post('/irs-log', async (req: Request, res: Response) => {
  ...
  try {
    const logSchema = z.object({
      ...
      odometerStart: z.string().or(z.number()).transform(String).optional(),
      odometerEnd: z.string().or(z.number()).transform(String).optional(),
      vehicleUsed: z.string().optional(),
      tolls: z.string().or(z.number()).transform(String).optional(),
      parking: z.string().or(z.number()).transform(String).optional(),
