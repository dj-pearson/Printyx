...
router.post('/records', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const record = await mileageService.recordDailyMileage(
      user.tenantId,
      targetTechnicianId,
      data
    );
...
router.post('/auto-generate', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const recordsCreated = await mileageService.autoGenerateDailyMileage(
      user.tenantId,
      targetDate
    );
...
router.get('/reports', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!isAdminOrManager(user)) {
      query = query.where(
        and(
          eq(mileageReports.tenantId, user.tenantId),
          eq(mileageReports.technicianId, user.id)
        )
      ) as any;
    }
...
router.get('/reports/:id', async (req: Request, res: Response) => {
  ...
  try {
    const [report] = await db
      ...
      .where(
        and(
          eq(mileageReports.id, req.params.id),
          eq(mileageReports.tenantId, user.tenantId)
        )
      )
      .limit(1);
...
router.post('/reports/:id/reject', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const report = await mileageService.rejectReport(
      req.params.id,
      user.tenantId,
      user.id,
      reason
...
router.post('/irs-log', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!isAdminOrManager(user) && targetTechnicianId !== user.id) {
      return res.status(403).json({ error: 'You can only create your own IRS mileage log entries' });
    }
