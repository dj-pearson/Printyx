...
async function runMeetingTranscriptionMigration() {
  ...
  try {
    ...
    console.log('📝 Meeting Transcriptions - AI-powered speech-to-text with speaker identification');
    console.log('📋 Meeting Notes - Intelligent note generation with structured content extraction');
...
if (import.meta.url.startsWith('file:') && fileURLToPath(import.meta.url) === process.argv[1]) {
  runMeetingTranscriptionMigration().then(() => {
