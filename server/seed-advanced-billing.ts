...
async function seedAdvancedBilling() {
  try {
    ...
    const spikeAnomaly = await storage.createMeterAnomaly({
      ...
      anomalyDescription: "Sudden spike in B&W printing detected (42.86% increase). Current reading: 125,000, Expected: 87,500",
      suggestedAction: "Contact customer to verify meter reading accuracy. Possible causes: large print job, meter error, or new equipment usage patterns.",
      requiresReview: true,
      reviewed: true,
      ...
      resolutionNotes: "Reading accepted. Customer confirmed bulk marketing materials printed for trade show.",
    ...
    const negativeAnomaly = await storage.createMeterAnomaly({
      ...
      anomalyDescription: "CRITICAL: Meter reading decreased from previous reading. Current: 2,500 (was 95,000). Possible meter replacement or equipment swap.",
      suggestedAction: "URGENT: Contact customer immediately. Verify equipment serial number. Check if meter was replaced or equipment swapped.",
      requiresReview: true,
      reviewed: false,
      resolved: false,
      autoNotificationSent: true,
    ...
    const stagnantAnomaly = await storage.createMeterAnomaly({
      ...
      anomalyDescription: "No usage detected for 30+ days. Meter reading unchanged: B&W: 52,000, Color: 15,000",
      suggestedAction: "Contact customer to verify equipment status. Equipment may be offline, unplugged, or no longer in use.",
      requiresReview: true,
      reviewed: true,
      reviewedAt: new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000), // 7 days ago
      reviewedBy: "technician-001",
      reviewNotes: "Customer confirmed equipment moved to storage during office renovation. Not in use temporarily.",
    ...
    const outOfRangeAnomaly = await storage.createMeterAnomaly({
      ...
      anomalyDescription: "Slight deviation from expected usage pattern. May indicate seasonal variation.",
      suggestedAction: "Monitor for next reading. If pattern continues, may need to adjust usage baseline.",
    ...
    const meterDisputeOpen = await storage.createBillingDispute({
      ...
      customerComplaint: "The meter reading for October shows 42,000 pages, but our internal records show only 28,000 pages printed. We believe there's an error in the meter reading.",
      ...
      priorityLevel: 2,
      internalNotes: "Customer has requested technician visit to verify meter reading. Scheduling for this week.",
    ...
    const pricingDisputeReview = await storage.createBillingDispute({
      ...
      customerComplaint: "We were quoted a rate of $0.008 per B&W page but invoice shows $0.01 per page. Contract shows the lower rate.",
      ...
      priorityLevel: 2,
      internalNotes: "Confirmed contract shows $0.008 rate. Billing system was using old rate. Need to correct and issue credit memo.",
      researchNotes: "Reviewed contract file. Customer is correct. System has wrong rate configured.",
      communicationLog: [
        {
    ...
    const serviceDisputeResolved = await storage.createBillingDispute({
      ...
      customerComplaint: "Equipment was down for 3 days due to delayed technician visit. We should not be charged for those days.",
      ...
      priorityLevel: 1,
      internalNotes: "Customer is a long-standing account. Service logs confirm 3-day delay due to parts availability. Goodwill gesture warranted.",
      researchNotes: "Service ticket #ST-1234 confirms equipment down Sep 12-15. Technician dispatch delayed due to part backorder.",
      resolutionType: "partial_credit",
      resolutionDescription: "Issued 50% credit ($225) for service interruption. Expedited parts ordering process implemented.",
      ...
      customerFeedback: "Thank you for working with us on this. We appreciate the quick resolution.",
      preventativeAction: "Implemented priority parts ordering for critical accounts. Updated SLA to guarantee 24-hour response.",
      createdBy: "customer-service-001",
    });
    ...
    const unauthorizedDisputeEscalated = await storage.createBillingDispute({
      ...
      customerComplaint: "Invoice includes charges for equipment we returned 2 months ago. We specifically requested termination of this contract.",
      ...
      priorityLevel: 1,
      internalNotes: "Customer is threatening legal action. Need to verify equipment return receipt and contract termination documentation.",
      researchNotes: "Equipment return logged on Aug 15. Contract termination request submitted Sep 1. System continued billing. This is our error.",
      escalated: true,
      escalatedTo: "vp-operations-001",
      escalatedAt: new Date(now.getTime() - 2 * 24 * 60 * 60 * 1000),
      escalationReason: "Customer threatened legal action. Full invoice reversal required. Senior management approval needed.",
      communicationLog: [
        {
          date: new Date(now.getTime() - 4 * 24 * 60 * 60 * 1000),
        ...
        },
        {
          ...
          user: "manager-001",
          type: "phone",
          message: "Called customer. Confirmed equipment returned Aug 15. Customer is very upset. Escalating to VP."
        },
        {
          date: new Date(now.getTime() - 2 * 24 * 60 * 60 * 1000),
          user: "vp-operations-001",
          type: "phone",
          message: "Called customer personally. Apologized. Committed to full credit and review of termination process."
        }
      ],
      requiresManagerApproval: true,
    ...
    const successLog1 = await storage.createInvoiceGenerationLog({
      ...
      status: "success",
      meterReadingsUsed: [
        { meterReadingId: "meter-reading-001", bw: 42000, color: 12500 }
      ],
      billingRulesApplied: [
    ...
    const failedLog = await storage.createInvoiceGenerationLog({
      ...
      errorMessage: "No meter reading found for billing period. Customer must submit meter reading before invoice can be generated.",
      warningsGenerated: [
        { type: "missing_data", message: "No meter reading submitted for October 2024" },
        { type: "billing_delayed", message: "Invoice generation delayed until meter reading received" }
      ],
      triggeredBy: "system",
      triggeredByEvent: "scheduled_billing",
    });
    ...
    const partialLog = await storage.createInvoiceGenerationLog({
      ...
      status: "partial",
      meterReadingsUsed: [
        { meterReadingId: "meter-reading-003", bw: 52000, color: 15000 }
      ],
      billingRulesApplied: [
      ...
      warningsGenerated: [
        ...
        { type: "customer_notification", message: "Customer should be contacted to verify equipment status" }
      ],
      sentToCustomer: true,
    ...
    const weeklySchedule = await storage.createBillingSchedule({
      ...
      dayOfWeek: 5, // Friday
      nextRunDate: new Date(now.getFullYear(), now.getMonth(), now.getDate() + ((5 - now.getDay() + 7) % 7)),
    ...
    const issuedCredit = await storage.createCreditMemo({
      ...
      creditDescription: "Partial credit for 3-day service interruption due to delayed technician visit and parts backorder.",
      creditStatus: "issued",
    ...
    const approvedCredit = await storage.createCreditMemo({
      ...
      creditDescription: "Billing rate correction - customer charged $0.01/page instead of contracted rate of $0.008/page for 32,500 B&W pages.",
      ...
      metadata: {
        calculationDetails: "32,500 pages x ($0.01 - $0.008) = $65.00 overcharge x 5 months = $325.00",
    ...
    const pendingCredit = await storage.createCreditMemo({
      ...
      creditDescription: "Goodwill credit for customer inconvenience during meter reading verification process.",
    ...
    const voidedCredit = await storage.createCreditMemo({
      ...
      creditAmount: "500.00",
      creditReason: "billing_error",
      creditDescription: "Credit for duplicate invoice (later determined to be valid separate invoice)",
      creditStatus: "voided",
      appliedToInvoice: false,
      ...
      voidReason: "Customer error - invoices were for different billing periods. Both invoices valid. Credit memo voided per customer request.",
      metadata: {
