  ...
  try {
    ...
    for (const product of tonerProducts) {
      ...
      const existing = await db
        .select()
        .from(supplies)
        .where(sql`${supplies.productCode} = ${product.productCode} AND ${supplies.tenantId} = ${DEFAULT_TENANT_ID}`)
        .limit(1);
    ...
    for (const product of tonerProducts) {
      ...
      const existing = await db
        .select()
        .from(inventoryItems)
        .where(sql`${inventoryItems.partNumber} = ${product.productCode} AND ${inventoryItems.tenantId} = ${DEFAULT_TENANT_ID}`)
        .limit(1);
      ...
      console.log(`   ✅ Created inventory for ${product.productCode} (${product.stock.available} available)`);
    }
        ...
        if (needsUpdate) {
          await db
            ...
            .set({
              ...
              phone: user.phone || `+1555${Math.floor(Math.random() * 10000000).toString().padStart(7, '0')}`,
            })
