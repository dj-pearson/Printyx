...
class AIEmployeeService {
  ...
  async assignTask(
    ...
    try {
      ...
      if (!employeeId) {
        employeeId = await this.findBestEmployeeForTask(
          tenantId,
          taskData.taskType,
  ...
  private async performLeadQualification(taskData: any): Promise<any> {
    ...
    return {
      ...
      aiRecommendation:
        analysis.score >= 70 ? 'Proceed with sales process' : 'Continue lead nurturing',
      confidence: 0.85,
    };
  }
  ...
  private async performReportGeneration(taskData: any): Promise<any> {
    ...
    return {
      ...
      executiveSummary:
        'Performance exceeded expectations with strong growth across all key metrics.',
      visualizations: [
  ...
  private assessPriority(ticket: any): string {
    ...
    return highPriorityKeywords.some((keyword) => description.includes(keyword))
      ? 'high'
      : 'medium';
  }
