...
export class ConsentManagementService {
  ...
  async withdrawConsent(
    ...
    const [updatedConsent] = await db.update(consentRecords)
      .set({
        status: 'withdrawn',
  ...
  async getActiveConsent(
    ...
    return await db.query.consentRecords.findFirst({
      where: and(
        eq(consentRecords.tenantId, tenantId),
        eq(consentRecords.subjectId, subjectId),
  ...
  async getConsentSummary(
    tenantId: string,
    subjectId: string
  ): Promise<Record<ConsentType, { status: string; givenAt: Date | null; expiresAt: Date | null }>> {
    ...
    const summary: Record<string, { status: string; givenAt: Date | null; expiresAt: Date | null }> = {};
    ...
    return summary as Record<ConsentType, { status: string; givenAt: Date | null; expiresAt: Date | null }>;
  }
  ...
  async expireConsents(): Promise<number> {
    const result = await db.update(consentRecords)
      ...
      .where(and(
        eq(consentRecords.status, 'given'),
        lte(consentRecords.expiresAt, new Date())
      ));
  ...
  async createTemplate(
    ...
    if (data.isDefault) {
      await db.update(consentPreferencesTemplate)
    ...
    const [template] = await db.insert(consentPreferencesTemplate).values({
      ...data,
      tenantId,
    }).returning();
  ...
  async listTemplates(tenantId: string): Promise<ConsentPreferencesTemplate[]> {
    return await db.query.consentPreferencesTemplate.findMany({
      ...
      orderBy: [desc(consentPreferencesTemplate.isDefault), desc(consentPreferencesTemplate.createdAt)],
    });
  }
  ...
  async getDefaultTemplate(tenantId: string): Promise<ConsentPreferencesTemplate | null> {
    return await db.query.consentPreferencesTemplate.findFirst({
      where: and(
        eq(consentPreferencesTemplate.tenantId, tenantId),
        eq(consentPreferencesTemplate.isDefault, true),
        eq(consentPreferencesTemplate.isActive, true)
  ...
  async getConsentStats(tenantId: string): Promise<{
    ...
    const [
      ...
      db.select({ status: consentRecords.status, count: sql`count(*)` })
        .from(consentRecords)
        .where(eq(consentRecords.tenantId, tenantId))
        .groupBy(consentRecords.status),
      db.select({ type: consentRecords.consentType, count: sql`count(*)` })
        .from(consentRecords)
        .where(eq(consentRecords.tenantId, tenantId))
