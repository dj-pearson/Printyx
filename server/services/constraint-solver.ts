...
Key Violations:
${initialSolution.violations.slice(0, 5).map(v => `- ${v.description} (severity: ${v.severity})`).join('\n')}
...
- expectedImprovement: estimated cost reduction percentage`;
   ...
   */
  private countPotentialViolations(variableId: string, value: any, assignment: Record<string, any>): number {
    let violations = 0;
    const tempAssignment = { ...assignment, [variableId]: value };
  ...
  private fallbackSolution(): OptimizationResult {
    ...
    return {
      solution,
      cost: 9999,
      violations: [{ constraintId: 'fallback', severity: 100, description: 'Fallback solution used' }],
      confidence: 0.1,
