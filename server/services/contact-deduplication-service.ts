...
export class ContactDeduplicationService {
  ...
  async createRule(
    ...
    const [rule] = await db.insert(duplicateDetectionRules).values({
      ...data,
      tenantId,
      createdBy,
    }).returning();
  ...
  async getRule(tenantId: string, ruleId: string): Promise<DuplicateDetectionRule | null> {
    return await db.query.duplicateDetectionRules.findFirst({
      where: and(
  ...
  async updateRule(
    ...
    const [updated] = await db.update(duplicateDetectionRules)
  ...
  private async getCandidateRecords(
    ...
    if (entityType === 'contact' || entityType === 'company' || entityType === 'lead' || entityType === 'customer') {
        ...
        switch (field.matchType) {
          case 'exact':
            similarity = field.caseSensitive
              ? (val1 === val2 ? 100 : 0)
              : (val1.toLowerCase() === val2.toLowerCase() ? 100 : 0);
            break;
          case 'fuzzy':
  ...
  async createMatch(
    ...
    if (existing) {
      ...
      if (data.matchScore > existing.matchScore) {
        const [updated] = await db.update(duplicateMatches)
          .set({
    ...
    const [match] = await db.insert(duplicateMatches).values({
      ...data,
      tenantId,
  ...
  async resolveMatch(
    ...
    const [updated] = await db.update(duplicateMatches)
      ...
      .where(and(
        eq(duplicateMatches.id, matchId),
        eq(duplicateMatches.tenantId, tenantId)
      ))
      .returning();
  ...
  async mergeRecords(
    ...
    const [survivingRecord, mergedRecord] = await Promise.all([
      (db.query as any)[tableRef].findFirst({
        where: and(
          eq(table.id, survivingRecordId),
          eq(table.tenantId, tenantId)
        ),
      }),
      (db.query as any)[tableRef].findFirst({
        where: and(
          eq(table.id, mergedRecordId),
          eq(table.tenantId, tenantId)
        ),
      }),
    ]);
    ...
    if (entityType !== 'enhanced_contact') {
      ...
      const contactsResult = await db.update(enhancedContacts)
    ...
    const [mergeHistory] = await db.insert(contactMergeHistory).values({
      tenantId,
      entityType,
      survivingRecordId,
      mergedRecordId,
  ...
  async rollbackMerge(
    ...
    if (history.relatedUpdates) {
      ...
      for (const update of relatedUpdates) {
        if (update.tableName === 'enhanced_contacts') {
          await db.update(enhancedContacts)
    ...
    await db.update(contactMergeHistory)
      .set({
        canRollback: false,
  ...
  async createScanJob(
    ...
    const [job] = await db.insert(duplicateScanJobs).values({
      ...data,
      tenantId,
      triggeredBy,
      status: 'pending',
  ...
  async runScanJob(tenantId: string, jobId: string): Promise<void> {
    const job = await db.query.duplicateScanJobs.findFirst({
      where: and(
        eq(duplicateScanJobs.id, jobId),
        eq(duplicateScanJobs.tenantId, tenantId)
      ),
    });
    ...
    await db.update(duplicateScanJobs)
      .set({
        status: 'running',
        startedAt: new Date(),
        updatedAt: new Date(),
  ...
  async getStats(tenantId: string): Promise<{
    ...
    const [
      ...
      db.select({ count: sql`count(*)` })
        ...
        .where(and(
          eq(duplicateMatches.tenantId, tenantId),
          eq(duplicateMatches.status, 'pending')
        )),
      db.select({ count: sql`count(*)` })
        .from(duplicateMatches)
        .where(and(
          eq(duplicateMatches.tenantId, tenantId),
          eq(duplicateMatches.status, 'merged')
        )),
      db.select({ count: sql`count(*)` })
        .from(duplicateMatches)
        .where(and(
          eq(duplicateMatches.tenantId, tenantId),
          eq(duplicateMatches.status, 'dismissed')
        )),
      db.select({ avg: sql`avg(${duplicateMatches.matchScore})` })
        .from(duplicateMatches)
        .where(eq(duplicateMatches.tenantId, tenantId)),
