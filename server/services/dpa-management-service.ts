...
export class DpaManagementService {
  ...
  async createDpa(
    ...
    const [dpa] = await db.insert(dataProcessingAgreements).values({
      ...data,
      tenantId,
      agreementNumber,
      createdBy,
  ...
  async getDpa(tenantId: string, dpaId: string): Promise<DataProcessingAgreement | null> {
    return await db.query.dataProcessingAgreements.findFirst({
      where: and(
  ...
  async updateDpa(
    ...
    const [updated] = await db.update(dataProcessingAgreements)
      .set({
        ...updates,
        updatedBy,
        updatedAt: new Date(),
   ...
   */
  async submitForReview(tenantId: string, dpaId: string, submittedBy?: string): Promise<DataProcessingAgreement> {
   ...
   */
  async approveForSignature(tenantId: string, dpaId: string, approvedBy?: string): Promise<DataProcessingAgreement> {
  ...
  async renewDpa(
    ...
    return await this.updateDpa(tenantId, dpaId, {
      status: 'renewed',
   ...
   */
  async getExpiringDpas(tenantId: string, withinDays: number = 30): Promise<DataProcessingAgreement[]> {
  ...
  async expireDpas(): Promise<number> {
    const result = await db.update(dataProcessingAgreements)
      .set({
        status: 'expired',
        updatedAt: new Date(),
      })
  ...
  async createComplianceCheck(
    ...
    const [check] = await db.insert(dpaComplianceChecks).values({
      ...data,
      tenantId,
    }).returning();

    // Update DPA with last check date
    await db.update(dataProcessingAgreements)
      .set({
  ...
  async getComplianceCheck(tenantId: string, checkId: string): Promise<DpaComplianceCheck | null> {
    return await db.query.dpaComplianceChecks.findFirst({
      where: and(
  ...
  async updateComplianceCheck(
    ...
    const [updated] = await db.update(dpaComplianceChecks)
  ...
  async getStats(tenantId: string): Promise<{
    ...
    const [
      ...
      byRiskResult,
      expiringResult,
      pendingComplianceResult,
    ] = await Promise.all([
      db.select({ count: sql`count(*)` })
        .from(dataProcessingAgreements)
        .where(eq(dataProcessingAgreements.tenantId, tenantId)),
      db.select({ status: dataProcessingAgreements.status, count: sql`count(*)` })
        .from(dataProcessingAgreements)
        .where(eq(dataProcessingAgreements.tenantId, tenantId))
        .groupBy(dataProcessingAgreements.status),
      db.select({ riskLevel: dataProcessingAgreements.riskLevel, count: sql`count(*)` })
        .from(dataProcessingAgreements)
        .where(eq(dataProcessingAgreements.tenantId, tenantId))
    ...
    return {
      ...
      byRiskLevel: Object.fromEntries(byRiskResult.map(r => [r.riskLevel || 'unknown', Number(r.count)])),
