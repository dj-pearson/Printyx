...
interface ReschedulingEvent {
  id: string;
  type: 'task_added' | 'task_completed' | 'task_delayed' | 'resource_unavailable' | 'deadline_changed' | 'priority_changed';
  timestamp: Date;
  data: any;
...
interface ReschedulingStrategy {
  ...
  actions: Array<{
    type: 'reoptimize' | 'shift_tasks' | 'reassign_resource' | 'adjust_priorities' | 'add_buffer_time';
    parameters: any;
  }>;
  priority: number;
...
class DynamicReschedulingService {
  ...
  async triggerRescheduling(event: ReschedulingEvent): Promise<ReschedulingResult> {
    ...
    return {
      ...
      strategy: 'queued',
      changedTasks: [],
      impact: { tasksRescheduled: 0, resourcesAffected: 0, optimizationImprovement: 0, userNotificationsRequired: 0 },
      confidence: 1.0,
  ...
  private evaluateCondition(condition: any, event: ReschedulingEvent): boolean {
    ...
    switch (condition.operator) {
      ...
      case 'ne': return eventValue !== condition.value;
      ...
      case 'contains': return typeof eventValue === 'string' && eventValue.includes(condition.value);
   ...
   */
  private async executeStrategy(strategy: ReschedulingStrategy, event: ReschedulingEvent): Promise<ReschedulingResult> {
    const changedTasks = [];
  ...
  private async executeReoptimization(parameters: any, event: ReschedulingEvent): Promise<any> {
    ...
    return {
      changedTasks: event.affectedTasks.map(taskId => ({
        ...
        newSchedule: { startTime: new Date(Date.now() + 60000), endTime: new Date(Date.now() + 120000) },
   ...
   */
  private async executeResourceReassignment(parameters: any, event: ReschedulingEvent): Promise<any> {
   ...
   */
  private async calculateReschedulingConfidence(changedTasks: any[], event: ReschedulingEvent): Promise<number> {
    try {
...
Changes Made:
${changedTasks.slice(0, 5).map(ct => `- ${ct.taskId}: ${ct.reason}`).join('\n')}
...
Return confidence score (0.0 to 1.0):`;
   ...
   */
  private async generateReschedulingReasoning(strategy: ReschedulingStrategy, event: ReschedulingEvent, changedTasks: any[]): Promise<string> {
    try {
...
Provide a concise explanation of why this rescheduling was necessary and how it improves the schedule.`;
  ...
  private calculateNextReviewTime(strategy: ReschedulingStrategy, event: ReschedulingEvent): Date {
    ...
    switch (event.impact) {
      case 'critical': reviewMinutes = 15; break;
      case 'high': reviewMinutes = 30; break;
      case 'medium': reviewMinutes = 60; break;
      case 'low': reviewMinutes = 180; break;
    }
  ...
  private initializeDefaultStrategies(): void {
    ...
    this.strategies.set('urgent_task_added', {
      ...
      triggers: ['task_added'],
      conditions: [
        { field: 'data.priority', operator: 'in', value: ['asap', 'high'] }
      ],
      actions: [
        { type: 'reoptimize', parameters: { scope: 'affected_resources' } }
      ],
      priority: 9,
      cooldownMinutes: 5
    });
    ...
    this.strategies.set('deadline_changed', {
      ...
      conditions: [],
      actions: [
        { type: 'reoptimize', parameters: { scope: 'deadline_affected' } }
      ],
      priority: 8,
      cooldownMinutes: 15
    });
  ...
  getStatistics(): {
    ...
    return {
      ...
      strategiesUsed,
      averageTasksRescheduled: this.reschedulingHistory.length > 0 ? totalTasksRescheduled / this.reschedulingHistory.length : 0,
      averageConfidence: this.reschedulingHistory.length > 0 ? totalConfidence / this.reschedulingHistory.length : 0
    };
  }
}
