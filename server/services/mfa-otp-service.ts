...
export async function sendSmsOtp(
  ...
  try {
    ...
    const smsResult = await sendSmsMessage(phoneNumber, `Your Printyx verification code is: ${code}`);
...
class TwilioSmsProvider implements SmsProvider {
  ...
  async send(phoneNumber: string, message: string): Promise<{ success: boolean; messageId?: string }> {
 ...
 */
async function sendSmsMessage(phoneNumber: string, message: string): Promise<{ success: boolean; messageId?: string }> {
