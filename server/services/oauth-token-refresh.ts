 ...
 */
async function markIntegrationForReauth(
  integrationId: string,
  error: string
): Promise<void> {
  try {
...
export async function refreshExpiringTokens(): Promise<{
  ...
  try {
    ...
    console.log(`[TOKEN REFRESH] Checked ${result.total} integrations, refreshed ${result.refreshed}, failed ${result.failed}`);
  } catch (error) {
...
export function startTokenRefreshJob(): void {
  ...
  console.log(`[TOKEN REFRESH] Started periodic refresh job (every ${TOKEN_REFRESH_CONFIG.checkIntervalMinutes} minutes)`);
