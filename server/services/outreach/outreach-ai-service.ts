 ...
 * Outreach AI Service
 *
 * Wraps claude-ai-service with outbound-specific prompt engineering.
 *
 * Responsibilities:
 * 1. Build the system prompt from business context + specialty knowledge packs
 * 2. Generate sequence templates (email + LinkedIn coordinated)
 * 3. Generate per-prospect personalized drafts from a sequence step
 * 4. Run a lightweight spam-word linter on generated copy
 */
...
} from './specialty-knowledge-packs';
import type {
  BusinessContext,
  OutreachSequence,
...
const log = createModuleLogger('outreach-ai-service');
...
// Basic spam trigger words — not exhaustive, flags for rep review.
const SPAM_TRIGGERS = [
  'free',
  'guaranteed',
  'act now',
...
Return JSON with this exact schema:
{
  ...
  log.info({ angle: input.angle, channels }, 'Generating sequence');

  const raw = await claudeAI.generateCompletion({
    system,
...
CHANNEL: ${channelRules}${angleBlock}${templateBlock}
...
Return ONLY JSON:
{
  ...
  log.info({ channel: input.channel, prospectId: p.id }, 'Generating draft');
...
function parseJsonResponse<T>(raw: string): T {
  ...
  // Find the first { or [ to be safe against stray preamble
  ...
  try {
    ...
    log.error({ raw: raw.slice(0, 500) }, 'Failed to parse AI JSON response');
