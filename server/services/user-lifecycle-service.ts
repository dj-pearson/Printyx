...
export class UserLifecycleService {
  ...
  static async offboardUser(params: OffboardUserParams): Promise<{
    ...
    this.processOffboarding(workflow.id, tenantId).catch((err) =>
      log.error(err, 'Failed to process offboarding'),
    );
  ...
  private static async processOffboarding(workflowId: string, tenantId: string): Promise<void> {
    ...
    try {
      ...
      await this.updateWorkflowStep(workflowId, 0, true, {
        notes: 'Stub: report generation not implemented',
      });
      ...
      await this.updateWorkflowStep(workflowId, 1, true, {
        exported: false,
        notes: 'Stub: data export not implemented',
      });
      ...
      if (workflow.transferOwnershipTo) {
        ...
        log.warn(
          { workflowId, transferTo: workflow.transferOwnershipTo },
          'Offboarding step 3 (ownership transfer) not yet implemented',
        );
        await this.updateWorkflowStep(workflowId, 2, true, {
          notes: 'Stub: ownership transfer not implemented',
        });
      } else {
      ...
      await this.updateWorkflowStep(workflowId, 3, true, {
        notes: 'Stub: role revocation not implemented',
      });
      ...
      log.warn(
        { workflowId },
        'Offboarding step 5 (integration access revocation) not yet implemented',
      );
      await this.updateWorkflowStep(workflowId, 4, true, {
        notes: 'Stub: integration revocation not implemented',
      });
      ...
      await this.updateWorkflowStep(workflowId, 5, true, {
        notes: 'Stub: session termination not implemented',
      });
      ...
      await this.updateWorkflowStep(workflowId, 6, true, {
        notes: 'Stub: user archival not implemented',
      });
      ...
      await this.updateWorkflowStep(workflowId, 7, true, {
        notes: 'Stub: audit report not implemented',
      });
