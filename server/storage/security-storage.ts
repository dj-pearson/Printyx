...
export async function getComplianceSettings(
  ...
  try {
    ...
    return {
      ...
      sessionTimeoutMinutes: settings.sessionTimeoutMinutes ?? DEFAULT_COMPLIANCE_SETTINGS.sessionTimeoutMinutes,
      sessionWarningMinutes: settings.sessionWarningMinutes ?? DEFAULT_COMPLIANCE_SETTINGS.sessionWarningMinutes,
      maxConcurrentSessions: settings.maxConcurrentSessions ?? DEFAULT_COMPLIANCE_SETTINGS.maxConcurrentSessions,
      forceLogoutOnSuspiciousActivity: settings.forceLogoutOnSuspiciousActivity ?? DEFAULT_COMPLIANCE_SETTINGS.forceLogoutOnSuspiciousActivity,
      ...
      encryptSensitiveFields: settings.encryptSensitiveFields ?? DEFAULT_COMPLIANCE_SETTINGS.encryptSensitiveFields,
      maskDataInLogs: settings.maskDataInLogs ?? DEFAULT_COMPLIANCE_SETTINGS.maskDataInLogs,
      auditRetentionPeriod: settings.auditRetentionPeriod ?? DEFAULT_COMPLIANCE_SETTINGS.auditRetentionPeriod,
      ...
      dataRetentionPeriod: settings.dataRetentionPeriod ?? DEFAULT_COMPLIANCE_SETTINGS.dataRetentionPeriod,
    };
  } catch (error) {
  ...
  try {
    ...
    await db
      ...
      .where(
        and(
          eq(securitySessions.isActive, true),
          lt(securitySessions.expiresAt, now)
        )
      );
