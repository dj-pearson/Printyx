...
export function registerSalesforceTestRoutes(app: Express) {
  ...
  app.get('/api/test/salesforce/mappings', async (req, res) => {
    try {
      const accountMappings = SALESFORCE_FIELD_MAPPINGS.find(
        (m) => m.salesforceObject === 'Account',
      );
      const contactMappings = SALESFORCE_FIELD_MAPPINGS.find(
        (m) => m.salesforceObject === 'Contact',
      );
      const opportunityMappings = SALESFORCE_FIELD_MAPPINGS.find(
        (m) => m.salesforceObject === 'Opportunity',
      );
  ...
  app.get('/api/test/salesforce/health', async (req, res) => {
    try {
      ...
      res.json({
        ...
        message: allHealthy
          ? 'All Salesforce integration components operational'
...
async function testFieldMappings() {
  try {
    ...
    return {
      status:
        hasAccountMappings && hasContactMappings && hasOpportunityMappings ? 'healthy' : 'degraded',
      details: {
