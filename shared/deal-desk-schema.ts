...
export const approvalRequests = pgTable(
  'approval_requests',
  {
    ...
    conditionsAttached: jsonb('conditions_attached').$type<
      Array<{
        condition: string;
        required: boolean;
        completedBy?: string;
...
export const approvalComments = pgTable(
  ...
  (table) => ({
    ...
    tenantRequestIdx: index('approval_comments_tenant_request_idx').on(
      table.tenantId,
...
export const discountAnalytics = pgTable(
  'discount_analytics',
  {
    ...
    // Top Approvers
    topApprovers: jsonb('top_approvers').$type<
      Array<{
        approverId: string;
        approverName: string;
  ...
  (table) => ({
    ...
    aggregationIdx: index('discount_analytics_aggregation_idx').on(
...
export const approvalDelegations = pgTable(
  ...
  (table) => ({
    ...
    tenantActiveIdx: index('approval_delegations_tenant_active_idx').on(
      table.tenantId,
      table.isActive,
    ),
