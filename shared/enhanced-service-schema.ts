...
export const ticketPartsRequests = pgTable('ticket_parts_requests', {
  ...
  tenantId: varchar('tenant_id').notNull(),
  serviceTicketId: varchar('service_ticket_id').notNull(),
  sessionId: uuid('session_id').references(() => technicianTicketSessions.id),
  technicianId: varchar('technician_id').notNull(),
...
export const technicianTicketSessionsRelations = relations(
...
export const insertTechnicianTicketSessionSchema = createInsertSchema(
  technicianTicketSessions,
).omit({
  id: true,
  createdAt: true,
