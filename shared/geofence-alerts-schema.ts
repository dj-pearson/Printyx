...
export const geofenceAlertRules = pgTable(
  ...
  (table) => ({
    ...
    geofenceIdIdx: index("geofence_alert_rules_geofence_id_idx").on(table.tenantId, table.geofenceId),
    activeIdx: index("geofence_alert_rules_active_idx").on(table.tenantId, table.isActive),
    triggerTypeIdx: index("geofence_alert_rules_trigger_type_idx").on(table.tenantId, table.triggerType),
  })
);
...
export const geofenceAlerts = pgTable(
  ...
  (table) => ({
    ...
    technicianIdIdx: index("geofence_alerts_technician_id_idx").on(table.tenantId, table.technicianId),
    ...
    acknowledgedIdx: index("geofence_alerts_acknowledged_idx").on(table.tenantId, table.isAcknowledged),
...
export const insertGeofenceAlertSubscriptionSchema = createInsertSchema(geofenceAlertSubscriptions).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const technicianDwellSessions = pgTable(
  ...
  (table) => ({
    ...
    technicianIdIdx: index("dwell_sessions_technician_id_idx").on(table.tenantId, table.technicianId),
