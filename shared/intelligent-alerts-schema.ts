...
export const alertStatusEnum = pgEnum('alert_status', ['new', 'triaged', 'investigating', 'contained', 'resolved', 'false_positive']);
...
export const resolutionConfidenceEnum = pgEnum('resolution_confidence', ['very_low', 'low', 'medium', 'high', 'very_high']);
...
export const alertTriageResults = pgTable("alert_triage_results", {
  ...
  // Pattern Matching
  similarIncidents: jsonb("similar_incidents").$type<Array<{
    incidentId: string;
  ...
  autoContainmentActions: jsonb("auto_containment_actions").$type<Array<{
    actionType: string;
    target: string;
    executed: boolean;
    executedAt?: string;
...
export const incidentResolutionPatterns = pgTable("incident_resolution_patterns", {
  ...
  // Resolution Steps
  commonResolutionSteps: jsonb("common_resolution_steps").$type<Array<{
    stepNumber: number;
    description: string;
    automated: boolean;
    command?: string;
  ...
  // Root Causes
  commonRootCauses: jsonb("common_root_causes").$type<Array<{
    cause: string;
  ...
  // Prevention Measures
  preventionMeasures: jsonb("prevention_measures").$type<Array<{
    measure: string;
...
export const insertAutomatedContainmentLogSchema = createInsertSchema(automatedContainmentLogs).omit({
  id: true,
  createdAt: true,
});
...
export const insertIncidentResolutionPatternSchema = createInsertSchema(incidentResolutionPatterns).omit({
  id: true,
  createdAt: true,
});

export const insertProactiveThreatDetectionSchema = createInsertSchema(proactiveThreatDetection).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertResolutionSuggestionFeedbackSchema = createInsertSchema(resolutionSuggestionFeedback).omit({
  id: true,
  createdAt: true,
});
