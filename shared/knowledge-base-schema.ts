...
export const articleFeedback = pgTable(
  ...
  (table) => ({
    articleTypeIdx: index('article_feedback_article_type_idx').on(
      table.articleId,
      table.feedbackType,
    ),
...
export const aiContentGenerationQueue = pgTable(
...
export const articleEmbeddings = pgTable(
  'article_embeddings',
  {
    ...
    // Metadata
    embeddingConfidence: decimal('embedding_confidence', { precision: 5, scale: 2 }).default(
      '0.95',
    ),
  ...
  (table) => ({
    articleIdx: unique('article_embedding_unique').on(
      table.articleId,
      table.embeddingScope,
      table.sectionId,
    ),
...
export const knowledgeSearchQueries = pgTable(
...
export const articleRatings = pgTable(
  ...
  (table) => ({
    userArticleTypeIdx: unique('rating_user_article_type_unique').on(
      table.userId,
      table.articleId,
      table.ratingType,
    ),
