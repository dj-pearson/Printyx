 ...
 * Accept.
 *
 * The Node/browser twin of supabase/functions/proposals/_acquisition.ts, which is
 * the Deno copy the accept handler runs (index.ts imports zod and pdf-lib from
 * esm.sh, so it cannot be loaded outside Deno). LeaseForm's ?proposalId= prefill
 * imports THIS one, so the draft a rep sees is the draft the server would have
 * written. server/tests/unit/proposal-acquisition.test.ts locks them together.
 * Everything here is pure: the caller does the IO.
 *
