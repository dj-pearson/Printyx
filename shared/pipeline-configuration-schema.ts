...
export const pipelineTemplates = pgTable("pipeline_templates", {
  ...
  tenantActiveIdx: index('pipeline_templates_tenant_active_idx').on(table.tenantId, table.isActive),
}));
...
export const pipelineStages = pgTable("pipeline_stages", {
  ...
  // Automation Triggers
  automationTriggers: jsonb("automation_triggers").$type<Array<{
    triggerType: 'on_enter' | 'on_exit' | 'on_sla_breach' | 'on_field_change';
    actionType: 'send_email' | 'create_task' | 'update_field' | 'webhook' | 'assign_to' | 'notify';
  ...
  tenantPipelineIdx: index('pipeline_stages_tenant_pipeline_idx').on(table.tenantId, table.pipelineTemplateId),
...
export const stageTransitions = pgTable("stage_transitions", {
  ...
  automaticConditions: jsonb("automatic_conditions").$type<Array<{
    field: string;
    operator: string;
    value: any;
  ...
  // Transition Actions
  onTransitionActions: jsonb("on_transition_actions").$type<Array<{
...
export const pipelineAutomationLogs = pgTable("pipeline_automation_logs", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: varchar("tenant_id").notNull(),
  dealId: varchar("deal_id").notNull(),
