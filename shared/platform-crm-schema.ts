...
import { pgTable, varchar, text, timestamp, boolean, integer, decimal, jsonb, index, pgEnum, unique } from 'drizzle-orm/pg-core';
...
export const platformRecordStatusEnum = pgEnum('platform_record_status', [
  ...
  // Tenant statuses
  'active_customer', 'at_risk', 'renewal_pending', 'churned', 'former_customer',
  // Terminal statuses
  'lost', 'disqualified', 'not_a_fit'
]);

export const platformDealStageEnum = pgEnum('platform_deal_stage', [
  'prospecting', 'qualification', 'demo_scheduled', 'demo_completed',
  'trial_started', 'proposal', 'negotiation', 'closed_won', 'closed_lost'
]);

export const platformActivityTypeEnum = pgEnum('platform_activity_type', [
  'call', 'email', 'meeting', 'demo', 'proposal', 'trial_check_in',
  'support_ticket', 'renewal_discussion', 'executive_review', 'note', 'task'
]);

export const platformHealthStatusEnum = pgEnum('platform_health_status', [
  'excellent', 'healthy', 'at_risk', 'critical', 'churned'
]);
...
export const platformLeadTierEnum = pgEnum('platform_lead_tier', [
  'hot', 'warm', 'cold'
]);
...
export const platformBusinessRecords = pgTable('platform_business_records', {
  ...
  nextFollowUpIdx: index('platform_business_records_next_followup_idx').on(table.nextFollowUpDate),
...
export const platformContacts = pgTable('platform_contacts', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  businessRecordId: varchar('business_record_id').notNull().references(() => platformBusinessRecords.id, { onDelete: 'cascade' }),
  ...
  businessRecordIdIdx: index('platform_contacts_business_record_id_idx').on(table.businessRecordId),
...
export const platformDeals = pgTable('platform_deals', {
  ...
  expectedCloseDateIdx: index('platform_deals_expected_close_date_idx').on(table.expectedCloseDate),
...
export const platformActivities = pgTable('platform_activities', {
  ...
  businessRecordIdIdx: index('platform_activities_business_record_id_idx').on(table.businessRecordId),
...
export const platformLeadScoreCalculations = pgTable('platform_lead_score_calculations', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  businessRecordId: varchar('business_record_id').notNull().references(() => platformBusinessRecords.id, { onDelete: 'cascade' }).unique(),
  ...
  businessRecordIdIdx: index('platform_lead_score_business_record_id_idx').on(table.businessRecordId),
...
export const platformBantQualification = pgTable('platform_bant_qualification', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  businessRecordId: varchar('business_record_id').notNull().references(() => platformBusinessRecords.id, { onDelete: 'cascade' }).unique(),
  ...
  decisionMakerContactId: varchar('decision_maker_contact_id').references(() => platformContacts.id),
...
export const platformSalesTerritories = pgTable('platform_sales_territories', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
...
export const platformLeadAssignmentRules = pgTable('platform_lead_assignment_rules', {
  ...
  // Assignment Target
  assignToTerritoryId: varchar('assign_to_territory_id').references(() => platformSalesTerritories.id),
...
export const platformLeadAssignmentHistory = pgTable('platform_lead_assignment_history', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  businessRecordId: varchar('business_record_id').notNull().references(() => platformBusinessRecords.id, { onDelete: 'cascade' }),
  ...
  businessRecordIdIdx: index('platform_assignment_history_record_id_idx').on(table.businessRecordId),
...
export const platformHealthScores = pgTable('platform_health_scores', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  businessRecordId: varchar('business_record_id').notNull().references(() => platformBusinessRecords.id, { onDelete: 'cascade' }).unique(),
  tenantId: varchar('tenant_id').references(() => tenants.id),
...
export const platformChurnPredictions = pgTable('platform_churn_predictions', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  businessRecordId: varchar('business_record_id').notNull().references(() => platformBusinessRecords.id, { onDelete: 'cascade' }),
  tenantId: varchar('tenant_id').references(() => tenants.id),
  ...
  businessRecordIdIdx: index('platform_churn_predictions_record_id_idx').on(table.businessRecordId),
  churnRiskIdx: index('platform_churn_predictions_risk_idx').on(table.churnRisk),
  predictedChurnDateIdx: index('platform_churn_predictions_date_idx').on(table.predictedChurnDate),
}));

export const platformSuccessInterventions = pgTable('platform_success_interventions', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  businessRecordId: varchar('business_record_id').notNull().references(() => platformBusinessRecords.id, { onDelete: 'cascade' }),
  ...
  // Related Records
  relatedHealthScoreId: varchar('related_health_score_id').references(() => platformHealthScores.id),
  relatedChurnPredictionId: varchar('related_churn_prediction_id').references(() => platformChurnPredictions.id),
...
export const platformRenewalOpportunities = pgTable('platform_renewal_opportunities', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  businessRecordId: varchar('business_record_id').notNull().references(() => platformBusinessRecords.id, { onDelete: 'cascade' }),
  tenantId: varchar('tenant_id').references(() => tenants.id),
...
export const platformSalesGoals = pgTable('platform_sales_goals', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
...
export const platformActivityReports = pgTable('platform_activity_reports', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  ...
  userPeriodIdx: unique('platform_activity_reports_user_period_idx').on(table.userId, table.period, table.periodStart),
...
export const platformCohortAnalysis = pgTable('platform_cohort_analysis', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
