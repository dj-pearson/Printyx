import { pgTable, varchar, integer, decimal, boolean, timestamp, jsonb, text, pgEnum, index } from "drizzle-orm/pg-core";
...
export const calculatorLeads = pgTable("calculator_leads", {
  ...
  sequenceIdx: index('calculator_leads_sequence_idx').on(table.emailSequenceStarted, table.emailSequenceDay),
...
export const emailSequenceTracking = pgTable("email_sequence_tracking", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  ...
  leadSequenceIdx: index('email_sequence_tracking_lead_sequence_idx').on(table.leadId, table.sequenceDay),
}));
...
export const industryBenchmarks = pgTable("industry_benchmarks", {
  ...
  avgDowntimeHoursPerMonth: decimal("avg_downtime_hours_per_month", { precision: 6, scale: 2 }).notNull(),
  avgServiceCallsPerDevice: decimal("avg_service_calls_per_device", { precision: 4, scale: 2 }).notNull(),
  ...
  industryCompanySizeIdx: index('industry_benchmarks_industry_company_size_idx').on(table.industry, table.companySize),
}));

// ==================== Calculator Analytics Events ====================
// Track detailed user interactions for optimization
export const calculatorAnalyticsEvents = pgTable("calculator_analytics_events", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  ...
  sessionEventIdx: index('calculator_analytics_events_session_event_idx').on(table.sessionId, table.eventType),
}));
