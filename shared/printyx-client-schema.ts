 ...
 */
export const clientCollectedMetrics = pgTable('client_collected_metrics', {
  id: serial('id').primaryKey(),
  tenantId: integer('tenant_id').notNull(),
  ...
  tenantSerialIdx: index('client_collected_metrics_tenant_serial_idx').on(table.tenantId, table.serialNumber),
  collectionTimeIdx: index('client_collected_metrics_collection_time_idx').on(table.collectionTimestamp),
...
export const monitoredDevices = pgTable('monitored_devices', {
  ...
  tenantClientIdx: index('monitored_devices_tenant_client_idx').on(table.tenantId, table.clientId),
...
export const clientActivityLogs = pgTable('client_activity_logs', {
  ...
  tenantClientIdx: index('client_activity_logs_tenant_client_idx').on(table.tenantId, table.clientId),
 ...
 */
export const tonerAlerts = pgTable('toner_alerts', {
  id: serial('id').primaryKey(),
  tenantId: integer('tenant_id').notNull(),
  clientId: text('client_id').notNull(),
 ...
 */
export const oidMappings = pgTable('oid_mappings', {
  id: serial('id').primaryKey(),
 ...
 */
export const deviceMeterHistory = pgTable('device_meter_history', {
  id: serial('id').primaryKey(),
  tenantId: integer('tenant_id').notNull(),
  serialNumber: text('serial_number').notNull(),
  ...
  tenantSerialIdx: index('device_meter_history_tenant_serial_idx').on(table.tenantId, table.serialNumber),
  readingTimeIdx: index('device_meter_history_reading_time_idx').on(table.readingTimestamp),
  billingPeriodIdx: index('device_meter_history_billing_period_idx').on(table.billingPeriodStart, table.billingPeriodEnd),
}));
