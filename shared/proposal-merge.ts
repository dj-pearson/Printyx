// PROP-005: shared proposal merge engine.
//
// Turns an authored template (sections of HTML with {{merge.tokens}}) + a quote
// record + a branding profile into rendered, customer-facing HTML sections.
//
// CONSTRAINTS:
//  - Dependency-free + no Node/DOM-only imports, so the Deno edge functions
//    (PDF / email render) AND the browser (preview) can both import it.
//  - Customer-facing ONLY: never emits unit_cost, dealer cost, or margin.
//  - Injected DATA values are HTML-escaped at substitution time, so quote/customer
//    data can't inject markup. The surrounding template HTML is admin-authored and
//    is sanitized via the existing sanitizeRichHtml pathway at save/display time
//    (isomorphic-dompurify is not Deno-safe, so it cannot live here).

// .ts extension is required so Deno (edge PDF/email render) can import this; the
// project's tsconfig has allowImportingTsExtensions, so Node tooling accepts it too.
import { MERGE_TOKENS } from './proposal-merge-fields.ts';
...
export interface MergeLineItem {
  ...
  discount?: number | string | null; // per-line discount amount (QUOTE-016)
  isRecurring?: boolean | null;
  recurringFrequency?: string | null; // monthly | quarterly | annually
}
...
// ─── Formatting (canonical — PROP-007's PDF should adopt these) ───────────────
...
const DATE_FMT = new Intl.DateTimeFormat('en-US', {
  ...
  month: 'long',
  day: 'numeric',
  // Render the stored calendar date as-is, not shifted by the viewer's timezone.
  timeZone: 'UTC',
});
...
// Attribute-safe URL: block javascript:/vbscript:/data: schemes, then escape quotes.
...
// Builds the token → resolved-HTML/text map for one quote+branding context.
// Scalar values are HTML-escaped; *Table / logo are pre-built HTML blocks.
function buildTokenMap(data: MergeData): Record<string, string> {
  ...
  const recurringTotal =
    q.recurringTotal !== undefined && q.recurringTotal !== null && q.recurringTotal !== ''
      ? (() => {
          // Derive a frequency label from the first recurring line, default monthly.
          const freq =
...
/**
 * Replace every {{token}} in `content`. Known tokens resolve from `tokenMap`
 * (missing data → empty string). Unknown tokens render empty and are collected
 * into `warnings`.
 */
...
/**
 * Render every visible section of a template against the quote/branding data.
 * Returns per-section HTML, a combined document, and any unknown-token warnings.
 * Callers must run the output through their HTML sanitizer before display/persist.
 */
