...
export const renewalActivities = pgTable('renewal_activities', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  tenantId: varchar('tenant_id').notNull(),
  ...
  tenantRenewalIdx: index('renewal_activities_tenant_renewal_idx').on(table.tenantId, table.renewalId),
...
export const renewalPlaybooks = pgTable('renewal_playbooks', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  tenantId: varchar('tenant_id').notNull(),
...
export const expansionOpportunities = pgTable('expansion_opportunities', {
  ...
  // Proposed Solution
  proposedProducts: jsonb('proposed_products').$type<Array<{
    productId: string;
    productName: string;
    quantity: number;
    mrr: number;
  ...
  proposedServices: jsonb('proposed_services').$type<Array<{
    serviceType: string;
    description: string;
    mrr?: number;
    oneTimePrice?: number;
