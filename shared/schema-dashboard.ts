...
export const dashboardWidgetLibrary = pgTable('dashboard_widget_library', {
  id: varchar('id').primaryKey().default(sql`gen_random_uuid()`),
  
  tenantId: varchar('tenant_id').references(() => tenants.id, { onDelete: 'cascade' }),
  
  // Widget reference
  widgetId: varchar('widget_id').notNull().references(() => dashboardWidgets.id, { onDelete: 'cascade' }),
