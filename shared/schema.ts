...
export {
  ...
  insertDeviceAlertSchema,
  insertDeviceSupplyOrderSchema,
...
export const mfaBackupCodes = pgTable(
  ...
  (table) => {
    return {
      ...
      idxMfaBackupCodesUserUnused: index('idx_mfa_backup_codes_user_unused').on(
        table.userId,
        table.isUsed,
      ),
    };
...
export const mfaAuditLogs = pgTable(
  'mfa_audit_logs',
  {
    id: varchar('id')
      .primaryKey()
  ...
  (table) => {
    return {
      ...
      idxMfaAuditLogsEventType: index('idx_mfa_audit_logs_event_type').on(
        table.eventType,
        table.createdAt,
      ),
      idxMfaAuditLogsUserEvent: index('idx_mfa_audit_logs_user_event').on(
        table.userId,
        table.eventType,
        table.createdAt,
      ),
...
export const leasePayments = pgTable(
  'lease_payments',
  {
    id: varchar('id')
      .primaryKey()
...
export const leaseRenewals = pgTable(
  'lease_renewals',
  {
    id: varchar('id')
      .primaryKey()
...
export const leaseDispositions = pgTable(
  'lease_dispositions',
  {
    id: varchar('id')
      .primaryKey()
...
export const productAccessoriesRelations = relations(productAccessories, ({ one, many }) => ({
  tenant: one(tenants, {
...
export const integrationCredentials = pgTable(
  ...
  (table) => ({
    tenantProviderIdx: index('integration_credentials_tenant_provider_idx').on(
      table.tenantId,
      table.provider,
    ),
...
export const signatureRequests = pgTable(
  ...
  (table) => ({
    tenantRequestNumberUnique: unique('signature_requests_tenant_request_number_unique').on(
      table.tenantId,
      table.requestNumber,
    ),
...
export const signatureSigners = pgTable(
  ...
  (table) => ({
    requestOrderIdx: index('signature_signers_request_order_idx').on(
      table.requestId,
      table.signerOrder,
    ),
...
export const signatureDocuments = pgTable(
  ...
  (table) => ({
    requestOrderIdx: index('signature_documents_request_order_idx').on(
      table.requestId,
      table.documentOrder,
    ),
  }),
...
export const signatureAuditLogs = pgTable(
  'signature_audit_logs',
  {
    ...
    // Related records
    requestId: varchar('request_id').references(() => signatureRequests.id, {
      onDelete: 'cascade',
    }),
    signerId: varchar('signer_id').references(() => signatureSigners.id, { onDelete: 'cascade' }),
    documentId: varchar('document_id').references(() => signatureDocuments.id, {
      onDelete: 'cascade',
    }),
...
export const installations = pgTable(
  'installations',
  {
    ...
    customerId: varchar('customer_id')
      .notNull()
      .references(() => businessRecords.id, { onDelete: 'restrict' }),
    equipmentId: varchar('equipment_id').references(() => equipment.id, { onDelete: 'restrict' }),
    serviceTicketId: varchar('service_ticket_id').references(() => serviceTickets.id, {
      onDelete: 'set null',
    }),
  ...
  (table) => ({
    ...
    numberUnique: unique('installations_number_unique').on(
      table.tenantId,
...
export const serviceSignatures = pgTable(
  'service_signatures',
  {
    id: varchar('id')
      .primaryKey()
...
export const installationChecklists = pgTable(
...
export const emailUnsubscribes = pgTable(
  ...
  (table) => ({
    ...
    unique: unique('email_unsubscribes_unique').on(
      table.tenantId,
      table.email,
      table.unsubscribeType,
    ),
  }),
);
