...
export const ssoProviderConfigs = pgTable(
  'sso_provider_configs',
  {
    ...
    jitProvisioningRules: jsonb('jit_provisioning_rules')
      .$type<{
  ...
  (table) => ({
    ...
    uniqueTenantProvider: unique('sso_provider_configs_tenant_provider_unique').on(
      table.tenantId,
      table.providerType,
      table.name,
    ),
...
export const ssoUserMappings = pgTable(
  'sso_user_mappings',
  {
    ...
    providerId: uuid('provider_id')
      .notNull()
      .references(() => ssoProviderConfigs.id, { onDelete: 'cascade' }),
  ...
  (table) => ({
    ...
    uniqueProviderExternalId: unique('sso_user_mappings_provider_external_id_unique').on(
      table.providerId,
      table.externalId,
    ),
    uniqueUserProvider: unique('sso_user_mappings_user_provider_unique').on(
      table.userId,
      table.providerId,
    ),
  }),
