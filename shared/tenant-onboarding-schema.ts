...
export const tenantOnboardingStepStatusEnum = pgEnum('tenant_onboarding_step_status', [
  'pending',
  'in_progress',
  'completed',
  'skipped',
...
export const healthScoreGradeEnum = pgEnum('health_score_grade', [
  'excellent',
  'good',
  'fair',
  'poor',
...
export const tenantOnboardingTemplates = pgTable('tenant_onboarding_templates', {
  ...
  // User Role Templates
  userRoleTemplates: jsonb('user_role_templates').$type<
    Array<{
...
export const dataImportValidations = pgTable('data_import_validations', {
  ...
  duplicateDetails: jsonb('duplicate_details').$type<
    Array<{
      row: number;
      duplicateOf: number;
  ...
  bulkFixSuggestions: jsonb('bulk_fix_suggestions').$type<
    Array<{
...
export const onboardingAnalytics = pgTable('onboarding_analytics', {
  ...
  // Bottlenecks
  commonBottlenecks: jsonb('common_bottlenecks').$type<
    Array<{
      step: number;
      stepName: string;
...
export const tenantCloneOperations = pgTable('tenant_clone_operations', {
  ...
  // What to Clone
  cloneSettings: jsonb('clone_settings')
    .$type<{
  ...
  // Modifications
  modifications: jsonb('modifications').$type<
    Array<{
      field: string;
      originalValue: any;
      newValue: any;
...
export const insertTenantOnboardingTemplateSchema = createInsertSchema(
...
export const insertTenantOnboardingSessionSchema = createInsertSchema(
