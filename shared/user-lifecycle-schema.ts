...
export const lifecycleStatusEnum = pgEnum('lifecycle_status', ['pending', 'in_progress', 'completed', 'failed']);

export const accessReviewStatusEnum = pgEnum('access_review_status', ['not_started', 'in_progress', 'completed', 'overdue']);

export const reviewDecisionEnum = pgEnum('review_decision', ['approved', 'revoked', 'modified', 'escalated']);
...
export const userProvisioningTemplates = pgTable("user_provisioning_templates", {
  ...
  onboardingSteps: jsonb("onboarding_steps").$type<Array<{
    stepNumber: number;
    stepName: string;
    description: string;
    automated: boolean;
...
export const onboardingChecklists = pgTable("onboarding_checklists", {
  ...
  // Check-ins
  checkIns: jsonb("check_ins").$type<Array<{
    day: number;
    scheduledAt: string;
    completedAt?: string;
    notes?: string;
...
export const offboardingWorkflows = pgTable("offboarding_workflows", {
  ...
  equipmentList: jsonb("equipment_list").$type<Array<{
    itemType: string;
    serialNumber?: string;
    returnedAt?: string;
  }>>(),
  ...
  // Compliance & Audit
  complianceChecklist: jsonb("compliance_checklist").$type<Array<{
    item: string;
    completed: boolean;
    completedAt?: string;
    notes?: string;
...
export const accessReviewCertifications = pgTable("access_review_certifications", {
  ...
  userSnapshot: jsonb("user_snapshot").$type<{
    name: string;
    email: string;
    department?: string;
    jobTitle?: string;
  ...
  currentPermissions: jsonb("current_permissions").$type<{
    [resource: string]: string[];
  }>().notNull(),
  elevatedAccess: jsonb("elevated_access").$type<Array<{
    permission: string;
    grantedAt: string;
    lastUsed?: string;
  }>>(),
...
export const insertUserProvisioningTemplateSchema = createInsertSchema(userProvisioningTemplates).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export const insertAccessReviewCertificationSchema = createInsertSchema(accessReviewCertifications).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
