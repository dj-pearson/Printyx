...
export const fpyMetrics = pgTable('fpy_metrics', {
  ...
  fpyByTechnician: jsonb('fpy_by_technician')
    .$type<Record<string, { total: number; firstPass: number; percentage: number }>>()
    .default({}),
  fpyByEquipmentType: jsonb('fpy_by_equipment_type')
    .$type<Record<string, { total: number; firstPass: number; percentage: number }>>()
    .default({}),

  // Defect analysis
  topDefectTypes: jsonb('top_defect_types')
    .$type<{ defectType: string; count: number; percentage: number }[]>()
    .default([]),
...
// Relations
export const warehouseKittingOperationsRelations = relations(
...
// Insert schemas
export const insertWarehouseKittingOperationSchema = createInsertSchema(
  warehouseKittingOperations,
).omit({
  id: true,
  createdAt: true,
