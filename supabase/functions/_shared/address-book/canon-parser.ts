...
import { CanonCryptoError, decryptCanonPwd } from './canon-crypto.ts';
...
// PA-055: async in the edge tree. Web Crypto has no synchronous API, so
// decryptCanonPwd is async here and the row loop awaits it. The Node copy stays
// synchronous; the parity test pins the SCHEME, not the signature.
export async function parseCanon(buffer: Uint8Array, password?: string): Promise<ParseResult> {
      ...
      if (objectclass === 'email') {
        ...
        if (raw.pwd) {
          if (pwdEncrypted && password) {
            try {
              md._password = await decryptCanonPwd(raw.pwd, password);
            } catch (err) {
              const reason =
