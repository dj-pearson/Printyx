 ...
 */
// PA-055: node:crypto's randomUUID becomes the Web Crypto one, which every
// runtime here has.
...
import { encryptCanonPwd } from './canon-crypto.ts';
...
// PA-055: async for the same reason as parseCanon - encryptCanonPwd is Web
// Crypto here.
  ...
  for (const entry of entries) {
    ...
    if (isEmailEntry(entry)) {
      ...
      if (usePassword) {
        ...
        if (plaintext) {
          row.pwd = await encryptCanonPwd(plaintext, options.password!);
        } else {
          row.pwd = '';
