// Purchase-agreement HTML renderer for edge functions (PROD-013).
//
// KEEP IN SYNC with generateDocumentHTML in server/routes-documents.ts. Copied
// verbatim so the document a tenant exports is byte-identical whichever backend
// served it, following the _shared/print-cost-calculator.ts idiom.
//
// Two things inherited from the Express original, deliberately NOT changed here
// because a silent divergence between the two backends would be worse than the
// bugs themselves:
//   - The seller line is the hardcoded string "INFOMAX OFFICE SYSTEMS, INC.".
//     In a multi-tenant product every tenant's agreement names that company.
//     The fix is to source it from the tenant (white_label_config.company_name
//     is the obvious candidate) and must land on both sides at once.
//   - The output is HTML, not PDF. The caller must not label it .pdf.

type DocumentRow = Record<string, any>;

export function generateDocumentHtml(doc: DocumentRow): string {
