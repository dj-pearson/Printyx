// Print Cost Calculator — shared calculation engine (Deno port).
//
// This is a near-verbatim port of server/services/print-cost-calculator-service.ts.
// The Express service is a dependency-free pure-function module (no DB, no Node
// APIs), so the two copies stay in lockstep. Keep them in sync: the
// public-calculator edge function (EDGE-005f) uses this copy so the
// /api/public/calculator/* flow resolves in production (functions.printyx.net),
// where the Express server is unreachable.
//
// One intentional divergence: FLEET_AGE_MULTIPLIERS lookup falls back to
// under_2_years for an unknown fleetAge instead of throwing on undefined
// (the Express version would crash). Validation upstream still restricts the
// value, so this only hardens the public, unauthenticated entry point.
