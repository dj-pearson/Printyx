/**
 * Print Cost Calculator — pure calculation engine (EDGE-005f port).
 *
 * Verbatim Deno port of server/services/print-cost-calculator-service.ts. The
 * source is dependency-free pure TypeScript (no Node APIs, no Date/Math.random),
 * so it ports unchanged — keep the two files in lockstep. The math is exercised
 * by supabase/functions/print-cost-calculator/calculator.test.ts (deno test).
 */
