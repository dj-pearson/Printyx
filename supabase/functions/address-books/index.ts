...
import {
  ...
  handleImportExistingBook,
  handleImportNewBook,
} from './_import-export.ts';
...
export default async function handler(req: Request) {
  ...
  try {
    ...
    //
    // PA-052 found all five of these paths called by the address-book UI with no
    // branch here at all, so in production they fell to the generic 404 - or, for
    // the download, into getBook() looking up an address book whose id is the
    // literal string "exports", which answered "Address book not found". PA-055
    // ported the vendor layer (supabase/functions/_shared/address-book/) and both
    // halves now run here.
    ...
    // POST /address-books/:id/export - convert the book to a target vendor
    if (req.method === 'POST' && bookId && sub === 'export') {
      return await handleExport(req, admin, tenantId, user.id, bookId);
    }

    // GET /address-books/exports/:id/download - stream the generated bytes
    if (req.method === 'GET' && bookId === 'exports' && entryId === 'download') {
      return await handleExportDownload(req, admin, tenantId, sub);
    }
