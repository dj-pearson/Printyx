...
export default async function handler(req: Request) {
  ...
  try {
    ...
    // /inventory, making this correct whether or not the prefix survived.
    const { parts } = normalizePath(url.pathname, 'inventory');
    const itemId = parts[0];
    const action = parts[1]; // 'adjust', 'reserve', 'release', etc.
