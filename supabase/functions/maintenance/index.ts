...
export default async function handler(req: Request) {
  ...
  try {
    ...
    // /maintenance, making this correct whether or not the prefix survived.
    const { parts } = normalizePath(url.pathname, 'maintenance');
    const endpoint = parts[0]; // /maintenance/schedules, /maintenance/upcoming, etc.
