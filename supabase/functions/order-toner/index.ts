...
export default async function handler(req: Request) {
  ...
  try {
    ...
    // /order-toner, making this correct whether or not the prefix survived.
    const { parts } = normalizePath(url.pathname, 'order-toner');
    const deviceId = parts[0]; // devices/:id/order-toner
