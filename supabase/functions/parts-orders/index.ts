...
export default async function handler(req: Request) {
  ...
  try {
    ...
    // Coolify-safe: server.ts strips the function-name prefix, so the order id
    // is parts[0] and the sub-resource ('items') is parts[1]. normalizePath
    // also tolerates the native Supabase runtime (prefix still present).
