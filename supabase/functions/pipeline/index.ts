...
export default async function handler(req: Request) {
  ...
  try {
    ...
    // /pipeline, making this correct whether or not the prefix survived.
    const { parts } = normalizePath(url.pathname, 'pipeline');
    const resource = parts[0]; // 'stages', 'config', 'stats'
