 ...
 *   POST /predictions/:id/approve        create the service ticket (see _ticket.ts)
...
import { ticketFromPrediction } from './_ticket.ts';
...
async function handleScore(
  ...
  // Scoring creates no tickets (round 207): the draft it used to insert carried
  // a status the CHECK constraint refuses, so none was ever saved. A prediction
  // at or above the threshold is a dispatch CANDIDATE, counted here and turned
  // into a ticket only when a human approves it (_ticket.ts).
  for (const s of scored) {
    const serviceTicketId: string | null = null;
    if (s.confidence >= threshold) aboveThreshold++;
  ...
  return createCorsResponse(
    {
      ...
      // `created` is kept for callers of the old shape; nothing is created here.
      created: 0,
      aboveThreshold,
      threshold,
...
async function handlePredictionAction(
  ...
  const { data: prediction } = await admin
    .from('equipment_failure_predictions')
    .select('id, machine_id, status, confidence, predicted_window_end, signals, service_ticket_id')
    .eq('id', predictionId)
    .eq('tenant_id', tenantId)
    .maybeSingle();
  ...
  if (action === 'approve') {
    if (prediction.status === 'approved' || prediction.service_ticket_id) {
      return createCorsResponse(
        { message: 'This prediction has already been approved.', code: 'ALREADY_APPROVED' },
        409,
        req,
      );
    }
    const { data: machine, error: machineError } = await admin
      ...
      .eq('id', prediction.machine_id)
      .eq('tenant_id', tenantId)
      .maybeSingle();
    if (machineError) throw machineError;

    const plan = ticketFromPrediction(prediction, machine, {
      tenantId,
      userId,
      now: new Date(nowIso),
      ticketNumber: `PF-${Date.now().toString(36)}-${crypto.randomUUID().slice(0, 4)}`,
    });
    if (!plan.ok) {
      return createCorsResponse({ message: plan.message, code: plan.code }, 409, req);
    }

    // Claim the prediction first, conditioned on its current status, so two
    // approvals racing each other create one ticket rather than two.
    const { data: claimed, error: claimError } = await admin
      .from('equipment_failure_predictions')
      .update({ ...base, status: 'approved' })
      .eq('id', predictionId)
      .eq('tenant_id', tenantId)
      .eq('status', prediction.status)
      .is('service_ticket_id', null)
      .select('id');
    if (claimError) throw claimError;
    if (!claimed || claimed.length === 0) {
      return createCorsResponse(
        { message: 'This prediction was reviewed by someone else.', code: 'ALREADY_REVIEWED' },
        409,
        req,
      );
    }
    ...
    if (ticketError || !ticket) {
      // Release the claim so the approval can be retried; a prediction marked
      // approved with no ticket would read as dispatched when nothing was.
      const { error: releaseError } = await admin
        .from('equipment_failure_predictions')
        .update({ status: prediction.status, reviewed_by_user_id: null, reviewed_at: null })
        .eq('id', predictionId)
        .eq('tenant_id', tenantId);
      if (releaseError) console.error('[predictive-failure] release failed:', releaseError.message);
      throw ticketError ?? new Error('Service ticket insert returned no row');
    }
  ...
  if (action === 'dismiss') {
    // No ticket exists until approval (round 207), so dismissing has nothing to
    // cancel. This does NOT set `outcome`: a reviewer dismissing a prediction
    // is not the same evidence as a confirmed false positive, and the accuracy
    // report derives outcomes from closed tickets instead.
    const updated = await updatePrediction(admin, tenantId, predictionId, {
      ...base,
      status: 'dismissed',
    });
