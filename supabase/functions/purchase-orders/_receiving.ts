/**
 * Receiving a purchase order: the arithmetic, in one place (WF-P-02).
 *
 * The receive handler already existed and already moved inventory, but every
 * decision it made was inline in the request loop, so none of it could be tested
 * and the same rules had to be rewritten to serve the endpoint on the other host.
 * /api/purchase-orders is not proxied, so Express serves dev and the edge function
 * serves production - the split that let WF-P-01 drop every line item in
 * production while dev looked fine.
 *
 * Everything here is pure. The callers own the I/O.
 *
 * THREE RULES WORTH STATING, because each was a decision:
 *
 * OVER-RECEIPT IS ALLOWED AND REPORTED, not silently clamped and not rejected. A
 * vendor really does ship 11 when you ordered 10, and refusing the receipt leaves
 * the warehouse holding stock the system says does not exist. Clamping to the
 * ordered quantity loses the same stock more quietly. So the excess is applied and
 * named in `overReceipts`, and the caller surfaces it.
 *
 * A SERIALIZED LINE DOES NOT MOVE BULK INVENTORY. inventory_items.is_serialized
 * marks stock tracked one unit at a time; those units become equipment rows with
 * serial numbers (WF-L-04), and adding them to quantity_on_hand as well would
 * count them twice. Such a line still records its received quantity and is
 * returned in `serialCaptureRequired` so the caller can say what is outstanding
 * rather than reporting a clean receipt.
 *
 * STATUS IS DERIVED FROM THE LINES, NEVER FROM THE RECEIPT. A receipt that
 * completes the last outstanding line makes the order `received`; anything else
 * with at least one unit in makes it `partially_received`. Computing it from what
 * was just received instead would call a second partial receipt "received"
 * whenever it happened to be the larger one.
 */
