...
export default async function handler(req: Request) {
  ...
  try {
    ...
    // /tenant-settings, making this correct whether or not the prefix survived.
    const { parts } = normalizePath(url.pathname, 'tenant-settings');
    const settingCategory = parts[0];
