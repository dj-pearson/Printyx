...
function simulateWorkflowProgress(workflowManager, roleManager, workflowId, targetStage) {
  ...
  stages.forEach((stage, index) => {
    try {
      workflowManager.advanceWorkflow(
        workflowId,
        stage,
        {
...
if (dashboard.bottlenecks.length > 0) {
  ...
  dashboard.bottlenecks.forEach((bottleneck) => {
    console.log(
      `  ${bottleneck.stageName}: ${bottleneck.workflowCount} workflows (${bottleneck.assignedRole})`,
    );
  });
}
...
if (dashboard.upcomingDeadlines.length > 0) {
  ...
  dashboard.upcomingDeadlines.forEach((deadline) => {
    console.log(
      `  Customer ${deadline.customerId}: ${deadline.daysRemaining} days remaining (${deadline.currentStage})`,
    );
  });
}
...
if (userDashboard) {
  ...
  if (userDashboard.workflows.length > 0) {
    ...
    userDashboard.workflows.forEach((workflow) => {
      console.log(
        `  • Customer: ${workflow.customerId} - Stage: ${workflow.currentStageName} (${Math.round(workflow.progressPercentage)}%)`,
      );
...
if (workflowProgress) {
  ...
  console.log(
    `Assigned To: ${workflowProgress.assignedRole.name} (${workflowProgress.assignedRole.department})`,
  );
...
if (handoffQueue.length > 0) {
  handoffQueue.forEach((handoff) => {
    ...
    console.log(
      `  From: ${handoff.fromStage} (${handoff.fromRole}) → To: ${handoff.toStage} (${handoff.toRole})`,
    );
