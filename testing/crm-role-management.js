class CRMRoleManager {
  ...
  assignWorkflow(workflowId, userId) {
    ...
    if (!user.permissions.canView.includes(workflow.currentStage)) {
      throw new Error(
        `User ${userId} does not have permission to work on stage ${workflow.currentStage}`,
      );
    }
  ...
  executeHandoff(workflowId, fromStage, toStage, handoffData = {}) {
    ...
    // Send notifications
    this.sendHandoffNotifications(
      workflowId,
      currentUser,
      targetUser,
      handoffRule.notificationTemplate,
    );
  ...
  findAvailableUser(role) {
    const usersInRole = Array.from(this.users.values()).filter(
  ...
  findManagerForRole(role) {
    ...
    if (managerRole) {
      return Array.from(this.users.values()).find(
        (user) => user.role === managerRole && user.active,
      );
    }
  ...
  getUserByWorkflow(workflowId) {
    return Array.from(this.users.values()).find((user) => user.assignedWorkflows.has(workflowId));
  }
  ...
  updateUserStats(userId, workflow) {
    ...
    if (user) {
      ...
      user.averageCompletionTime =
        (user.averageCompletionTime * (user.completedWorkflows - 1) + completionTime) /
        user.completedWorkflows;
  ...
  getUserDashboard(userId) {
    ...
    const overdue = assignedWorkflows.filter((w) => w.estimatedCompletion < new Date());

    const urgent = assignedWorkflows.filter(
      (w) => (w.estimatedCompletion - new Date()) / (1000 * 60 * 60 * 24) <= 3,
    );

    const blocked = assignedWorkflows.filter((w) => w.blockers.length > 0);
  ...
  getRoleWorkloadReport() {
    ...
    Array.from(this.users.values()).forEach((user) => {
      ...
      if (user.assignedWorkflows.size > 5) {
  ...
  canUserAdvanceWorkflow(userId, workflowId, targetStage) {
    ...
    return (
      user.permissions.canAdvance.includes(workflow.currentStage) &&
