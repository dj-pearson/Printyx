class CRMWorkflowManager {
  ...
  calculateStageDuration(workflow, stage) {
    const stageEntry = workflow.stageHistory.reverse().find((h) => h.toStage === stage);
  ...
  getWorkflowsByRole(role) {
    return Array.from(this.workflows.values()).filter((workflow) => workflow.assignedTo === role);
  }

  getWorkflowsByStage(stage) {
    return Array.from(this.workflows.values()).filter(
      (workflow) => workflow.currentStage === stage,
    );
  }
  ...
  generateDashboard() {
    ...
    allWorkflows.forEach((workflow) => {
      ...
      // Role workload
      roleWorkload[workflow.assignedTo] = (roleWorkload[workflow.assignedTo] || 0) + 1;
  ...
  calculateAverageCompletionTime() {
    const completedWorkflows = Array.from(this.workflows.values()).filter(
      (w) => w.currentStage === 'maintenance_monitoring',
    );
