...
async function runDemoTest() {
  ...
  console.log('==========================================');
  console.log(`Testing ${DEMO_ROUTES.length} sample routes to verify setup...\n`);
  ...
  try {
    ...
    for (const route of DEMO_ROUTES) {
      ...
      try {
        ...
        if (response.status() >= 200 && response.status() < 300) {
          ...
          console.log(`  ✅ SUCCESS (${loadTime}ms) - Found ${buttons.length} buttons`);
        } else {
    ...
    console.log('\n🎉 Demo test completed!');
    console.log('If all routes showed SUCCESS, your setup is working correctly.');
