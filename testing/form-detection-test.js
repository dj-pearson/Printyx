...
async function testFormDetection() {
  ...
  try {
    ...
    const needsAuth =
      ...
      authText.includes('authenticate') ||
    ...
    if (formTriggerButtons.length > 0) {
      try {
        ...
        formsAfterClick.forEach((form) => {
          console.log(
            `  - Form ${form.index}: ${form.inputCount} inputs, visible: ${form.visible}`,
          );
        });
      } catch (error) {
    ...
    const formLikeElements = await page.$$eval(
      '[role="form"], .form, [data-testid*="form"], [class*="form"]',
      (elements) => {
