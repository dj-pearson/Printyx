...
const HOOK_PATTERNS = {
  ...
  useEffect:
    /useEffect\s*\(\s*(?:async\s*)?\([^)]*\)\s*=>\s*\{([^}]*(?:\{[^}]*\}[^}]*)*)\}\s*,\s*\[([^\]]*)\]/gs,
  useCallback:
    /useCallback\s*\(\s*(?:async\s*)?\([^)]*\)\s*=>\s*\{([^}]*(?:\{[^}]*\}[^}]*)*)\}\s*,\s*\[([^\]]*)\]/gs,
...
export class HookAnalyzer {
  ...
  async analyzeFile(filePath: string): Promise<FileAnalysis> {
    ...
    for (const match of useEffectMatches) {
      ...
      if (effectBody.includes('await ') && match[0].includes('async (')) {
        issues.push({
          ...
          severity: 'warning',
          suggestedFix:
            'Define an async function inside useEffect and call it, rather than making the effect callback async',
        });
      }
    ...
    for (const match of customHookMatches) {
      // Skip built-in hooks
      if (
        ['useState', 'useEffect', 'useCallback', 'useMemo', 'useRef', 'useContext'].some((h) =>
