...
export class TestOrchestrator {
  ...
  constructor(userConfig: Partial<TestConfig> = {}) {
    ...
    this.edgeFunctionTester = new EdgeFunctionTester(
      this.config,
      this.logger.child('EdgeFunctionTester'),
    );
    this.performanceTester = new PerformanceTester(
      this.config,
      this.logger.child('PerformanceTester'),
    );
    this.accessibilityTester = new AccessibilityTester(
      this.config,
      this.logger.child('AccessibilityTester'),
    );
  ...
  private async testPages(pages: DiscoveredPage[]): Promise<void> {
    ...
    for (const pageInfo of pages) {
      ...
      this.logger.progress(
        testedCount,
        pages.length,
        `Testing: ${pageInfo.url.substring(0, 50)}...`,
      );
  ...
  private async testPage(pageInfo: DiscoveredPage): Promise<PageTestReport> {
    ...
    try {
      ...
      if (pageInfo.forms.length > 0) {
        const formResults = await this.formTester.testForms(
          this.page,
          pageInfo.forms,
          pageInfo.url,
        );
      ...
      const elementResults = await this.elementTester.testElements(
        this.page,
        pageInfo,
        pageInfo.url,
      );
  ...
  private printSummary(report: TestReport): void {
    ...
    if (report.errors.length > 0) {
      ...
      report.errors.slice(0, 5).forEach((error) => {
        this.logger.error(
          `${error.classification.toUpperCase()}: ${error.message.substring(0, 80)}...`,
        );
      });
    }
