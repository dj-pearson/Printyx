...
export class ParallelRunner {
      ...
      async (task, index) => {
        ...
        try {
          ...
          this.logger.progress(
            completedCount,
            totalTasks,
            `Worker ${worker.id}: ${task.url.substring(0, 40)}...`,
          );
        } catch (error) {
          this.logger.warn(
            `Worker ${worker.id} failed on ${task.url}: ${(error as Error).message}`,
          );
  ...
  getStats(): {
    ...
    const parallelTime =
      Math.max(...Object.values(tasksPerWorker).map((count) => count * avgTaskDuration)) || 1;
