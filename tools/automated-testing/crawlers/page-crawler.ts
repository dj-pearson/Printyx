...
import { normalizeUrl, isInternalUrl, shouldTestUrl, sleep, generateId } from '../utils/helpers';
    ...
    try {
      const formData = await page.evaluate((selectors) => {
        ...
        return Array.from(formElements).map((form, index) => {
          ...
          // Get form fields
          const fields = Array.from(formEl.querySelectorAll('input, textarea, select')).map(
            (field) => {
          ...
          return {
            ...
            selector: button.id
              ...
              : button.textContent?.trim()
                ? `button:has-text("${button.textContent.trim().substring(0, 30)}")`
            ...
            return {
              ...
              selector: element.id
                ...
                : element.tagName.toLowerCase() +
                  (element.classList.length ? `.${element.classList[0]}` : ''),
