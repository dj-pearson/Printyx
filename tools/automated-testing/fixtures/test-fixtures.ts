    ...
    try {
      ...
      if (this.context.supabaseClient) {
        ...
        const supabase = this.context.supabaseClient as {
          from: (table: string) => {
            insert: (data: unknown[]) => {
              select: () => Promise<{ data: { id: string }[] | null; error: Error | null }>;
            };
          };
        };

        const { data, error } = await supabase.from(fixture.table).insert(fixture.data).select();
    ...
    try {
      if (this.context.supabaseClient) {
        ...
        if (fixture.cleanup === 'truncate') {
          // Delete all records
          await supabase.from(fixture.table).delete().in('id', createdIds);
        } else {
