...
export class ReportGenerator {
  ...
  generateReport(
    ...
    const performanceResults = pageReports.filter((p) => p.performance).map((p) => p.performance!);
  ...
  private generateSummary(
    ...
    // Pages with errors
    const pagesWithErrors = pageReports.filter((p) =>
  ...
  private generateRecommendations(
    ...
    if (summary.avgPageLoadTime > 3000) {
      const slowPages = pageReports.filter((p) => p.loadTime > 3000).map((p) => p.url);
    ...
    if (summary.passRate < 80) {
      recommendations.push({
        ...
        affectedPages: pageReports
          .filter((p) => p.tests.some((t) => t.status === 'failed' || t.status === 'error'))
          .map((p) => p.url),
        suggestedFix:
          'Review failing tests and prioritize fixes based on severity and user impact.',
      });
    }
        ...
        <tbody>
          ${pages
            ...
            .map((page) => {
              ...
              const failed = page.tests.filter(
                (t) => t.status === 'failed' || t.status === 'error',
              ).length;
              const status =
                failed > 0 ? 'failed' : passed === page.tests.length ? 'passed' : 'warning';
              return `
            <tr>
      ...
      <table>
        ...
        <tbody>
          ${report.edgeFunctions
            .map((func) => {
              const status =
                func.reachable && func.statusCode && func.statusCode < 500 ? 'passed' : 'failed';
              return `
            <tr>
...
`;
    for (const page of pages.slice(0, 30)) {
      ...
      const status =
        failed > 0 ? '❌ Failed' : passed === page.tests.length ? '✅ Passed' : '⚠️ Warning';
