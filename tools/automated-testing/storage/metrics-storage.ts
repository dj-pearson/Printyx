...
export class MetricsStorage {
   ...
   */
  getMetricSeries(metricPath: string, lastN: number = 20): MetricSeries {
  ...
  getSummary(): {
    ...
    const passRates = runs.map((r) =>
      r.summary.totalTests > 0 ? (r.summary.passed / r.summary.totalTests) * 100 : 0,
    );
    const avgPassRate =
