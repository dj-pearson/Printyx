...
const A11Y_RULES: A11yRule[] = [
  {
    ...
    check: async (page) => {
      ...
      return {
        ...
        nodes: hasTitle
          ? []
          : [{ html: '<title></title>', target: ['title'], failureSummary: 'Page has no title' }],
      };
    },
  },
  {
          ...
          : [
              {
                html: '<html>',
                target: ['html'],
                failureSummary: 'HTML element is missing lang attribute',
              },
            ],
      };
    },
      ...
      const results = await page.evaluate(() => {
        ...
        focusable.forEach((el, index) => {
          ...
          if (outlineStyle === 'none' || outlineWidth === '0px') {
            ...
            const hasFocusRing =
              el.classList.contains('focus-ring') || el.classList.contains('focus-visible');

            if (!hasFocusRing) {
              violations.push({
                html: el.outerHTML.substring(0, 100),
                target: [
                  el.id ? `#${el.id}` : el.tagName.toLowerCase() + `:nth-of-type(${index + 1})`,
                ],
