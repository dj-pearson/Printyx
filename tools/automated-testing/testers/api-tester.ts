...
export class ApiTester {
    ...
    switch (schema.type) {
      ...
      case 'string':
        ...
        if (schema.minLength && str.length < schema.minLength) {
          errors.push(
            `${path}: string length ${str.length} is less than minimum ${schema.minLength}`,
          );
        }
  ...
  private buildErrorMessage(
    ...
    if (!responseTimeValid) {
      messages.push(
        `Response time ${responseTime}ms exceeds maximum ${endpoint.maxResponseTime}ms`,
      );
    }
  ...
  static createSchema(definition: {
    ...
    properties?: Record<
      string,
      { type: JsonSchema['type']; required?: boolean; nullable?: boolean }
    >;
